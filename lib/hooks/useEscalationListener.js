...
export function useEscalationListener(userId) {
  ...
  useEffect(() => {
    ...
    async function fetchActiveEscalation() {
      ...
      console.log('[useEscalationListener] All Core System chats:', data?.length || 0)
      if (data && data.length > 0) {
        console.log('[useEscalationListener] First 3 chats:', data.slice(0, 3).map(c => ({
          id: c.id,
          title: c.title,
          metadata: c.metadata
        })))
      ...
      const escalations = (data || []).filter(chat => {
        ...
        const isResolved = chat.metadata?.resolved === true
        console.log('[useEscalationListener] Chat', chat.id, ':', { isEscalation, isResolved, metadata: chat.metadata })
        return isEscalation && !isResolved
      })
