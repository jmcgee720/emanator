...
/**
 * Parse multipart/form-data request and extract text + file attachments.
 * Converts uploaded images to base64 data URLs for AI vision processing.
 */
async function parseMultipartRequest(request) {
  ...
  // Extract all file fields from the form
  for (const [key, value] of formData.entries()) {
    if (value instanceof File || (value && typeof value === 'object' && value.constructor?.name === 'File')) {
      ...
      // Convert to base64 data URL for images (AI vision support)
      let dataUrl = null
      let content = null
...
export async function handleStreamMessage(request, { chatId, authUser, dbUser, db }) {
  ...
  // Save user message — always store full content for AI context.
  // Silent/system messages are hidden from UI via metadata.silent flag.
  // CRITICAL: Store attachments in metadata so they persist across chat reloads
  const userMessage = await db.messages.create({
    ...
    metadata: { 
      ...metadata, 
      ...(isSilent ? { silent: true, full_content: content } : {}),
      // Persist attachments in metadata for chat history
      ...(metadata.attachments?.length > 0 ? { attachments: metadata.attachments } : {})
    }
  })
  ...
  const requestedModel = metadata.model || null
  
  // Smart model routing: use the best model for the task
  // Large context tasks (self-edit on big files) → use a high-capability model
  // Quick edits → use the requested model or default
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Send REAL SSE events (not comments) because some CDN/proxy layers ignore comments
      ...
      // Send the saved user message first (skip for silent/system messages)
      if (!isSilent) {
      ...
      let briefProgressAccumulator = null   // captures archetype + plan + waves for persistence

      try {
        // CRITICAL: Only pass selfEditTarget for actual self-edit chats (Core System mode)
        // Regular project chats should NEVER receive selfEditTarget, or they'll get the
        // wrong system prompt and think they're editing Auroraly's code instead of building projects
        const isSelfEditChat = chat.title?.startsWith(SELF_EDIT_PREFIX)
        
        // Debug: log what attachments are being passed
        console.log('[StreamHandler] Passing to AI:', {
          hasMetadata: !!metadata,
          hasAttachments: !!metadata.attachments,
          attachmentCount: metadata.attachments?.length || 0,
          attachmentDetails: metadata.attachments?.map(a => ({
            filename: a.filename,
            category: a.file_category,
            hasContent: !!a.content,
            hasPreview: !!a.preview_data,
        ...
        for await (const evt of generator) {
          // Don't break on client disconnect — let the generator finish
          // so tool calls complete and files get saved to the database.
          // send() already handles closed connections gracefully.
          ...
          // Accumulate new-pipeline progress metadata so we can persist it on message save
          if (evt.event === 'archetype' || evt.event === 'art_direction' || evt.event === 'brief_plan' || evt.event === 'wave_complete' || evt.event === 'review_result' || evt.event === 'screenshot_verify' || evt.event === 'visual_loop_summary') {
            briefProgressAccumulator = briefProgressAccumulator || { waves: [] }
            if (evt.event === 'archetype') briefProgressAccumulator.archetype = evt.data
            if (evt.event === 'art_direction') briefProgressAccumulator.artDirection = evt.data
            if (evt.event === 'brief_plan') {
              briefProgressAccumulator.plan = evt.data
              briefProgressAccumulator.waves = (evt.data.waves || []).map(w => ({ id: w.id, label: w.label, status: 'pending', filesBuilt: [] }))
            }
            if (evt.event === 'wave_complete') {
              briefProgressAccumulator.waves = (briefProgressAccumulator.waves || []).map(w =>
                w.id === evt.data.waveId ? { ...w, status: 'complete', filesBuilt: evt.data.filesBuilt || [] } : w
              )
            }
            if (evt.event === 'review_result') briefProgressAccumulator.review = evt.data
            if (evt.event === 'screenshot_verify') briefProgressAccumulator.screenshotVerify = evt.data
            if (evt.event === 'visual_loop_summary') briefProgressAccumulator.visualLoopSummary = evt.data
          }
        }
        ...
        // Persist new-pipeline brief progress so BriefProgressCard can re-render on chat reload
        if (briefProgressAccumulator) {
          msgMetadata.briefProgress = {
            ...briefProgressAccumulator,
            status: 'complete',
            startedAt: briefProgressAccumulator.startedAt || Date.now() - ((streamMeta.wavesCompleted || 0) * 18000),
        ...
        const chargedModel = finalModel || resolvedModel
        creditsDb.deductCredits(dbUser.id, 'chat_message', { model: chargedModel }).then(result => {
          if (!result.error) {
            send('credits_update', { balance: result.balance, cost: result.cost, model: chargedModel })
          }
        ...
        // Auto-trim errors should never reach the user — they should have been retried
        if (err?.error_type === 'context_length_auto_trim' || err?.message === 'context_length_auto_trim') {
          console.warn('[StreamAPI] context_length_auto_trim leaked to handler — treating as context_length')
          const userFacing = 'This conversation is very long. Please start a new chat — your project files are preserved.'
          const errorMessage = await db.messages.create({
            chat_id: chatId,
            project_id: chat.project_id,
            role: 'assistant',
            content: userFacing,
            metadata: { error: true, error_type: 'context_length', streamed: true }
          })
          send('error', { message: userFacing, error_type: 'context_length', messageId: errorMessage.id })
          send('done', { content: userFacing, messageId: errorMessage.id })
          send('message_saved', { id: errorMessage.id })
          if (!closed) controller.close()
          return
        }
