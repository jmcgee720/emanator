...
export async function handleStreamMessage(request, { chatId, authUser, dbUser, db }) {
  ...
  // Silent/system messages should not appear as user bubbles — save empty string
  const displayContent = isSilent ? '' : (metadata.displayContent || content)
  if (!content) {
  ...
  // Save user message immediately (use displayContent for visible chat, full content goes to AI)
