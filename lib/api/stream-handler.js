...
export async function handleStreamMessage(request, { chatId, authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const estimatedCost = estimateRequestCost(resolvedModel)
      ...
      try {
        ...
        for await (const evt of generator) {
          ...
          if (evt.event === 'error') receivedError = true
        }
        ...
        // If an error event was received with no content, save error message instead of empty one
        if (receivedError && !fullContent) {
          const errorMessage = await db.messages.create({
            ...
            metadata: { error: true, streamed: true, provider: providerName, model: modelName }
          })
        ...
        const isProviderError = err instanceof ProviderError || err.name === 'ProviderError'
        const userFacing = isProviderError ? err.user_message : 'An error occurred while generating the response.'
        ...
        const errorMessage = await db.messages.create({
          ...
          metadata: {
            ...
            error_type: isProviderError ? err.error_type : 'unknown',
            provider: isProviderError ? err.provider : providerName,
            model: isProviderError ? err.model : modelName,
