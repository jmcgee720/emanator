...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // If critical (>75%), block the request and force fork
      ...
      // Soft warning at 65-75%: let the request proceed but notify user
      ...
      if (metadata.attachments && Array.isArray(metadata.attachments) && metadata.attachments.length > 0) {
        const built = buildUserContent(actualContent, metadata.attachments, 'StreamV2/current')
      ...
      try {
        for await (const ev of runAgent({
          ...
          if (ev.type === 'text_delta') {
            ...
            //
            // EXCEPTION (2026-05-28): submit_screenshot_inventory is the
            // most-fabricated-against tool in the stack. Users have NO
            // way to verify the model actually saw what it claims to
            // have seen unless we surface the inventory itself in the
            // UI. Render it as a markdown <details> disclosure so the
            // user can click to expand "what Auroraly literally saw in
            // your screenshot" — and call out fabrications by comparing
            // it against their own image.
            let inline
            if (ev.name === 'submit_screenshot_inventory') {
              const argsJson = (() => {
                try { return JSON.stringify(ev.args, null, 2) } catch { return String(ev.args) }
              })()
