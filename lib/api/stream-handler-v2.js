...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // ── Load prior chat history + extract memory ───────────────
      // Load history early so we can extract session memory BEFORE building
      // system prompts (which inject the memory summary).
