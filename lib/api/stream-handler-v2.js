  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (metadata.attachments && Array.isArray(metadata.attachments) && metadata.attachments.length > 0) {
        console.log('[StreamV2] Processing attachments:', JSON.stringify(metadata.attachments.map(a => ({ type: a.type, hasData: !!a.data, dataLength: a.data?.length }))))
        const contentBlocks = []
        ...
        for (const att of metadata.attachments) {
          if (att.type?.startsWith('image/') && att.data) {
            ...
            if (base64Match) {
              const mediaType = `image/${base64Match[1]}`
              const base64Data = base64Match[2]
              console.log('[StreamV2] Adding image block:', { mediaType, base64Length: base64Data.length })
              contentBlocks.push({
                type: 'image',
                source: {
                  type: 'base64',
              ...
              console.warn('[StreamV2] Failed to parse image data URL:', att.data?.slice(0, 100))
            }
          }
        }
        ...
        if (contentBlocks.some(b => b.type === 'image')) {
          ...
          console.warn('[StreamV2] No image blocks created despite having attachments')
        }
      }
