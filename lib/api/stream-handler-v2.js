...
function buildProjectSystemPrompt(env) {
  return [
    ...
    '',
    `READS / WRITES: All file operations target the project's files in the database. read_file / write_file / edit_file / list_files / search_files operate on this project ONLY. Changes appear in the live preview within a few seconds.`,
    '',
    'Tools available:',
...
function buildSelfEditSystemPrompt(env) {
  ...
  const writeMode = env.writerKind === 'github'
    ? `WRITES: write_file and edit_file commit directly to ${env.repo}@${env.branch} via GitHub. Each edit triggers a Vercel redeploy (~2 minutes).`
    : env.writerKind === 'missing-config'
      ? 'WRITES: this environment requires GitHub-backed writes but GITHUB_TOKEN / GITHUB_REPO are not configured. Calls to write_file / edit_file will return setup instructions. Reads still work.'
      : 'WRITES: write_file and edit_file modify the local filesystem at the codebase root.'
  return [
 ...
 * Converts image attachments to Anthropic's vision format.
 */
  ...
  try {
    ...
    const prior = (rows || [])
      ...
      .map((m) => {
        ...
        // For user messages with image attachments, convert to Anthropic's vision format
        if (m.role === 'user' && m.metadata?.attachments && Array.isArray(m.metadata.attachments)) {
          console.log('[StreamV2] loadPriorMessages: found message with attachments:', { messageId: m.id, attachmentCount: m.metadata.attachments.length })
          const contentBlocks = []
          ...
          for (const att of m.metadata.attachments) {
            if (att.type?.startsWith('image/') && att.data) {
              ...
              const base64Match = att.data.match(/^data:image\/([^;]+);base64,(.+)$/)
              if (base64Match) {
                const mediaType = `image/${base64Match[1]}`
                const base64Data = base64Match[2]
                console.log('[StreamV2] loadPriorMessages: adding image block:', { mediaType, base64Length: base64Data.length })
                contentBlocks.push({
                  type: 'image',
                  source: {
                    type: 'base64',
          ...
          // If we have content blocks (images or text), return as array; otherwise string
          if (contentBlocks.length > 0) {
            console.log('[StreamV2] loadPriorMessages: returning content blocks:', { imageCount: contentBlocks.filter(b => b.type === 'image').length, hasText: contentBlocks.some(b => b.type === 'text') })
      ...
      .filter((m) => {
        // Keep messages that have either text content or content blocks
...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const tools = buildDefaultToolset(scope, writer, reader)
      ...
      // Format current user message with attachments (if any) for vision support
      let currentUserMessage = content
      if (metadata.attachments && Array.isArray(metadata.attachments) && metadata.attachments.length > 0) {
        console.log('[StreamV2] Processing attachments:', JSON.stringify(metadata.attachments.map(a => ({ 
          type: a.type, 
          mime_type: a.mime_type,
          file_category: a.file_category,
          hasData: !!a.data, 
          hasPreviewData: !!a.preview_data,
          dataLength: (a.data || a.preview_data)?.length 
        }))))
        const contentBlocks = []
        
        // Add image blocks first
        for (const att of metadata.attachments) {
          // Check both preview_data (from upload endpoint) and data (legacy/direct)
          const imageData = att.preview_data || att.data
          const isImage = att.file_category === 'image' || att.type?.startsWith('image/') || att.mime_type?.startsWith('image/')
          ...
          if (isImage && imageData) {
            ...
            if (base64Match) {
              const mediaType = `image/${base64Match[1]}`
              const base64Data = base64Match[2]
              console.log('[StreamV2] Adding image block:', { filename: att.filename, mediaType, base64Length: base64Data.length })
              contentBlocks.push({
                type: 'image',
                source: {
                  type: 'base64',
              ...
              console.warn('[StreamV2] Failed to parse image data URL for', att.filename, ':', imageData?.slice(0, 100))
            }
          }
        }
        ...
        if (contentBlocks.some(b => b.type === 'image')) {
          console.log('[StreamV2] Using content blocks with', contentBlocks.filter(b => b.type === 'image').length, 'images')
          currentUserMessage = contentBlocks
        } else {
          console.warn('[StreamV2] No image blocks created despite having attachments. Attachment details:', metadata.attachments.map(a => ({ filename: a.filename, file_category: a.file_category, hasPreviewData: !!a.preview_data, hasData: !!a.data })))
        }
      }
      ...
      try {
        for await (const ev of runAgent({
          ...
          if (ev.type === 'text_delta') {
            ...
            const inline = `> ↳ ${summarizeResult(ev.content)}\n\n`
            fullContent += inline
            send('token', { content: inline })
            send('tool_result', { name: ev.name, id: ev.id, content: ev.content })

            // Preview-refresh hook: when the agent successfully writes or
            // edits a PROJECT file, emit files_saved so the dashboard
            // re-fetches files and reloads the iframe. Without this, edits
            // persist to Supabase but the preview keeps showing the stale
            // version — which makes it look like the edit silently failed.
