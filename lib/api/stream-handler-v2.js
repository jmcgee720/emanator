...
import { buildPersistentMemorySummary, autoSaveFacts } from '@/lib/ai/persistent-memory'
import { buildCoreSystemAwareness } from '@/lib/ai/core-system-awareness'
import { routeModel } from '@/lib/ai/model-router'
import { detectIncident, recordIncident, searchSimilarIncidents, buildIncidentSummary } from '@/lib/ai/agent-learning'
import { buildProactivityProtocols } from '@/lib/ai/proactivity-protocols'
...
function buildProjectSystemPrompt(env) {
  ...
  // Inject incident summary if available (Phase 1 learning)
  const incidentSummary = env.incidentSummary || ''
  ...
  return [
    ...
    '',
    '**AUTO-START (new projects):**',
    '  • When you write an entry-point file (package.json, index.html, vite.config.js, etc.) to a NEW project that has no preview machine yet, the preview infrastructure auto-starts in the background',
    '  • You\'ll see a status message: "Starting preview (detected package.json)…" → "Preview started (machine abc12345). Dev server is installing dependencies — preview will be ready in ~60-90 seconds."',
    '  • After ~90 seconds, you can call `screenshot_preview` or `preview_diagnostics` to verify the app is live',
    '  • If auto-start fails, the status message will tell you — ask the user to click "Start Preview" manually',
    '',
    ...
    '',
    buildProactivityProtocols(),
    '',
    '## SELF-ESCALATION PROTOCOL — AGENT-TO-AGENT COLLABORATION',
    '',
    'When you encounter a task you cannot complete due to a missing tool or capability:',
    '',
    '**AUTOMATIC ESCALATION (preferred):**',
    'Call the `escalate_to_core_system` tool. This creates a joint conversation where you and the Core System agent collaborate in real-time. The user watches both of you work together.',
    '',
    'Example:',
    '```',
    ...
    '',
    'The tool will:',
    '  1. Create an escalation chat',
    '  2. Invite Core System to join',
    '  3. Open a split-screen UI where the user sees both chats',
    '  4. You and Core System collaborate until the capability is built',
    '  5. User clicks "Exit Escalation" when done',
    '',
    '**When to escalate:**',
    '- ✅ Missing tool (run_command, database access, API integration, etc.)',
    '- ✅ Tool exists but has insufficient parameters/options',
    '- ✅ Workflow requires manual steps that should be automatable',
    '- ✅ User explicitly says "I don\'t want to do anything" but you need them to do something',
    '- ✅ You\'re about to give >3 manual terminal commands',
    '',
    '**When NOT to escalate:**',
    '- ❌ User error (wrong API key, typo, etc.) - just fix it',
    ...
    'If `escalate_to_core_system` tool is not available (old chat, missing permissions), draft a feature request and ask the user to paste it into a Core System chat:',
    '   - **Subject:** Clear, specific capability name',
    '   - **REQUEST:** What tool/feature is needed',
    '   - **USE CASE:** The specific user task that\'s blocked',
    '   - **CURRENT BLOCKER:** What you tried and why it failed',
    '   - **PROPOSED SOLUTION:** Tool signature, API design, or implementation approach',
    '   - **BENEFITS:** Why this unblocks users',
    '',
...
function buildSelfEditSystemPrompt(env) {
  ...
  return [
    ...
    buildProactivityProtocols(),
    '',
    '## SELF-ESCALATION PROTOCOL — AUTO-GENERATE FEATURE REQUESTS',
    '',
    'When you encounter a task you cannot complete due to a missing tool or capability:',
    '',
    '1. **Recognize the gap** — identify the specific missing capability (not a workaround, the actual tool/feature needed)',
    '',
    '2. **Draft a feature request** in the same turn, following this template:',
    '   - **Subject:** Clear, specific capability name',
    '   - **REQUEST:** What tool/feature is needed',
    '   - **USE CASE:** The specific user task that\'s blocked',
    '   - **CURRENT BLOCKER:** What you tried and why it failed',
    '   - **PROPOSED SOLUTION:** Tool signature, API design, or implementation approach',
    '   - **SECURITY CONSIDERATIONS:** Risks and mitigations (if applicable)',
    '   - **BENEFITS:** Why this unblocks users',
    '   - **PRIORITY:** Severity and frequency assessment',
    '',
    '3. **Present to user:**',
    '   "I cannot complete [task] because I lack [capability]. I\'ve drafted a feature request below - should I implement this?"',
    '   ',
    '   [Show the draft]',
    '   ',
    '   Options:',
    '   - "Yes" → implement the feature immediately (you have full access to the codebase)',
    '   - "No, just give me manual steps" → fall back to instructions',
    '   - "Yes, but modify X first" → revise the draft',
    '',
    '4. **Do NOT:**',
    '   - ❌ Silently fall back to manual instructions without offering to implement the feature',
    '   - ❌ Say "this isn\'t possible" without explaining it\'s a platform limitation you could fix',
    '   - ❌ Make the user think of the meta-solution (you should think of it)',
    '',
    '**When to self-escalate:**',
    '- ✅ Missing tool (database access, API integration, etc.)',
    '- ✅ Tool exists but has insufficient parameters/options',
    '- ✅ Workflow requires manual steps that should be automatable',
    '- ✅ User explicitly says "I don\'t want to do anything" but you need them to do something',
    '- ✅ You\'re about to give >3 manual terminal commands',
    ...
    '- ❌ User error (wrong API key, typo, etc.) - just fix it',
    '- ❌ Code bug - debug and fix it yourself',
    '- ❌ Missing information you can ask the user for',
    '- ❌ Task is genuinely impossible (violates physics, security, etc.)',
    '',
    '**Core System advantage:** Unlike project agents, you CAN implement the missing capability immediately. Draft the feature request to clarify the design, then ask if you should build it.',
    '',
    'HARD RULES (violating these is a security incident — do not):',
...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  //       (i.e. the project the dashboard's "Core System" button creates), OR
  //   (c) it's an escalation chat (metadata.is_escalation === true) — these
  //       are joint conversations where Core System implements a capability
  //       for a project agent. Core System always responds in escalation chats.
  ...
  const isEscalation = chat.metadata?.is_escalation === true
  const isSelfEdit = isSelfEditByTitle || isCoreProject || isEscalation
  ...
  // ── Incident detection: check if this turn represents a failure pattern ──
  // Load recent history to detect loops, false confidence, redundant questions
  const recentHistory = await db.messages.findByChatId(chatId)
  const incident = detectIncident({
    userMessage: { content: actualContent },
    agentResponse: null, // will be filled after agent responds
  ...
  if (incident) {
    console.log('[StreamV2] incident detected:', incident.incident_type, '—', incident.what_failed.slice(0, 100))
    // Record incident (fire-and-forget, no embedding yet)
    recordIncident(incident).catch((e) => console.warn('[StreamV2] incident record failed:', e?.message))
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // ── Load PERSISTENT cross-session memory FIRST ──────────────────
      // This memory survives chat forks, browser refreshes, and new chats.
      // Load it BEFORE session memory so both can be injected into the
      // system prompt together.
      let persistentMemorySummary = ''
      try {
        persistentMemorySummary = await buildPersistentMemorySummary(
          chat.project_id, // null for self-edit chats (Core System has no project)
          dbUser.id
        )
        if (persistentMemorySummary) {
          console.log('[StreamV2] persistent memory loaded:', {
            projectId: chat.project_id || 'none',
            userId: dbUser.id,
            summaryLength: persistentMemorySummary.length,
          })
        }
      } catch (e) {
        console.warn('[StreamV2] persistent memory load failed:', e?.message)
      }
      ...
      if (priorMessages.length > 0) {
        ...
        sessionMemorySummary = buildMemorySummary(sessionMemory)
        console.log('[StreamV2] session memory extracted:', {
          ...
          memorySummaryLength: sessionMemorySummary.length,
        })
      } else {
        console.log('[StreamV2] new chat (no prior messages) — skipping memory extraction')
      }
      
      // Combine persistent + session memory for system prompt injection
      const memorySummary = [persistentMemorySummary, sessionMemorySummary].filter(Boolean).join('\n\n')

      // ── Search for similar past incidents ──────────────────────────
      // Before responding, check if the user's request matches a past
      // failure pattern. If so, inject the incident summary into the
      // system prompt so the agent learns from history.
      let incidentSummary = ''
      try {
        // For now, skip embedding-based search (requires OpenAI API call)
        // and just load recent incidents by type. Phase 2 will add embeddings.
        const recentIncidents = await db.agentIncidents.findByUserId(dbUser.id, 10)
        if (recentIncidents.length > 0) {
          // Filter to incidents that match the current request keywords
          const userTextLower = actualContent.toLowerCase()
          const relevant = recentIncidents.filter((inc) => {
            const reqLower = inc.user_request.toLowerCase()
            // Simple keyword overlap check (Phase 2 will use embeddings)
            const keywords = userTextLower.split(/\s+/).filter((w) => w.length > 4)
            return keywords.some((kw) => reqLower.includes(kw))
          }).slice(0, 3)
          
          if (relevant.length > 0) {
            incidentSummary = buildIncidentSummary(relevant.map((inc) => ({
              ...inc,
              similarity: 0.85, // placeholder until we have embeddings
            })))
            console.log('[StreamV2] injecting incident summary:', { count: relevant.length })
          }
        }
      } catch (e) {
        console.warn('[StreamV2] incident search failed:', e?.message)
      }
      ...
      if (isSelfEdit) {
        ...
        // Build a sync callback that pushes file changes to the Fly runner immediately
        const syncToRunner = async () => {
          try {
            const syncUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.auroraly.co'}/api/previews/${chat.project_id}/sync`
            ...
            if (!res.ok) {
              ...
              console.warn(`[StreamV2] runner sync failed: HTTP ${res.status}: ${text.slice(0, 200)}`)
            } else {
              const body = await res.json().catch(() => ({}))
              console.log(`[StreamV2] runner synced: ${body.written || 0} files in ${body.elapsed || 0}ms`)
            }
          } catch (err) {
            console.warn(`[StreamV2] runner sync error: ${err.message}`)
          }
        }
        
        const projectFs = buildProjectFs({ 
          db, 
          projectId: chat.project_id, 
      ...
      const screenshotCtx = (chat.project_id && !isSelfEdit)
        ...
        : null
      const tools = buildDefaultToolset(scope, writer, reader, allAttachments.length > 0 ? allAttachments : null, guardCtx, isSelfEdit ? null : chat.project_id, screenshotCtx, dbUser?.id, chatId)
      ...
      let errored = false
      const toolEvents = [] // Track all tool calls for auto-save
      ...
      try {
        for await (const ev of runAgent({
          ...
          if (ev.type === 'text_delta') {
            ...
            toolEvents.push({ type: 'tool_use', name: ev.name, input: ev.args, id: ev.id })
            ...
            // Store result for auto-save
            const pendingForResult = pendingToolArgs.get(ev.id)
            if (pendingForResult) {
              const matchingEvent = toolEvents.find(t => t.id === ev.id)
              if (matchingEvent) {
                matchingEvent.result = { content: ev.content }
              }
            }
              ...
              if (looksSuccessful) {
                ...
                notifyPreviewOfFileChange(chat.project_id, {
                  ...
                  .then((r) => {
                    if (r.notified) {
                    ...
                    } else if (r.reason === 'no-machine') {
                      // ── Auto-start preview on first entry-point write ──
                      // When an agent builds a new project from scratch, it
                      // writes 20+ files but the preview never auto-starts.
                      // The user must manually click "Start Preview" before
                      // the agent can call screenshot_preview or diagnostics.
                      // Detect entry-point files (package.json, index.html,
                      // etc.) and auto-trigger preview allocation.
                      ...
                      const isEntryPoint = ENTRY_POINT_FILES.some((ep) => 
                        filePath === ep || filePath?.endsWith('/' + ep)
                      )
                      if (isEntryPoint && action === 'write') {
                        console.log(`[StreamV2] auto-starting preview for project ${chat.project_id} (entry point: ${filePath})`)
                        send('status', { stage: 'preview_auto_start', detail: `Starting preview (detected ${filePath})…` })
                        // Import and call the auto-start service function
                        import('@/lib/fly/auto-start-preview.js')
                          .then(({ autoStartPreview }) => autoStartPreview(chat.project_id))
                          .then((result) => {
                            if (result.ok) {
                              console.log(`[StreamV2] preview auto-started for project ${chat.project_id} (machine ${result.machineId})`)
                              send('status', { 
                                stage: 'preview_started', 
                                detail: `✅ Preview started (machine ${result.machineId?.slice(0, 8)}). Dev server is installing dependencies — preview will be ready in ~60-90 seconds.` 
                              })
                            } else {
                              console.warn(`[StreamV2] preview auto-start failed for project ${chat.project_id}: ${result.reason}`)
                              send('status', { 
                                stage: 'preview_start_failed', 
                                detail: `Preview auto-start failed (${result.reason}). Click "Start Preview" manually to see the live app.` 
                              })
                            }
                          })
                          .catch((err) => {
                            console.error(`[StreamV2] preview auto-start crashed for project ${chat.project_id}:`, err)
                            send('status', { 
                              stage: 'preview_start_error', 
                              detail: `Preview auto-start error: ${err.message}. Click "Start Preview" manually.` 
                            })
                          })
                      } else {
            ...
            // Emit task_complete so preview can auto-refresh AFTER agent finishes
            if (!isSelfEdit) {
              send('task_complete', { projectId: chat.project_id })
            }
      ...
      // ── Auto-save learned facts to persistent memory ────────────────
      // Extract facts from tool calls (framework detection, API endpoints,
      // deployment URLs, etc.) and save them so future chats know them.
      // Fire-and-forget: never block the response on memory writes.
      if (toolEvents.length > 0) {
        autoSaveFacts(chat.project_id, dbUser.id, toolEvents)
          .then(() => console.log('[StreamV2] auto-saved facts from', toolEvents.length, 'tool calls'))
          .catch((e) => console.warn('[StreamV2] auto-save facts failed:', e?.message))
      }
        ...
        if (!errored || fullContent) {
          const assistantMessage = await db.messages.create({
            ...
            metadata: {
              ...
              // Tag which agent sent this message (for escalation chats)
              agent_source: isEscalation ? 'core_system' : (isSelfEdit ? 'core_system' : 'project_agent'),
            },
          })
