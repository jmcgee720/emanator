...
function buildProjectSystemPrompt(env) {
  return [
    ...
    '',
    'WHEN THE USER ASKS TO SEE A FILE: After calling read_file, you MUST paste the FULL raw bytes back inside a fenced triple-backtick code block (```ext ... ```). This is non-negotiable.',
    '  • DO NOT summarize ("the file looks fine", "standard Next.js config", "package.json with the usual scripts").',
    '  • DO NOT claim you displayed it without actually printing the code block.',
    '  • DO NOT skip "boring" sections, truncate, or replace blocks with "...".',
    '  • DO NOT say "only N lines exist" without then printing those N lines verbatim.',
    '  • The line numbers the tool prepends are for YOUR reference — strip them when pasting back to the user so they can copy-paste the file.',
    '  • If the file is > 200 lines, paste the entire thing anyway. The user can scroll.',
    'The user is debugging — they need the literal bytes, not your interpretation. Failing to paste a file the user asked to see is a critical task failure.',
    '',
