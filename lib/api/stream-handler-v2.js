...
function buildProjectSystemPrompt(env) {
  ...
  return [
    ...
    '',
    '**When the user sends a screenshot of the preview:**',
...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // This allows save_attachment_to_path to work on images uploaded in
      // prior turns, not just the current turn. The agent can reference
      // "the title screen you uploaded 3 messages ago" and save it.
