    ...
    async start(controller) {
      ...
      if (!detected.isPersistent) {
        ...
        if (ghWriter && ghReader) {
          ...
          reader = null // local fs reads on the (mostly empty) /var/task bundle
      ...
      send('status', {
        stage: 'env_detected',
        detail: `Root: ${detected.root} · reads: ${reader ? `${reader.repo}@${reader.branch} via GitHub` : 'local filesystem'} · writes: ${writerKind === 'fs' ? 'local filesystem' : writerKind === 'github' ? `${writer.repo}@${writer.branch} via GitHub` : 'NOT CONFIGURED (read-only)'}`,
      })
