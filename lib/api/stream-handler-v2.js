...
function buildProjectSystemPrompt(env) {
  return [
    ...
    '  • save_attachment_to_path — present when attachments are available (current turn or recent history). Use this for any image/PDF/binary the user uploaded; write_file is text-only and will silently truncate binaries.',
    '',
    'ATTACHMENT PERSISTENCE: Images and files the user uploads persist across the conversation. You can see them in prior messages as inline images or text blocks, AND you can save them with save_attachment_to_path even if they were uploaded several turns ago. The tool has access to attachments from the last 5 user messages, so you can reference "the title screen you uploaded earlier" and save it without asking the user to re-upload.',
    '',
...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // If the user reports "agent says save_attachment_to_path is missing",
      // this log shows whether attachments reached the backend at all. If
      // metadata.attachments is null/undefined/empty here, the frontend is
      // not sending them (or the user didn't attach anything on this turn).
      console.log('[StreamV2] attachment metadata received:', {
        hasMetadata: !!metadata,
        hasAttachmentsKey: 'attachments' in (metadata || {}),
        attachmentsValue: metadata?.attachments,
        attachmentsIsArray: Array.isArray(metadata?.attachments),
        attachmentsLength: Array.isArray(metadata?.attachments) ? metadata.attachments.length : 'N/A',
        attachmentsFilenames: Array.isArray(metadata?.attachments)
          ? metadata.attachments.map((a) => a.filename || a.name || 'unnamed')
          : 'N/A',
      })
      const tools = buildDefaultToolset(scope, writer, reader, metadata.attachments, guardCtx)
      ...
      if (Array.isArray(metadata.attachments) && metadata.attachments.length > 0) {
        const hasImage = metadata.attachments.some((a) =>
          a?.file_category === 'image' ||
          a?.type?.startsWith('image/') ||
          a?.mime_type?.startsWith('image/'),
        )
        if (hasImage && effectiveTools.some((t) => t.name === 'submit_screenshot_inventory')) {
