...
function buildProjectSystemPrompt(env) {
  return [
    ...
    '',
    'ATTACHMENT PERSISTENCE: Images and files the user uploads are stored in the conversation history. You can see them in prior messages as inline images or text blocks. However, the save_attachment_to_path tool ONLY works for attachments uploaded on the CURRENT turn — you cannot retroactively save an image from 3 turns ago. If the user asks you to save an image they uploaded earlier, tell them: "Please re-attach [filename] so I can save it with save_attachment_to_path. Attachments from prior turns are visible to me but not saveable retroactively."',
    '',
