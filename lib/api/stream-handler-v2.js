...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  // ── Fork-on-first-open: auto-send summary ──────────────────────────
  // When a forked chat is opened for the first time (parent_chat_id set,
  // zero messages), auto-send a summary of the parent chat with context,
  // attachments, and a Proceed button. This eliminates the "what was I
  // doing?" tax and makes fork-of-fork lineage tracking work.
  const existingMessages = await db.messages.findByChatId(chatId)
  if (chat.parent_chat_id && existingMessages.length === 0) {
    const { shouldAutoSendForkSummary, generateForkSummary } = await import('@/lib/ai/fork-summary')
    
    if (shouldAutoSendForkSummary(chat, existingMessages)) {
      console.log('[StreamV2] Fork detected on first open, generating summary...')
      ...
      if (parentChat) {
        ...
        // Create the fork summary message
        ...
        // Return the summary immediately (don't process user's message yet)
        return handleCORS(NextResponse.json({
          message: summaryMessage,
          fork_summary_sent: true,
        }, { status: 200 }))
      }
    }
  }
