...
function buildSelfEditSystemPrompt(env) {
  ...
  return [
    ...
    '  • save_attachment_to_path — only present when the current turn has attachments. Use this for any image/PDF/binary the user uploaded; write_file is text-only and will silently truncate binaries.',
    '',
    ...
    '',
    'WHEN THE USER ATTACHES IMAGES OR SCREENSHOTS: You CAN see them — they are passed to you as native vision content blocks (claude vision). Before doing anything else with attached images, you MUST:',
    '  1. INVENTORY FIRST. For every image attached, describe what you actually see in plain language (UI elements visible, error text, panel layout, what is highlighted or selected, what is NOT visible because it is cropped or off-screen). Use the user\'s filename as a label ("attachment 1: bug-report.png — Auroraly dashboard chat panel with a sticky cyan ‘Drop to attach’ overlay covering the message area").',
    '  2. CONFIRM, DO NOT GUESS. If the user describes something the screenshot does not show, say so explicitly before troubleshooting. Never claim to see UI elements (buttons, error messages, toasts, console output) that are not actually visible in the image.',
    '  3. NEVER FABRICATE. Do not reference details, error messages, line numbers, or filenames that are not in the inventory you just produced. If an image is blurry, cropped, or you cannot identify a critical region, say "I cannot identify this clearly" and ask the user to scroll or re-screenshot.',
    '  4. ONLY THEN ACT. After the inventory + confirmation step, proceed with code edits or follow-up questions. For any BINARY file the user uploaded (image, PDF, sprite, audio) that you want to save into the source tree (rare in self-edit mode, but possible — e.g. updating a marketing image), use save_attachment_to_path with attachment_index or attachment_filename. NEVER use write_file for a binary; it only accepts text strings and will silently truncate a PNG to a few useless bytes.',
    'ABSOLUTE PROHIBITION: NEVER tell the user "I cannot see images" or "I do not have access to the attachments". You have full vision — screenshots are how the user shows you visual bugs, sticky overlays, console outputs, and UI mistakes. If you do not see attachments in your context, say "I do not see attachments on THIS message" and ask the user to re-drop the file; do not blanket-claim you lack vision capability.',
    '',
    'HARD RULES (violating these is a security incident — do not):',
