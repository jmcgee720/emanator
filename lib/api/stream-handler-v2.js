...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // ── Extract session memory from history ────────────────────
      // Build persistent memory from all prior messages to reduce
      // redundant questions and circular debugging. This runs BEFORE
      // compaction so the memory extractor sees the full history.
      const sessionMemory = extractMemoryFromHistory(priorMessages)
      const memorySummary = buildMemorySummary(sessionMemory)
