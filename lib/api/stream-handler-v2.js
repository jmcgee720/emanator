...
function buildProjectSystemPrompt(env) {
  ...
  return [
    ...
    '',
    '## AFTER EVERY BUILD OR FILE CHANGE — MANDATORY PREVIEW VISIBILITY PROTOCOL',
    '',
    'Every time you create or edit files, you MUST:',
    '',
    '**1. Call `preview_diagnostics` immediately after the last file write**',
    '  • This triggers machine allocation if needed (new projects)',
    '  • Returns the current state: `no-machine`, `still-installing`, `dev-server-not-running`, `healthy`, etc.',
    '  • NEVER skip this — the user is staring at a blank iframe and needs to know WHY',
    '',
    '**2. Surface the preview state to the user in plain language**',
    '  • If `verdict: "no-machine"` or `verdict: "still-installing"`:',
    '    ✅ Created N files. Preview is installing dependencies (npm install) — this takes 2-5 minutes for new projects. The preview will auto-refresh when ready.',
    '  • If `verdict: "dev-server-not-running"`:',
    '    ✅ Files saved. Dev server is starting — preview will load in ~30 seconds.',
    '  • If `verdict: "healthy"`:',
    '    ✅ Changes deployed. Preview should refresh within 10 seconds. You\'ll see: [describe the landing page or main UI element].',
    '',
    '**3. Describe what the user WILL see (even if not visible yet)**',
    '  • Always include a 1-2 sentence preview of the UI:',
    '    "When the preview loads, you\'ll see the landing page with \'Find jobs. Apply smarter.\' hero text, three feature cards, and Sign Up / Sign In buttons."',
    '  • This gives the user confidence that something real was built, even during the install wait.',
    '',
    '**4. Set expectations for first-run vs. incremental changes**',
    '  • New project (no machine allocated): "First build takes 2-5 minutes (installing dependencies). Subsequent changes refresh in 5-10 seconds."',
    '  • Existing project (machine running): "Preview will auto-refresh in 5-10 seconds."',
    '',
    '**5. NEVER leave the user guessing**',
    '  • Forbidden: ending a response with "I created the files" and no mention of preview state',
    '  • Required: always close with preview status + what they\'ll see',
    '',
    'Example closing (new project):',
    '"✅ Created 15 files. Preview is installing dependencies — first build takes 2-5 minutes. Once ready, you\'ll see the landing page with hero text, feature cards, and auth buttons. Subsequent changes will auto-refresh in seconds."',
    '',
    'Example closing (existing project):',
    '"✅ Updated app/dashboard/page.tsx. Preview will auto-refresh in ~10 seconds. You\'ll see the new job card layout with match scores and status badges."',
    '',
...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const tools = buildDefaultToolset(scope, writer, reader, allAttachments.length > 0 ? allAttachments : null, guardCtx, isSelfEdit ? null : chat.project_id)
