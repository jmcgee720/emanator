...
import { buildGithubWriter, buildMissingConfigWriter } from '@/lib/ai/github-writer'
...
function buildSelfEditSystemPrompt(env) {
  const writeMode = env.writerKind === 'github'
    ? `WRITES: write_file and edit_file commit directly to ${env.repo}@${env.branch} via GitHub. Each edit triggers a Vercel redeploy (~2 minutes). Use commit messages that explain what changed and why.`
  ...
  return [
    'You are Auroraly\'s self-edit agent ("Core System mode"). You can read, search, edit, and run commands on the Auroraly source tree at ' + env.root + '.',
    '',
    writeMode,
    '',
    'Tools available:',
...
export async function handleStreamMessageV2(request, { chatId, authUser: _authUser, dbUser, db }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Detect codebase root and choose a writer based on filesystem persistence
      const detected = detectCodebaseRoot()
      const scope = buildSelfEditScope(detected.root)
      let writer = null
      let writerKind = 'fs'
      ...
      console.log('[StreamV2] env detected:', { root: detected.root, source: detected.source, isPersistent: detected.isPersistent, writerKind })
      send('status', {
        stage: 'env_detected',
        detail: `Root: ${detected.root} · writes: ${writerKind === 'fs' ? 'local filesystem' : writerKind === 'github' ? `${writer.repo}@${writer.branch} via GitHub` : 'NOT CONFIGURED (read-only)'}`,
      })
