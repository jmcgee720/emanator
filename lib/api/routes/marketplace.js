...
export async function handle(route, method, path, request) {
  ...
  // ── Add a review to a marketplace template ──
  if (route.match(/^\/marketplace\/[^/]+\/reviews$/) && method === 'POST') {
    const authUser = await getAuthUser(request)
    if (!authUser) return handleCORS(NextResponse.json({ error: 'Unauthorized' }, { status: 401 }))
    const dbUser = await checkAllowlist(authUser.email)
    if (!dbUser) return handleCORS(NextResponse.json({ error: 'Access denied' }, { status: 403 }))
    ...
    try {
      ...
      if (!rating || rating < 1 || rating > 5) {
        return handleCORS(NextResponse.json({ error: 'Rating must be between 1 and 5' }, { status: 400 }))
      }

      const review = await db.marketplaceTemplates.addReview(templateId, {
        user_id: dbUser.id,
        author_email: authUser.email,
        rating: Math.round(rating),
        comment: (comment || '').trim().slice(0, 500),
      })
      return handleCORS(NextResponse.json({ review }, { status: 201 }))
    } catch (err) {
      console.error('[Marketplace] Review error:', err)
      return handleCORS(NextResponse.json({ error: 'Failed to add review' }, { status: 500 }))
    }
  }

  // ── Get reviews for a marketplace template ──
  if (route.match(/^\/marketplace\/[^/]+\/reviews$/) && method === 'GET') {
    const templateId = route.split('/')[2]
    try {
      const reviews = await db.marketplaceTemplates.getReviews(templateId)
      return handleCORS(NextResponse.json({ reviews }))
    } catch (err) {
      console.error('[Marketplace] Get reviews error:', err)
      return handleCORS(NextResponse.json({ reviews: [] }))
    }
  }
