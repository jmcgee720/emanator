...
export async function handle(route, method, path, request) {
  ...
  if (route.match(/^\/chats\/[^/]+\/fork$/) && method === 'POST') {
    ...
    try {
      ...
      // Create a single assistant message with the fork summary
      // (Previously created both system + assistant messages with identical
      // content, causing duplicate "Continued from" cards in the UI)
