...
import { compressContext } from '@/lib/ai/stream-helpers'
...
export async function handle(route, method, path, request) {
  ...
  if (route.match(/^\/chats\/[^/]+\/fork$/) && method === 'POST') {
    ...
    try {
      ...
      // Generate a short 1-3 word title via a lightweight AI call
      let forkTitle = body.title
      if (!forkTitle) {
        const userMessages = messages.filter(m => m.role === 'user').slice(-5)
        const snippets = userMessages.map(m => (m.content || '').slice(0, 80)).join('\n')
        try {
          ...
          const result = await provider.chat([
            { role: 'system', content: 'Reply with ONLY a 1-3 word title summarizing this conversation. No quotes, no punctuation, no explanation.' },
            { role: 'user', content: `Original chat: "${sourceChat.title}"\nRecent messages:\n${snippets}` }
          ], { temperature: 0.3, max_tokens: 10 })
          const generated = (result.content || '').trim().replace(/['"]/g, '')
          if (generated.length >= 2 && generated.length <= 40) {
            forkTitle = generated
          }
        } catch (e) {
          console.warn('[Fork] Title generation failed, using fallback:', e.message?.slice(0, 80))
        }
        // Fallback: use source title shortened
        if (!forkTitle) {
          const base = sourceChat.title || 'Chat'
          forkTitle = base.length > 25 ? base.slice(0, 22) + '...' : base + ' (cont.)'
        }
      }
