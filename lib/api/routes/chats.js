...
export async function handle(route, method, path, request) {
  ...
  if (route.match(/^\/chats\/[^/]+\/fork$/) && method === 'POST') {
    ...
    try {
      ...
      // Generate a smart title from conversation context
      let forkTitle = body.title
      if (!forkTitle) {
        // Extract key topics from recent user messages for a meaningful name
        const userMessages = messages.filter(m => m.role === 'user').slice(-5)
        if (userMessages.length > 0) {
          const lastMsg = userMessages[userMessages.length - 1].content || ''
          // Use last user message as title base, truncated to something readable
          const cleaned = lastMsg.replace(/[\n\r]+/g, ' ').trim()
          if (cleaned.length > 5 && cleaned.length <= 60) {
            forkTitle = cleaned
          } else if (cleaned.length > 60) {
            forkTitle = cleaned.slice(0, 57) + '...'
          } else {
            forkTitle = `${sourceChat.title} (continued)`
          }
        } else {
          forkTitle = `${sourceChat.title} (continued)`
        }
