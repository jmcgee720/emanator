...
export async function handle(route, method, path, request) {
  ...
  if (route.match(/^\/projects\/[^/]+\/generate-image$/) && method === 'POST') {
    ...
    try {
      ...
      const { prompt, mode, spriteOpts, size, chatId, variation, imageProvider, referenceImages } = body
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          try {
            ...
            const proxyBudgetMatch = (err.message || '').match(/Budget has been exceeded.*?Max budget:\s*([\d.]+)/i)
            if (proxyBudgetMatch) {
              safeError = `Your Universal Key spending limit ($${parseFloat(proxyBudgetMatch[1]).toFixed(2)}) has been reached. Go to Profile → Universal Key → Add Balance.`
            } else if (rawMsg.includes('billing') || rawMsg.includes('budget') || rawMsg.includes('credit') || rawMsg.includes('quota')) {
      ...
      const rawMsg = (err.message || '').toLowerCase()
      let safeError = 'Image generation failed. Please try again.'
      const proxyBudgetMatch2 = (err.message || '').match(/Budget has been exceeded.*?Max budget:\s*([\d.]+)/i)
      if (proxyBudgetMatch2) {
        safeError = `Your Universal Key spending limit ($${parseFloat(proxyBudgetMatch2[1]).toFixed(2)}) has been reached. Go to Profile → Universal Key → Add Balance.`
      } else if (rawMsg.includes('billing') || rawMsg.includes('budget') || rawMsg.includes('credit') || rawMsg.includes('quota')) {
        safeError = "You're out of credits. Tap Buy Credits to top up and keep building."
      }
      return handleCORS(NextResponse.json({
