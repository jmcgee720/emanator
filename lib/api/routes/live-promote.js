...
export async function handle(route, method, pathSegments, request) {
  ...
  if (route.match(/^\/projects\/[^/]+\/promote-to-live$/) && method === 'POST') {
      ...
      try {
        ...
        if (filePath.match(/\.(js|jsx|ts|tsx|mjs)$/)) {
          ...
          try {
            ...
            // Bracket balance check: count (), {}, [] — if unbalanced, the AI broke the file
            const opens = (content.match(/[({[]/g) || []).length
            const closes = (content.match(/[)}\]]/g) || []).length
            const hasBracketMismatch = Math.abs(opens - closes) > 2  // Allow small tolerance for template literals

            if (hasBrokenObject || hasOrphanedCode || hasBracketMismatch) {
              errors.push({ path: filePath, error: `Syntax error detected — file not written to prevent corruption. Error: ${syntaxErr.message}${hasBracketMismatch ? ` (bracket mismatch: ${opens} opens vs ${closes} closes)` : ''}` })
              console.warn('[promote-to-live] BLOCKED:', filePath, '— syntax error:', syntaxErr.message, hasBracketMismatch ? `(brackets: ${opens}/${closes})` : '')
              continue
            }
        ...
        // Regression guard: don't overwrite disk file with a significantly smaller version
        if (fs.existsSync(fullPath)) {
          const diskContent = fs.readFileSync(fullPath, 'utf-8')
          const diskLines = diskContent.split('\n').length
          const newLines = content.split('\n').length
          if (diskLines > 100 && newLines < diskLines * 0.5) {
            console.warn(`[promote-to-live] REGRESSION BLOCKED: ${filePath} — disk has ${diskLines} lines, new has ${newLines} lines`)
            errors.push({ path: filePath, error: `Regression blocked: new version (${newLines} lines) is much smaller than current (${diskLines} lines). The AI may have lost existing code.` })
            continue
          }
        }
    ...
    if (written.length > 0) {
      try {
        const { invalidateCache } = await import('../../ai/filesystem.js')
        invalidateCache?.()
      } catch { /* non-critical */ }
        ...
        if (!isHealthy) {
          ...
          if (snapshot?.files_snapshot) {
            ...
            return handleCORS(NextResponse.json({
              ...
              files_attempted: written,
              error: 'Changes caused a compilation error and were automatically reverted.',
              detail: `Health check returned 500 after writing: ${written.join(', ')}. The patch likely introduced a syntax error (mismatched brackets, unclosed JSX, etc). The original files have been restored from snapshot.`,
