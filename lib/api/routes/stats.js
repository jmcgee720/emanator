...
export async function handle(route, method, path, request) {
  ...
  if (route === '/stats/my-builds' && method === 'GET') {
    try {
      // Require auth for user-specific stats
      const authUser = await getAuthUser(request)
      if (!authUser?.email) {
        return handleCORS(NextResponse.json({ error: 'Unauthorized' }, { status: 401 }))
      }
      // Resolve dbUser.id — generation_runs.user_id references the app users table
      const dbUser = await db.users.findByEmail(authUser.email)
      if (!dbUser?.id) {
        return handleCORS(NextResponse.json({
          total_this_week: 0,
          fastest_seconds: null,
          favorite_archetype: null,
      ...
      const { data, error } = await supabaseAdmin
        .from('generation_runs')
        .select('tool_mode, duration, success, created_at')
      ...
      const topEntry = Object.entries(archetypeCounts).sort((a, b) => b[1] - a[1])[0]
      const favoriteArchetype = topEntry
        ? { id: topEntry[0], count: topEntry[1], label: ARCHETYPES[topEntry[0]]?.label || topEntry[0] }
        : null

      return handleCORS(NextResponse.json({
        total_this_week: successful.length,
        fastest_seconds: durations.length ? Math.min(...durations) : null,
        favorite_archetype: favoriteArchetype,
      }, { headers: { 'Cache-Control': 'private, max-age=30' } }))
    } catch (err) {
      return handleCORS(NextResponse.json({ error: err.message }, { status: 500 }))
    }
