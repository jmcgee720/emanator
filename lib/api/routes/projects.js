...
import { destroyPreviewApp } from '@/lib/fly/apps'
...
export async function handle(route, method, path, request) {
  ...
  if (route === '/projects' && method === 'POST') {
    ...
    try {
      ...
      // Create initial chat thread using the brief summary (or project name as fallback)
      const initialChat = await db.chats.create({
        project_id: project.id,
        title: body.chat_title || name
      })
  ...
  if (route.match(/^\/projects\/[^/]+$/) && (method === 'PUT' || method === 'PATCH')) {
    const projectId = path[1]
    const authUser = await getAuthUser(request)
    if (!authUser) {
      return handleCORS(NextResponse.json({ error: 'Unauthorized' }, { status: 401 }))
  ...
  if (route.match(/^\/projects\/[^/]+$/) && method === 'DELETE') {
    ...
    // Destroy the Fly preview app (idempotent — safe if app doesn't exist)
    try {
      await destroyPreviewApp(projectId)
    } catch (flyErr) {
      console.warn(`[DeleteProject] Fly app cleanup failed for ${projectId}:`, flyErr.message)
      // Non-blocking — project is already deleted from DB
    }
  ...
  if (route === '/account/cleanup' && method === 'POST') {
    ...
    for (const project of userProjects) {
      try {
        ...
        // Destroy Fly app (best-effort, non-blocking)
        try {
          await destroyPreviewApp(project.id)
        } catch (flyErr) {
          console.warn(`[BulkDelete] Fly cleanup failed for ${project.id}:`, flyErr.message)
        }
        deletedCount++
      } catch (err) {
