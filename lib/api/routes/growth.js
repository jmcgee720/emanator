...
import { fetchTrends, listTrends } from '@/lib/growth/trends-native'
import { analyzeNative, generateDraftsNative } from '@/lib/growth/analyze-native'

// Configurable backend URL — points to the FastAPI growth service.
// Defaults to local supervisor-managed backend for dev; set BACKEND_URL in
// Vercel to point at your Railway FastAPI deployment if you want to keep
// Playwright-based crawling in production.
const BACKEND_URL = process.env.BACKEND_URL || 'http://localhost:8001'
// Graceful fallback error when backend isn't reachable (pure-Vercel deploys).
const BACKEND_UNAVAILABLE = {
  error: 'Crawler backend unavailable',
  hint: 'Set BACKEND_URL env var to a deployed FastAPI service (e.g., Railway) or use the simpler /trends endpoints which run natively on Vercel.',
...
export async function handle(route, method, path, request) {
  if (route === '/growth/crawl' && method === 'POST') {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/internal/growth/crawl`, {
        method: 'POST',
      ...
      // ECONNREFUSED / fetch failed → no backend reachable.
      if (err.cause?.code === 'ECONNREFUSED' || err.message?.includes('fetch failed')) {
        return handleCORS(NextResponse.json(BACKEND_UNAVAILABLE, { status: 503 }))
      }
  ...
  if (route === '/growth/crawl/progress' && method === 'GET') {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/internal/growth/crawl/progress?user_id=${dbUser.id}`)
      const data = await res.json()
  ...
  if (route === '/growth/analyze' && method === 'POST') {
    ...
    try {
      ...
      const result = await analyzeNative({
        userId: dbUser.id,
        pageId: body.page_id,
        personaId: body.persona_id,
      })
      return handleCORS(NextResponse.json(result.body, { status: result.status }))
    } catch (err) {
  ...
  if (route === '/growth/generate-drafts' && method === 'POST') {
    ...
    try {
      ...
      const result = await generateDraftsNative({
        userId: dbUser.id,
        pageId: body.page_id,
        personaId: body.persona_id,
      })
      ...
      console.error('[Growth] Generate drafts error:', err)
  ...
  if (route === '/growth/monitors/check-all' && method === 'POST') {
    ...
    try {
      ...
      for (const monitor of enabledMonitors) {
        try {
          const crawlRes = await fetch(`${BACKEND_URL}/api/internal/growth/crawl`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  // ── Get auto-crawl schedule config ──
  if (route === '/growth/monitors/schedule' && method === 'GET') {
    const authUser = await getAuthUser(request)
    if (!authUser) return handleCORS(NextResponse.json({ error: 'Unauthorized' }, { status: 401 }))
    const dbUser = await checkAllowlist(authUser.email)
    if (!dbUser) return handleCORS(NextResponse.json({ error: 'Access denied' }, { status: 403 }))
    try {
      const { monitorDb } = await import('@/lib/growth/service')
      const schedule = await monitorDb.getSchedule(dbUser.id)
      return handleCORS(NextResponse.json({ schedule: schedule || { enabled: false, frequency: '24h', last_run: null, next_run: null } }))
    } catch (err) {
      console.error('[Growth] Get schedule error:', err)
      return handleCORS(NextResponse.json({ schedule: { enabled: false, frequency: '24h', last_run: null, next_run: null } }))
    }
  }

  // ── Update auto-crawl schedule config ──
  if (route === '/growth/monitors/schedule' && method === 'POST') {
    const authUser = await getAuthUser(request)
    if (!authUser) return handleCORS(NextResponse.json({ error: 'Unauthorized' }, { status: 401 }))
    const dbUser = await checkAllowlist(authUser.email)
    if (!dbUser) return handleCORS(NextResponse.json({ error: 'Access denied' }, { status: 403 }))
    try {
      ...
      const validFreqs = ['6h', '12h', '24h', '48h', '7d']
      if (frequency && !validFreqs.includes(frequency)) {
        return handleCORS(NextResponse.json({ error: `Invalid frequency. Use one of: ${validFreqs.join(', ')}` }, { status: 400 }))
      }

      const { monitorDb } = await import('@/lib/growth/service')
      const schedule = await monitorDb.setSchedule(dbUser.id, {
        enabled: enabled !== undefined ? !!enabled : true,
        frequency: frequency || '24h',
      })
      return handleCORS(NextResponse.json({ schedule }))
    } catch (err) {
      console.error('[Growth] Set schedule error:', err)
      return handleCORS(NextResponse.json({ error: 'Failed to update schedule' }, { status: 500 }))
    }
  }

  // ============ TRENDS (native — runs on Vercel) ============
  ...
  if (route === '/trends/fetch' && method === 'POST') {
    ...
    try {
      ...
      return handleCORS(NextResponse.json({ success: true, ...result }, { status: 201 }))
    } catch (err) {
