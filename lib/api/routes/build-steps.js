...
function buildProvider(authUser, provider, model) {
  // Derive AIService from user's selected provider/model. Falls back to defaults
  // if not provided (Claude Sonnet 4.5).
...
export async function handle(route, method, path, request) {
  ...
  if (route === '/build/copy' && method === 'POST') {
    ...
    try {
      ...
      if (state.userId !== auth.authUser.sub) return handleCORS(NextResponse.json({ error: 'forbidden' }, { status: 403 }))
