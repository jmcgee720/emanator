...
function buildGeminiProvider() {
  ...
  return new GeminiProvider(key, 'gemini-2.5-flash-image-preview', {})
}
...
export async function handle(route, method, path, request) {
  ...
  // a trivial prompt so we can tell WHY images fall back to stock.
  if (route === '/build/ping-nano-banana' && method === 'GET') {
    ...
    if (!keyConfigured) {
      return handleCORS(NextResponse.json({
        ok: false,
        step: 'env',
        message: 'Neither GEMINI_API_KEY nor GOOGLE_API_KEY is set in Vercel env vars. Set one in Project Settings → Environment Variables → Production.',
      }))
    }
    const geminiProvider = buildGeminiProvider()
    if (!geminiProvider) {
      return handleCORS(NextResponse.json({ ok: false, step: 'provider', message: 'Provider constructor returned null unexpectedly' }))
    }
    try {
      const t0 = Date.now()
      ...
      if (res?.b64_json) {
        return handleCORS(NextResponse.json({
          ...
          bytes: res.b64_json.length,
          message: `Nano Banana returned ${res.b64_json.length} bytes in ${elapsed}ms — image generation is working.`,
        }))
      }
      return handleCORS(NextResponse.json({
        ok: false,
        step: 'no_image_in_response',
        elapsedMs: elapsed,
        message: 'Gemini responded but did not include inline image data. Your account may not have Nano Banana access (requires paid tier on Google AI Studio).',
      }))
    } catch (err) {
      ...
      return handleCORS(NextResponse.json({
        ...
        hint: status === 403 || /permission|access/i.test(msg)
          ? 'Your Gemini API key is valid but your Google AI Studio account does not have Nano Banana access. Image generation requires a paid tier. Go to https://aistudio.google.com/app/plan to enable billing.'
          : status === 400 || /invalid/i.test(msg)
          ? 'The request was rejected. Most likely cause: the Gemini API key is valid but cannot call this model from this region/project. Try regenerating the key at https://aistudio.google.com/apikey'
          : status === 429
          ? 'Rate limit hit — free tier allows ~15 requests per minute. Upgrade to paid tier or retry in a minute.'
          : 'Unknown error — paste this response back to support.',
      }))
    }
  }
