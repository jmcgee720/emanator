...
export async function handle(route, method, path, request) {
  ...
  if (route === '/build/images' && method === 'POST') {
    ...
    if (auth.err) return handleCORS(auth.err)
    try {
      const { runId, skipImagery } = await request.json()
      const state = await phaseStates.findByRunId(runId)
      if (!state) return handleCORS(NextResponse.json({ error: 'run not found' }, { status: 404 }))
      if (state.userId !== (auth.authUser.id || auth.authUser.sub)) return handleCORS(NextResponse.json({ error: 'forbidden' }, { status: 403 }))
      if (!state.results?.design_tokens) return handleCORS(NextResponse.json({ error: 'design_tokens phase not complete' }, { status: 400 }))

      // Lever 2 (wizard): when the user clicks "Skip imagery for now",
      // short-circuit Phase 4 with the same `deferred: true` sentinel
      // the chat-stream pipeline already uses. Phase 5 falls back to
      // SVG/gradient placeholders, the project is stamped imagery_status
      // = 'deferred', and the ImageryDeferredBanner shows up in PreviewTab.
      if (skipImagery) {
        const result = { images: [], deferred: true, _ms: 0 }
        await phaseStates.saveImagesForRun(runId, []) // wipe any prior batch
        state.results.images = result
        state.phase = 'images'
        state.updatedAt = new Date()
        await phaseStates.upsertByRunId(runId, state)

        // Stamp project so the post-build banner shows up. Best-effort —
        // we don't fail the wizard on a settings update glitch.
        try {
          const project = await supaDb.projects.findById(state.projectId)
          if (project) {
            const settings = { ...(project.settings || {}) }
            settings.imagery_status = 'deferred'
            settings.imagery_status_at = new Date().toISOString()
            await supaDb.projects.update(state.projectId, { settings })
          }
        } catch (err) {
          console.warn('[build/images] settings stamp failed:', err.message)
        }
