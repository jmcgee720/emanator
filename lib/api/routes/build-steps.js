...
function buildOpenAIImageProvider() {
  ...
  // (e.g. user's Google AI Studio account is on Free tier and refuses
  // image generation). gpt-image-1 produces high-quality on-brand
  // images and our existing OpenAI key already covers this.
    ...
    if (!openaiKey) {
      ...
      try {
        ...
        if (res?.b64_json || res?.url) {
          result.openai = { ok: true, step: 'gen_ok', elapsedMs: elapsed, message: `gpt-image-1 returned ${res.b64_json ? res.b64_json.length + ' bytes' : 'a URL'} in ${elapsed}ms.` }
        } else {
          result.openai = { ok: false, step: 'no_image_in_response', elapsedMs: elapsed, message: 'OpenAI responded with no image data.' }
        }
      } catch (err) {
        const msg = err?.user_message || err?.raw_error || err?.message || String(err)
        result.openai = { ok: false, step: 'api_error', message: msg }
      }
    }
