...
export async function handle(route, method, path, request) {
  if (route === '/credits' && method === 'GET') {
    ...
    try {
      ...
      // Show preview of loyalty bonus + first-purchase bonus for each pack.
      const packagesWithBonus = CREDIT_PACKAGES.map((p) => {
        const res = applyLoyaltyBonus(p.amount, balance.lifetime_purchased_usd, {
          isFirstPurchase: !balance.first_purchase_completed,
        })
        return {
          ...p,
          baseCredits: p.amount,
          bonusCredits: res.bonus,
          loyaltyBonus: res.loyaltyBonus,
          firstPurchaseBonus: res.firstPurchaseBonus,
          totalCredits: res.total,
          tier: res.tier.label,
        }
      })
  ...
  // POST /credits/apply-referral — record that the authed user was referred by `referral_code`.
  if (route === '/credits/apply-referral' && method === 'POST') {
    const authUser = await getAuthUser(request)
    if (!authUser) {
      return handleCORS(NextResponse.json({ error: 'Unauthorized' }, { status: 401 }))
    }
    ...
    try {
      ...
      if (!referralCode) {
        return handleCORS(NextResponse.json({ error: 'referral_code is required' }, { status: 400 }))
      }
      // Referral code = referrer's user_id (stable, unique, no new table needed).
      const result = await creditsDb.recordReferral(dbUser.id, referralCode)
      if (!result.recorded) {
        const status = result.reason === 'self_referral' || result.reason === 'invalid_referrer' ? 400 : 200
        return handleCORS(NextResponse.json({ recorded: false, reason: result.reason }, { status }))
      }
      return handleCORS(NextResponse.json({ recorded: true }))
    } catch (err) {
      console.error('[Credits] Apply referral error:', err)
      return handleCORS(NextResponse.json({ error: 'Failed to apply referral' }, { status: 500 }))
    }
  }
