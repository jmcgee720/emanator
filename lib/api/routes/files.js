...
export async function handle(route, method, path, request) {
  ...
  if (route.match(/^\/projects\/[^/]+\/preview-snapshot$/) && method === 'GET') {
    ...
    if (snapshot && snapshot.version !== SNAPSHOT_VERSION) {
      const res = NextResponse.json({ snapshot: null, invalidated: true })
      // Never let Vercel/CDN cache an invalidation response — otherwise the
      // user keeps seeing a stale broken thumbnail (e.g. InstaSort showing
      // the old Babel v8 red wall) until the CDN entry expires on its own.
