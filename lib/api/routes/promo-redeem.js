...
export async function handle(route, method, path, request) {
  ...
  if (route === '/promo/redeem' && method === 'POST') {
    ...
    try {
      ...
      // Update user plan in Supabase (legacy, kept for compatibility)
      await db.users.update(currentUser.id, { plan: promoCode.plan })
      
      // Grant unlimited credits in MongoDB (where credits are actually tracked)
      const { creditsDb } = await import('@/lib/credits/service')
      await creditsDb.grantUnlimited(currentUser.id)
      
      return handleCORS(NextResponse.json({
        success: true,
        plan: promoCode.plan,
        message: `Successfully redeemed! You now have UNLIMITED credits.`
      }))
    } catch (error) {
