...
export async function handle(route, method, path, request) {
  ...
  if (route.match(/^\/projects\/[^/]+\/deploy\/vercel$/) && method === 'POST') {
    ...
    try {
      ...
      // Vercel v13 /deployments accepts { file, data } objects. For text files
      // we send UTF-8 strings directly (supported); for safety we base64-encode
      // to guarantee no transport corruption for special chars.
      const vercelFiles = Object.entries(fileMap).map(([filePath, content]) => ({
        file: filePath,
