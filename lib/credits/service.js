...
export const MODEL_COSTS = {
  ...
  // Anthropic
  'claude-fable-5':                   { credits: 5.0,  tier: 'ultra',    label: 'Ultra' },
...
export const creditsDb = {
  ...
  /**
   * Grant unlimited credits to a user (promo code redemption).
   * Sets a permanent flag that bypasses all credit checks.
   */
  async grantUnlimited(userId) {
    ...
    await db.collection('credits_balance').updateOne(
      { user_id: userId },
      {
        $set: { unlimited: true, unlimited_granted_at: now, updated_at: now },
        $setOnInsert: {
          user_id: userId,
          balance: DEFAULT_BALANCE,
          lifetime_purchased_usd: 0,
  ...
  async deductCredits(userId, actionType, opts = {}) {
    ...
    // Unlimited users bypass all credit checks
    if (current.unlimited) {
      return { balance: 'unlimited', cost: 0, action_type: actionType, model: opts.model || null, unlimited: true }
    }
