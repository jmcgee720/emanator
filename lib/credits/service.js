...
export function applyLoyaltyBonus(baseCredits, lifetimeUsd = 0) {
  const tier = resolveLoyaltyTier(lifetimeUsd)
  const bonus = Math.floor((baseCredits * tier.bonusPercent) / 100)
  return { baseCredits, bonus, total: baseCredits + bonus, tier }
}
...
export const creditsDb = {
  ...
  async addCredits(userId, amount, opts = {}) {
    ...
    // Snapshot current lifetime BEFORE incrementing so the bonus reflects the
    // tier the user was in at purchase time (not the post-purchase tier).
    const current = await this.getBalance(userId)
    const { bonus, total, tier } = applyLoyaltyBonus(base, current.lifetime_purchased_usd)
