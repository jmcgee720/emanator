...
// Path prefixes whose content IS a single data URI by design — extracting
// from them would destroy the file (replacing 2MB of image bytes with a
// 60-char placeholder URL). Always skipped.
const SKIP_PREFIXES = ['_assets/', '_generated/', '_uploads/']

// Imported projects (CRA, Next.js games, etc.) carry binary assets as
// project_files rows whose content is the data URI of the actual image.
// Their path looks like `assets/logo.png`, `public/icons/foo.svg`, etc.
// Skip any source file with a binary asset extension to keep those bytes
// intact.
const BINARY_ASSET_EXT = /\.(png|jpe?g|gif|webp|svg|ico|bmp|avif|tiff?|heic|heif)$/i

// Specific brand-VFS modules where the AI deliberately inlines data URIs
// as exported constants (PreviewTab parses them back at render time —
// see parseBrandVfsFromAssetsModule). Rewriting these breaks the brand.
const SKIP_FILE_PATHS = new Set(['components/assets.js'])
 ...
 *
 * Re-entry safety: bails out for `_assets/`, `_generated/`, and
 * `_uploads/` paths — those rows ARE the binary stores by design and
 * extracting from them would destroy the file.
 */
export async function extractInlineImages(projectId, filePath, content) {
  ...
  if (typeof filePath === 'string') {
    ...
    if (BINARY_ASSET_EXT.test(filePath)) return { content, extractedCount: 0 }
    if (SKIP_FILE_PATHS.has(filePath)) return { content, extractedCount: 0 }
  }
