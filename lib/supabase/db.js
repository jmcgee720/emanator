...
export const db = {
  users: {
    ...
    // ── User preferences (stored in metadata JSONB column) ──
    async getPreferences(userId) {
      const user = await this.findById(userId)
      return user?.metadata?.preferences || {}
    },

    async updatePreferences(userId, prefsUpdate) {
      const user = await this.findById(userId)
      if (!user) throw new Error('User not found')
      
      const currentMeta = user.metadata || {}
      const currentPrefs = currentMeta.preferences || {}
      const newPrefs = { ...currentPrefs, ...prefsUpdate }
      const newMeta = { ...currentMeta, preferences: newPrefs }
      
      return this.update(userId, { metadata: newMeta })
    },

    async setLastCoreChatId(userId, chatId) {
      return this.updatePreferences(userId, { last_core_chat_id: chatId })
    },

    async getLastCoreChatId(userId) {
      const prefs = await this.getPreferences(userId)
      return prefs.last_core_chat_id || null
    },
  },

  projects: {
    // ── Read-path scrub ───────────────────────────────────────────
    // Strip base64 thumbnail data URLs from any project row we return.
    // Legacy rows (pre-2026-07-08) stored 60-150 KB PNG data URLs inside
    // settings.thumbnail_screenshot.data_url. Any endpoint that returned
    // multiple such projects (e.g. the dashboard list) would exceed
    // Vercel's 4.5 MB serverless response cap → HTTP 413 → dashboard
    // loads fail → previews look broken from the outside. The scrub is
    // additive-safe: it doesn't touch new URL-based screenshots.
    _scrubBloat(row) {
      if (!row || !row.settings || typeof row.settings !== 'object') return row
      const ts = row.settings.thumbnail_screenshot
      if (ts && typeof ts === 'object' && typeof ts.data_url === 'string') {
        return {
          ...row,
          settings: {
            ...row.settings,
            thumbnail_screenshot: {
              url: ts.url || null,
              captured_at: ts.captured_at || null,
              bytes: ts.bytes || null,
    ...
    async findByUserId(userId) {
      ...
      if (error) throw error
      return (data || []).map(row => this._scrubBloat(row))
    },
    ...
    async update(id, updates) {
      // Defensive cleanup: strip legacy base64 thumbnail data URLs from
      // any settings blob we're about to write. Pre-2026-07-08 code stored
      // 60-150 KB PNG data URLs inside settings.thumbnail_screenshot.data_url;
      // subsequent settings writes then blew past Vercel's 4.5 MB serverless
      // body cap → HTTP 413 → every subsequent preview action failed for
      // that project. This one-line self-heal migrates the row on next write.
      if (updates?.settings && typeof updates.settings === 'object') {
        const ts = updates.settings.thumbnail_screenshot
        if (ts && typeof ts === 'object' && typeof ts.data_url === 'string') {
          updates = {
            ...updates,
            settings: {
  ...
  chats: {
    ...
    // ── Core System organization helpers ──
    async updateMetadata(id, metadataUpdates) {
      // Merge new metadata with existing (preserves other fields)
      const existing = await this.findById(id)
      if (!existing) throw new Error('Chat not found')
      
      const newMetadata = { ...(existing.metadata || {}), ...metadataUpdates }
      return this.update(id, { metadata: newMetadata })
    },

    async setTags(id, tags) {
      return this.updateMetadata(id, { tags: Array.isArray(tags) ? tags : [] })
    },

    async setPinned(id, pinned) {
      return this.updateMetadata(id, { pinned: !!pinned })
    },

    async setArchived(id, archived) {
      return this.updateMetadata(id, { archived: !!archived })
    },
  },
  ...
  // ============ AGENT LEARNING ============
  // Incident memory and feedback capture for cross-session learning
  agentIncidents: {
    async create(payload) {
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          agent_response: payload.agent_response || null,
    ...
    async findByType(userId, incidentType, limit = 20) {
      if (!userId || !incidentType) return []
      const { data, error } = await supabaseAdmin
        .from('agent_incidents')
        .select('*')
        .eq('user_id', userId)
    ...
    async searchSimilar(embedding, userId, threshold = 0.8, limit = 5) {
      if (!embedding) return []
      const { data, error } = await supabaseAdmin
        .rpc('search_similar_incidents', {
      ...
      if (error) {
        console.error('[agentIncidents.searchSimilar] Error:', error)
        return []
      }
      return data || []
    },
  ...
  agentFeedback: {
    ...
    async findByActionType(userId, actionType, limit = 50) {
    ...
    async getStats(actionType, userId = null) {
      const { data, error } = await supabaseAdmin
        .rpc('get_action_feedback_stats', {
          p_action_type: actionType,
          p_user_id: userId,
        })
      if (error) {
        console.error('[agentFeedback.getStats] Error:', error)
        return null
      }
      return data?.[0] || null
    },
