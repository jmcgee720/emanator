...
// Hard ceiling on a SINGLE source file (post-image-extraction). Anything
// above this is almost certainly malformed — a runaway generation, a
// misplaced binary, a pasted CSV. We log loudly and reject so the AI
// pipeline can't silently destroy a project's previewability.
//
// Real source files (page.jsx, component, css) max out around 100KB.
// A 500KB ceiling gives 5× headroom and still keeps `findByProjectId`
// fast (50 files × 500KB = 25MB worst-case response, tolerable).
export const MAX_FILE_BYTES = 500 * 1024

// Image asset rows store data URIs as text in Storage and CAN legitimately
// exceed 500KB (a high-quality hero image is 1-3MB as base64). Use a
// looser cap for `_assets/` paths so the extractor's own writes succeed.
