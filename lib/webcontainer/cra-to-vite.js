...
/**
 * Make sure the project has a Vite-compatible root `index.html` with a
 * `<script type="module" src="/src/...">` entry tag injected.
 *
 * Behavior:
 *   - If root `index.html` already has the entry script, leave it alone.
 *   - Else if root `index.html` exists but is missing the entry script,
 *     rewrite it (handles partial / Emergent-template HTML).
 *   - Else if `public/index.html` exists, copy + rewrite it as root
 *     `index.html` AND delete the public copy so Vite's static fallback
 *     can never serve the un-injected version.
 *   - Else generate a minimal placeholder.
 *
 * This runs both inside `convertCraToVite` (for fresh CRA imports) AND
 * via the safety net in `file-tree.js` (for projects that were already
 * converted in a prior session — `isCraPackage` returns false on those,
 * so without this, no entry script would ever be injected and the
 * iframe renders an empty body).
 */
export function ensureRootIndexHtml(scope) {
  if (!scope || typeof scope !== 'object') return scope
  const srcDir = scope.src?.directory
  const entryPath = detectEntryPoint(srcDir)
  const entryScriptRe = new RegExp(`<script[^>]+type=['"]module['"][^>]+src=['"]\\/${entryPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"]`)

  const rootHtmlNode = scope['index.html']
  if (rootHtmlNode?.file?.contents) {
    if (entryScriptRe.test(rootHtmlNode.file.contents)) {
      // Already correct — nothing to do (idempotent on re-runs).
      return scope
    }
    // Root index.html exists but is missing the entry script. Rewrite it.
    rootHtmlNode.file.contents = rewriteIndexHtml(rootHtmlNode.file.contents, entryPath)
    return scope
  }

  // No root index.html — try to copy + rewrite from public/index.html.
  const publicDir = scope.public?.directory
  const publicIndex = publicDir?.['index.html']?.file?.contents
  if (publicIndex) {
    scope['index.html'] = {
      file: { contents: rewriteIndexHtml(publicIndex, entryPath) },
    }
    // Delete the public copy so Vite's static-asset fallback (publicDir)
    // can't serve the un-injected version when the dev server resolves
    // `/`. This was the root cause of Mangia-Mama / Dopples gray screens
    // on re-import: convertCraToVite skipped because react-scripts had
    // already been removed in a prior session, so no root index.html
    // ever got created and Vite served the stale public/index.html
    // verbatim.
    delete publicDir['index.html']
    return scope
  }

  // No HTML anywhere — drop in a minimal Vite-compatible shell.
  scope['index.html'] = {
    file: {
      contents: rewriteIndexHtml(`<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
