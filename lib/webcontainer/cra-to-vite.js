...
export default defineConfig({
  ...
  // esbuild WASM with a 'runtime.gopark' Go panic, OR — when
  // pre-bundling is fully disabled — leave Vite hung silently in a
  // discovery scan. The reliable combo inside WebContainers:
  //   1. Explicit \`entries\` so Vite scans ONLY user code (src/), not
  //      every transitive dep.
  //   2. \`disabled: 'dev'\` skips bundling at dev-time only — much
  //      lighter than \`disabled: true\` and avoids the noDiscovery race
  //      condition documented at https://github.com/vitejs/vite/issues/13864
  //   3. \`force: true\` clears any stale .vite cache from previous
