...
export async function runDevServer(files, cbs = {}) {
  ...
  try {
    ...
    // Detect project layout up front — nested workspaces (frontend/, web/,
    // apps/web, etc.) need install + dev to run in that subdirectory.
    const devCmd = detectDevCommand(mountResult.tree || _currentMount?.tree)
    const cwd = devCmd.cwd || ''
    if (cwd) onLog(`[wc] detected nested workspace: ./${cwd}\n`)

    if (!mountResult.reused) {
      onStage('install', cwd ? `Running npm install in ./${cwd}…` : 'Running npm install…')
      // --legacy-peer-deps mirrors the pre-npm-7 behavior, sidestepping the
      // peer-dep ERESOLVE failures that block almost every real-world import
      // (react-day-picker vs date-fns, ESLint plugin chains, etc.). This is
      // the npm-recommended workaround when peer constraints are too strict.
      const install = await wc.spawn(
        'npm',
        ['install', '--no-audit', '--no-fund', '--loglevel=warn', '--legacy-peer-deps', '--prefer-offline'],
        cwd ? { cwd } : undefined
      )
    ...
    onStage('dev', `Starting dev server (${devCmd.cmd} ${devCmd.args.join(' ')}${cwd ? ` in ./${cwd}` : ''})…`)
    // CI=true + BROWSER=none keep CRA / craco from launching browsers or
    // prompting on port collision. NEXT_TELEMETRY_DISABLED keeps Next.js
    // logs clean. Forces Babel via injected .babelrc for Next imports.
