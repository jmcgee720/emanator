...
export async function runDevServer(files, cbs = {}) {
  ...
  try {
    ...
    // NEXT_DISABLE_SWC_NATIVE_BINARY=1 forces Next.js to use the WASM SWC
    // we injected via patchNextSwcWasm. Without it, Next still attempts
    // to download the native linux-x64 binary and fails inside the WC sandbox.
    const dev = await wc.spawn(devCmd.cmd, devCmd.args, {
      env: {
        NEXT_DISABLE_SWC_NATIVE_BINARY: '1',
