...
/**
 * Mutate an imported package.json (in WC tree form) to inject
 * `@next/swc-wasm-nodejs` matching the Next.js version. Required for
 * Next.js to run inside WebContainers (WASM Node sandbox can't load
 * the native linux-x64 SWC binary).
 *
...
export function patchNextSwcWasm(tree) {
  ...
  // Already pinned? Leave it.
  if (deps['@next/swc-wasm-nodejs'] || devDeps['@next/swc-wasm-nodejs']) return
  // Match the same version the project uses for next itself so npm
  // doesn't dual-install. Strip leading ^/~ for an exact pin.
