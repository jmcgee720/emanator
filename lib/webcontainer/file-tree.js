...
import { convertCraToVite, isCraPackage, renameCjsConfigsToCjs, bubbleCssImportsInTree, ensureRootIndexHtml } from './cra-to-vite.js'
...
export function ensureScaffolding(tree, opts = {}) {
  ...
  if (layout.packageJson) {
    ...
    if (scope) {
      ...
      // Root index.html safety net — for projects that were already
      // converted CRA→Vite in a prior session (Mangia-Mama, Dopples,
      // etc.), `convertCraToVite` is skipped because react-scripts is
      // gone from package.json. Without this call, no root index.html
      // would ever be created and Vite would serve the stale, entry-
      // script-less `public/index.html` to the iframe — body renders,
      // React never mounts. Idempotent: bails out if root index.html
      // already has the entry script tag.
