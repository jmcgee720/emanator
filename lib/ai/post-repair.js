...
export function ensureNavbarLogo(content) {
  ...
  // (a) Replace the recipe's default gradient-square placeholder. Matches both
  //     the pre-Session-22 hardcoded form (`bg-gradient-to-br from-violet-500`)
  //     AND the post-Session-22 themed form (`bg-[var(--primary)]` inside a
  //     small square span). The stale regex was why Session 24 builds still
  //     rendered a generic dot instead of the user's logo.
  const themedSquare = /<span[^>]*\bw-\d+\s+h-\d+\b[^"]*(?:rounded-(?:xl|lg|\[var\(--radius[^)]*\)\]))[^"]*(?:bg-\[var\(--primary\)\]|bg-\[var\(--accent\)\]|bg-gradient[^"]*(?:violet|indigo)[^"]*)[^"]*"[^/>]*\/>/
  if (themedSquare.test(out)) {
    out = out.replace(themedSquare, imgTag)
    return { content: out, changed: out !== original }
  }
  ...
  // (c) Last resort — prepend the img inside the first navbar brand button.
...
export function ensureHeroImage(content, imageAssets) {
  ...
  const imgTag = `<img src={${heroExport}} alt="" className="w-full rounded-[var(--radius-lg)] shadow-2xl" />`

  // (a) Replace the recipe's themed hero placeholder div — the flat
  //     accent-tinted square (Session 22+ recipes) OR the legacy violet/indigo
  //     gradient (pre-Session 22). Catches BOTH variants.
  const heroPlaceholder = /<div[^>]*className=["'][^"']*(?:bg-\[var\(--accent\)\]\s+opacity-[0-9]+|bg-gradient[^"]*(?:violet|indigo)[^"]*)[^"']*["'][^/>]*\/>/
  if (heroPlaceholder.test(out)) {
    out = out.replace(heroPlaceholder, imgTag)
    return { content: out, changed: out !== original }
  }

  // (b) Fallback: inject inside the hero section if no placeholder exists.
