...
export function checkForkNeeded(messages, modelName = 'claude-sonnet-4-5-20250929') {
  ...
  // Warn at 65% (gives user plenty of room to finish current task)
  // Hard cutoff at 75% (prevents hitting actual limit during next response)
  const needsFork = percentage >= 65
  const critical = percentage >= 75
