...
export function checkForkNeeded(messages, modelName = 'claude-sonnet-4-5-20250929') {
  ...
  // Warn at 70% (gives user plenty of room to finish current task)
  // Hard cutoff at 85% (prevents hitting actual limit during next response)
  const needsFork = percentage >= 70
  const critical = percentage >= 85
