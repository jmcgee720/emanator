...
export const AI_TOOLS = [
  ...
  // ── Search-and-replace editing tool — SAFEST way to edit files ──
  {
    type: 'function',
    function: {
      name: 'search_replace',
      description: 'Edit a file by finding exact text and replacing it. This is the SAFEST editing method. Copy the EXACT text you want to change from the file (as shown by read_files), then provide the replacement. If the search text is not found exactly, the edit fails safely — no partial changes. You can make multiple replacements in one call.',
      parameters: {
        type: 'object',
        properties: {
          path: {
            type: 'string',
            description: 'File path to edit (must match a path from read_files)'
          },
          edits: {
            type: 'array',
            items: {
              ...
              properties: {
                old_str: {
                  type: 'string',
                  description: 'The EXACT text to find in the file. Must match character-for-character including whitespace and indentation. Include 1-2 lines of surrounding context to ensure uniqueness.'
                },
                new_str: {
                  type: 'string',
                  description: 'The replacement text. Must maintain proper indentation matching the surrounding code.'
                }
              },
              required: ['old_str', 'new_str']
            },
            description: 'Array of search/replace pairs. Each is applied in order.'
          },
          summary: {
            type: 'string',
  ...
  // ── Line-number-based editing tool — fallback for large files ──
  {
    type: 'function',
    function: {
      name: 'edit_lines',
      description: 'Edit an existing file using line numbers. Use search_replace first — only use edit_lines when search_replace fails (e.g., the exact text is too long to copy accurately). First call read_files to see the numbered lines, then specify which lines to replace, insert after, or delete.',
      parameters: {
        type: 'object',
        properties: {
          path: {
            type: 'string',
            description: 'File path to edit (must match a path returned by read_files)'
          },
          edits: {
            type: 'array',
            items: {
              ...
              properties: {
                type: {
                  ...
                  description: 'replace = replace lines start-end with new code. insert_after = insert new code after the given line. delete = remove lines start-end.'
                },
                line_start: {
                  type: 'integer',
                  description: 'Starting line number (1-indexed, as shown by read_files)'
                },
                line_end: {
                  type: 'integer',
                  description: 'Ending line number (inclusive). For insert_after, this is ignored. For single-line edits, set equal to line_start.'
                },
                content: {
                  type: 'string',
                  description: 'The new code to insert or replace with. For delete operations, this is ignored.'
                }
              },
              required: ['type', 'line_start']
            },
            description: 'Array of line edits. Applied bottom-to-top automatically.'
          },
          summary: {
            type: 'string',
            description: 'Brief description of the changes'
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Take a screenshot of a URL and get a text description of what the page shows. Use after deploying/building to visually verify the result. Returns page title, headings, buttons, inputs, console errors, and a text preview.',
      parameters: {
        type: 'object',
        properties: {
          url: {
            type: 'string',
            description: 'URL to screenshot. For the current project preview, use the preview URL shown in the sandbox.'
          },
          description: {
            type: 'string',
            description: 'What you expect to see — used to compare against actual result'
          }
        },
        required: ['url']
      }
