 ...
 *   6 pts — logo present
 *   4 pts — hero / photo present
 *   3 pts — palette tokens present (non-default)
 *   2 pts — display font non-system
 * @private
 */
function scoreAssets(manifest) {
  ...
  let pts = 0
  const parts = []
  if (assets.hasLogo) { pts += 6; parts.push('logo') }
  if (assets.hasHero || assets.photoCount > 0) { pts += 4; parts.push('hero') }
  const themeStr = JSON.stringify(theme || {})
  if (theme.primary && !/#0a0a0a|#111827|#0f172a/i.test(theme.primary)) { pts += 3; parts.push('palette') }
