...
const WAVE_TEMPLATE = [
  {
    ...
    includes: ({ plan }) => {
      ...
      // When Supabase is wired, the scaffold must also emit the client file.
      if (plan.useSupabase) base.splice(1, 0, 'components/supabaseClient.jsx')
      // When Stripe is wired, scaffold also emits the stripe client.
      if (plan.useStripe) base.splice(1, 0, 'components/stripeClient.jsx')
      // landing_only archetype skips Footer? No — always include. Minimal overhead.
      return base.filter((f) => plan.archetypeId === 'landing_only'
        ? !['components/AuthContext.jsx', 'components/MockAPIProvider.jsx', 'components/supabaseClient.jsx', 'components/stripeClient.jsx'].includes(f)
        : true)
    },
  },
  {
 ...
 */
export async function generatePlan({ brief, archetype, provider, artDirection }) {
...
${artDirection ? '\nART DIRECTION (from user reference images — the generated app must visually match this):\n' + artDirection + '\n\nLet this aesthetic steer the component choices and heroCopy tone.\n' : ''}
