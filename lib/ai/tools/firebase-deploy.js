  ...
  return {
    ...
    async execute({ service_account_json, reason }) {
      ...
      const result = await fetchFromRunner(projectId, '/api/control/run-command', {
        ...
        body: {
          command: [
            // Write service account to /tmp (ephemeral, deleted after deploy)
            `echo '${service_account_json.replace(/'/g, "'\\''")}' > /tmp/firebase-sa.json`,
            // Set GOOGLE_APPLICATION_CREDENTIALS env var
            'export GOOGLE_APPLICATION_CREDENTIALS=/tmp/firebase-sa.json',
            // Deploy functions only (not hosting, firestore, etc.)
