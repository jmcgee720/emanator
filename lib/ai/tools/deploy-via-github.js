  ...
  return {
    ...
    async execute({ github_token, vercel_token, repo_name, repo_visibility = 'private', vercel_project_name, production = true }) {
      try {
        ...
        if (tree.length === 0) {
          console.error(`[deploy_via_github] ERROR: All ${files.length} files were filtered out`)
          console.error(`[deploy_via_github] Sample filtered paths:`, files.slice(0, 10).map(f => ({
            path: f.path,
            segments: f.path.split('/').filter(Boolean),
            wouldSkip: f.path.split('/').filter(Boolean).some(seg => 
              ['node_modules', '.git', 'build', 'dist', '.next', 'out', 'coverage'].includes(seg)
            ) || f.path.endsWith('.log')
          })))
          throw new Error(`No files to deploy after filtering. Found ${files.length} files but all were filtered. Check console logs for details.`)
        }
        
        console.log(`[deploy_via_github] Pushing ${tree.length} files to GitHub (filtered ${files.length - tree.length} files)`)
