...
function assembleCanvasContext(canvas) {
  ...
  const keySections = [
    ...
    'open_tasks', 'successful_patterns'
  ]
  const contextCanvas = {}
...
function formatProjectSystemMessage(context, builderMode) {
  const modeInstructions = {
    app: 'You are building a web application. Generate clean, modular, production-ready code.',
    website: 'You are building a website. Focus on responsive design, SEO, and user experience.',
    image: 'You are helping design visual assets. Describe images in detail for generation.',
    document: 'You are creating documentation and specifications. Be thorough and well-structured.'
  }
...
## Scope: PROJECT
You are working within the context of the user's currently selected project. All files and canvas data below belong to this project.

## Guidelines
1. Always provide complete, working code - never fragments
2. Follow modern best practices and conventions
3. Generate clean, well-commented code
4. When editing files, preserve existing functionality
5. Explain your changes clearly
6. Create proper file structure for projects

## Response Format
When generating or editing files, use this format:
...
### For chat-only responses (explanations, questions):
Respond naturally without the JSON format.
