...
export async function handleReadFilesTool(args, toolCall, ctx) {
  ...
  ctx.messages.push({ role: 'tool', tool_call_id: readCallId, content: toolResult })

  if (ctx.agentLoopCount >= 1) {
...
export async function handleVerifyBuildTool(args, toolCall, ctx) {
  ...
  ctx.messages.push({ role: 'tool', tool_call_id: verifyCallId, content: verifyResult })
...
export async function handleExecCommandTool(args, toolCall, ctx) {
  ...
  ctx.messages.push({ role: 'tool', tool_call_id: execCallId, content: execResult })
...
export async function handleScreenshotVerifyTool(args, toolCall, ctx) {
  ...
  ctx.messages.push({ role: 'tool', tool_call_id: ssCallId, content: screenshotResult })
