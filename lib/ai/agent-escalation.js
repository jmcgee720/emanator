...
import { db as defaultDb, getSupabaseAdmin } from '@/lib/supabase/db'
...
export async function createEscalationChat({
  ...
  try {
    ...
    const metadata = {
      is_escalation: true,
      auto_open: true, // Auto-expand the panel when detected
      escalation_source: {
        chat_id: fromChatId,
    ...
    const { data: escalationChat, error: chatError } = await supabase
      .from('chats')
      .insert({
        user_id: userId,
        project_id: null,
...
/**
 * Tool definition for project agents to send messages to an active escalation chat.
 * This allows the project agent to communicate with Core System after escalation.
 */
export const sendEscalationMessageTool = {
  name: 'send_escalation_message',
  description: [
    'Send a message to the active escalation chat where you are collaborating with Core System.',
    '',
    'Use this to:',
    '  • Report progress on your end ("I verified the new tool works in my project")',
    '  • Ask Core System for clarification ("What parameters does the new tool accept?")',
    '  • Provide feedback ("The tool works but returns unclear error messages")',
    '',
    'Your message will be tagged with agent_source="project_agent" so Core System knows it came from you.',
  ].join('\n'),
  input_schema: {
    type: 'object',
    properties: {
      message: {
        type: 'string',
        description: 'The message to send to Core System. Be clear and specific about what you need or what you observed.',
      },
    },
    required: ['message'],
  },
...
/**
 * Handler for send_escalation_message tool.
 * Posts a message to the escalation chat tagged with agent_source='project_agent'.
 */
export async function handleSendEscalationMessage({
  message,
  userId,
  chatId,
  projectId,
  ...
  try {
    // Find the active escalation chat for this user
    const { data: escalationRows, error: findError } = await supabase
      .from('chats')
      .select('id, title, metadata')
      .eq('user_id', userId)
      .is('project_id', null)
      .filter('metadata->>is_escalation', 'eq', 'true')
    ...
    if (!escalationRows || escalationRows.length === 0) {
      throw new Error('No active escalation found. Call escalate_to_core_system first to start a collaboration.');
    }
    
    const escalationChat = escalationRows[0];
    ...
    return {
      content: [
        `✅ **Message sent to Core System**`,
        ``,
        `Your message has been posted to the escalation chat. Core System will see it and respond.`,
      ].join('\n'),
    };
  } catch (error) {
    console.error('[SendEscalationMessage] Error:', error);
    throw new Error(`Failed to send escalation message: ${error.message}`);
  }
}
