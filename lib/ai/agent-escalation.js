...
export async function createEscalationChat({
  ...
  // Use the connection pooler endpoint (Supavisor format: postgres.[PROJECT_REF] as username)
...
export async function handleSendEscalationMessage({
  ...
  // Find the active escalation chat for this user using raw SQL
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  ...
  try {
    ...
    const { rows: escalationRows } = await client.query(`
      ...
      WHERE user_id = $1
        ...
        AND metadata->>'is_escalation' = 'true'
