 ...
 *   • Creates a new Core System chat with special metadata: { escalation: { from_chat_id, from_project_id, task_description } }
 *   • Both agents see each other's messages in the escalation chat
 *   • User sees a special UI: split view with project agent on left, Core System on right
 ...
 *   • Core System cannot write to the project's files (scope isolation)
...
export async function createEscalationChat({
  ...
  // Create the initial system message that explains the context to Core System
  const contextMessage = [
    `**ESCALATION FROM PROJECT AGENT**`,
    ``,
    `A project agent has escalated a task it cannot complete. You are now in a joint conversation with the project agent.`,
    ``,
    ...
    ``,
    `**Your role:**`,
    `  • Implement the missing capability in the Auroraly platform`,
    `  • Test it works`,
    `  • Tell the project agent when it's ready`,
    ``,
    `**Project agent's role:**`,
    `  • Verify the new capability works in their project`,
    `  • Report any issues`,
    ``,
    `**When done:**`,
    `The user will click "Exit Escalation" and return to the project chat. You should summarize what was built.`,
  ].join('\n')
...
export async function handleEscalation({
  ...
  // Return a message to the project agent
  return {
    content: [
      ...
      `**Urgency:** ${urgency}`,
      ``,
      `The user will now see both of us working together. Once Core System has built the feature, I'll verify it works in this project.`,
      ``,
      `[Open escalation chat →](/chats/${escalationChatId})`,
    ].join('\n'),
    escalationChatId,
  }
}
