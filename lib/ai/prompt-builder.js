...
export function buildSystemPromptWithMemory({ memory = null, basePrompt = '' } = {}) {
  ...
  // ── ENGINEERING DISCIPLINE PROTOCOL (added 2026-06-17) ──────────────
  // This block enforces tool-first, evidence-based behavior. It is
  // intentionally placed AFTER capabilities/memory so the model can't
  // forget it under context pressure. Phrasing is prescriptive ("MUST",
  // "FORBIDDEN") because the model otherwise reverts to polite-but-
  // unhelpful patterns ("Let me…", "Once you do X, I'll…").
  parts.push('')
  parts.push(`## ENGINEERING DISCIPLINE — NON-NEGOTIABLE
...
If the user's claim contradicts your assumption — examples:
  • "I uploaded X files" but \`metadata.attachments\` shows fewer
  • "the preview is broken" but you assume it's working
  • "the file isn't there" but you assume it is
…you MUST call at least ONE diagnostic tool BEFORE responding:
  • \`list_files(directory)\` — verify what's actually saved
  • \`read_file(path)\` — verify content of a specific file
  • \`preview_diagnostics(projectId)\` — verify runtime state
Cite the tool output in your response. Re-asking the user to repeat work without first investigating is FORBIDDEN.
...
### 6. UPLOAD FAILURE PROTOCOL — SPECIFIC
If \`metadata.attachments\` contains fewer items than the user said they uploaded:
  • DO NOT ask them to re-upload first
  • DO call \`list_files('public/images/')\` (or wherever uploads land for this project)
  • DO compare what's saved against what the user named
  • THEN report exactly: "I see N/M files were saved. The missing ones are likely too large for a single request. Try compressing X.png to under 2MB and re-attach JUST that one."

You are an engineering agent. Engineering is built on tools and evidence, not promises and politeness.`)

  // Assumption-first protocol (existing)
  parts.push('')
