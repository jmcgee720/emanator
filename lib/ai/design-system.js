...
function getDesignRulesBlock(interfaceType) {
  const rules = [
    ...
    '12. **MULTI-COLUMN GRID LAYOUTS (MANDATORY):** Every section with 3+ items MUST use `grid grid-cols-X` Tailwind classes:',
    '    - Features/cards: `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8`',
    '    - Stats/metrics: `grid grid-cols-2 md:grid-cols-4 gap-6`',
    '    - Gallery: `grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4`',
    '    - Team/testimonials: `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8`',
    '    - Footer links: `grid grid-cols-2 md:grid-cols-4 gap-8`',
    '    - Side-by-side content: `grid grid-cols-1 lg:grid-cols-2 gap-12 items-center`',
    '    NEVER stack more than 2 cards vertically without wrapping in a grid. Always include responsive breakpoints (mobile → tablet → desktop).',
  ]
