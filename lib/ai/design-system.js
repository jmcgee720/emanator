...
function getDefaultDesignBlock() {
  const preset = DESIGN_PRESETS.modern_saas
  const parts = []

  parts.push('## DESIGN DIRECTION (Default: Modern SaaS)')
  parts.push('')
  parts.push(`**Typography:** ${preset.typography.headline}; body: ${preset.typography.body}`)
  parts.push(`**Spacing:** ${preset.spacing.scale} density, section padding ${preset.spacing.sectionPadding}`)
...
function getDesignRulesBlock(interfaceType) {
  const rules = [
    ...
    '1. **Never output plain unstyled HTML.** Every element must have intentional styling.',
    '2. **Use a clear visual hierarchy:** headlines > subheads > body > captions. Size, weight, and color must differentiate levels.',
    '3. **Apply consistent spacing.** Use the spacing scale — no random padding/margin values.',
    '4. **Buttons must look intentional:** styled fills or outlines, proper padding, hover states, appropriate radius.',
    '5. **Cards must have structure:** consistent padding, rounded corners, subtle shadows or borders.',
    '6. **Use real-looking placeholder content** — no "Lorem ipsum". Use realistic names, descriptions, prices.',
    '7. **Color usage:** stick to the defined color philosophy. Max 2-3 brand colors + neutrals. Use color purposefully.',
    '8. **Responsive behavior:** design desktop-first, include mobile breakpoints. Stack columns, resize text, adapt navigation.',
    '9. **Whitespace is design:** generous spacing creates premium feel. Never cram elements together.',
    '10. **Consistent styling across all components.** Same radius, shadow, padding rules everywhere.',
    '11. **CRITICAL — STANDARD TAILWIND ONLY:** You MUST use ONLY built-in Tailwind CSS utility classes. NEVER invent custom class names like `bg-dark-premium`, `text-accent`, `bg-brand`, `bg-primary-dark`. Instead use standard Tailwind colors:',
    '    - Dark backgrounds: `bg-gray-900`, `bg-slate-950`, `bg-zinc-900`, `bg-neutral-950`',
    '    - Accent/brand: `bg-indigo-600`, `bg-violet-500`, `bg-emerald-500`, `bg-rose-500`',
    '    - Light text on dark: `text-white`, `text-gray-100`, `text-slate-200`',
    '    - Dark text on light: `text-gray-900`, `text-slate-800`',
    '    - Cards on dark bg: `bg-gray-800`, `bg-slate-800/50`, `bg-white/5`',
    '    - Borders: `border-gray-700`, `border-white/10`, `border-slate-200`',
    '    - Buttons: `bg-indigo-600 hover:bg-indigo-700`, `bg-white text-gray-900`',
    '    The Tailwind CDN is loaded — all standard utilities work. Custom names will NOT render.',
  ]

  if (interfaceType === 'dashboard') {
    rules.push('11. **Dashboard-specific:** use a sidebar + topbar shell. Metric cards at top. Dense but readable data.')
  } else if (interfaceType === 'mobile') {
    rules.push('11. **Mobile-specific:** single column, 44px touch targets, bottom nav, system-native patterns.')
