...
export async function loadPlatformContext(chatId) {
  const [chat, messages] = await Promise.all([
    ...
    db.messages.findByChatId(chatId, MAX_RECENT_MESSAGES),
  ])
  const compressedMessages = compressContext(messages)
  return {
    project: null,
    chat: chat ? { id: chat.id, title: chat.title, messages: compressedMessages.slice(-MAX_RECENT_MESSAGES).map(m => ({ role: m.role, content: m.metadata?.full_content || m.content, created_at: m.created_at })) } : null,
    files: [],
    canvas: null,
  }
}
