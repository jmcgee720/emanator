...
export function classifyProviderError(err, provider, model) {
  ...
  if (
    ...
    return new ProviderError({
      ...
      user_message: `${mName} is unavailable because the ${pName} API account for this key does not currently have enough billing/credits.`,
    })
  }
  ...
  return new ProviderError({
    ...
    provider, model, status_code: status,
    raw_error: err.message || String(err),
    user_message: `An unexpected error occurred with ${mName}. Please try again or switch to a different model.`,
  })
}
