...
export function buildVerifiedPatchResponse(result, isRefinement = true, isSelfEdit = false, userMessage = '') {
  ...
  if (isSelfEdit) {
    ...
    // Build a brief summary of what was done based on the user's request
    const msgShort = (userMessage || '').length > 120 ? userMessage.slice(0, 120) + '...' : userMessage
    let summary = ''
    if (verifiedItems.length > 0) {
      summary = verifiedItems.slice(0, 3).map(i => `- ${i}`).join('\n')
    } else if (msgShort) {
      summary = `- ${msgShort}`
    }

    let response = `Updated **${shortFiles}**.\n\n`
    if (summary) response += `**What changed:**\n${summary}\n`
