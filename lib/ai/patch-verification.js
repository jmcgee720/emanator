 ...
 */
export function buildVerifiedPatchResponse(result, isRefinement = true, isSelfEdit = false) {
  ...
  if (isSelfEdit) {
    ...
    if (effectiveStatus === 'VERIFIED') {
      response = `Done — updated ${shortFiles}. `
      if (verifiedItems.length > 0) response += verifiedItems.slice(0, 2).join('. ') + '. '
    } else if (effectiveStatus === 'CODE_VERIFIED_ONLY') {
      response = `Updated ${shortFiles} — code looks solid. `
    } else if (effectiveStatus === 'APPLIED_NO_AUTO_CHECKS') {
      response = `Updated ${shortFiles}. `
    } else {
      response = `Made changes to ${shortFiles}, but couldn't fully verify. `
      if (unverifiedItems.length > 0) response += `Check: ${unverifiedItems.slice(0, 2).join(', ')}. `
    }
