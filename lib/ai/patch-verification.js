 ...
 */
export function buildVerifiedPatchResponse(result, isRefinement = true, isSelfEdit = false, userMessage = '') {
  ...
  // Self-edit uses a conversational tone + inline action marker (rendered as button by frontend)
  const selfEditCta = '\n\n{{APPLY_TO_LIVE_BUTTON}}'

  if (isSelfEdit) {
    const shortFiles = filesChanged.map(f => f.split('/').pop()).join(', ')
    // Generate a clean summary from the user's request — extract key actions
    const msg = (userMessage || '').replace(/^(do all of this|please|can you|i want you to|add|implement|explore|update)[:\s]*/i, '').trim()
    // Extract action phrases (split by periods, commas, "and", newlines)
    const actions = msg.split(/[.,\n]|(?:\band\b)/i).map(a => a.trim()).filter(a => a.length > 5 && a.length < 100)
    let summary = ''
    if (actions.length > 1) {
      summary = actions.slice(0, 4).map(a => {
        // Capitalize first letter, lowercase the rest of first word
        const clean = a.replace(/^(add|implement|explore|create|update|enable|integrate)\s+/i, '')
        return clean.charAt(0).toUpperCase() + clean.slice(1)
      }).join(' · ')
    } else if (msg.length > 5) {
      summary = msg.charAt(0).toUpperCase() + msg.slice(1, 100)
    }

    let response = `Updated **${shortFiles}**`
    if (summary) response += ` — ${summary}`
    response += '.'
    return response + selfEditCta
  }

  // Non-self-edit responses (standard builder)
