...
export function buildVerifiedPatchResponse(result, isRefinement = true, isSelfEdit = false) {
  ...
  const reviewCta = isSelfEdit
    ? 'Review the changes in the **Code** tab. Click **Apply to Live** when ready.'
    : ''
  ...
  if (effectiveStatus === 'VERIFIED') {
    ...
    response += isSelfEdit ? reviewCta : `Check the preview — everything should be live. What would you like to tweak next?`
    return response
  }
  ...
  if (effectiveStatus === 'CODE_VERIFIED_ONLY') {
    ...
    response += isSelfEdit ? reviewCta : `Take a look at the preview to confirm it matches what you had in mind. Want me to adjust anything?`
    return response
  }
  ...
  if (effectiveStatus === 'APPLIED_NO_AUTO_CHECKS') {
    ...
    response += isSelfEdit ? reviewCta : `The changes are live in the preview — take a look and let me know if anything needs adjusting.`
    return response
  }
