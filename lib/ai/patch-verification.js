...
export function buildVerifiedPatchResponse(result, isRefinement = true, isSelfEdit = false, userMessage = '') {
  ...
  if (isSelfEdit) {
    ...
    // Always use the user's request as the summary — verifiedItems are designed for website builds and useless here
    const msgShort = (userMessage || '').length > 150 ? userMessage.slice(0, 150) + '...' : (userMessage || 'Code updated')

    let response = `Updated **${shortFiles}**.\n\n**What changed:** ${msgShort}\n`
