 ...
 */
export function constructArtDirectedPrompts(brief, count = 4) {
 ...
 */
export async function generateArtDirectedImages(provider, brief, count = 4) {
  const prompts = constructArtDirectedPrompts(brief, count)
  const results = []

  for (const placement of prompts) {
    try {
      const result = await provider.generateImage(placement.prompt, {
        size: placement.size,
        quality: 'high',
      })
      ...
      console.error(`[ArtDirection] Image generation failed for ${placement.role}:`, err.message)
    }
  }
...
export function buildImagePromptContext(images, brief, isCustom = false) {
  ...
  block += `You MUST use these real image URLs. Do NOT use placeholder URLs. Do NOT say you cannot add images.\n\n`

  images.forEach((img, i) => {
    const roleLabel = img.role ? ` [${img.role}]` : ''
    block += `- Image ${i + 1}${roleLabel}: \`${img.url}\`\n  Alt: "${img.alt}"\n`
  })
