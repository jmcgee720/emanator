...
const STOCK_LIBRARY = {
  ...
  food: [
    ...
    // Coffee / cafe / bar — added so subject-aware fallback finds real
    // matches for coffee-shop, espresso, barista, bar, bistro briefs
    // instead of returning random pizza/salad photos.
    { url: 'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=600&h=600&fit=crop', alt: 'Barista pulling espresso shot in cafe' },
    { url: 'https://images.unsplash.com/photo-1509042239860-f550ce710b93?w=600&h=600&fit=crop', alt: 'Latte with coffee art' },
    { url: 'https://images.unsplash.com/photo-1442512595331-e89e73853f31?w=600&h=600&fit=crop', alt: 'Coffee beans roasting in bag' },
    { url: 'https://images.unsplash.com/photo-1453614512568-c4024d13c247?w=600&h=600&fit=crop', alt: 'Cozy cafe interior with wooden bar' },
    { url: 'https://images.unsplash.com/photo-1511920170033-f8396924c348?w=600&h=600&fit=crop', alt: 'Pour over coffee brew' },
    { url: 'https://images.unsplash.com/photo-1559925393-8be0ec4767c8?w=600&h=600&fit=crop', alt: 'Espresso cup on cafe table' },
    { url: 'https://images.unsplash.com/photo-1559496417-e7f25cb247f3?w=600&h=600&fit=crop', alt: 'Whiskey bar with bottles' },
    { url: 'https://images.unsplash.com/photo-1572116469696-31de0f17cc34?w=600&h=600&fit=crop', alt: 'Cocktail bartender at bistro' },
  ],
  people: [
...
const VIBE_LEXICON = {
  ...
  subjects: {
    ...
    abstract:     'abstract macro photography, color field, texture detail, pattern repetition, creative visual',
    ...
    technology:   'technology product shot, screen interface mockup, futuristic workspace, clean lines, blue-toned lighting',
    education:    'warm educational setting, books and learning materials, bright environment, knowledge and growth',
    music:        'concert atmosphere, studio equipment, sound waves, vibrant stage lighting, artistic expression',
    fashion:      'fashion editorial, model pose, designer aesthetic, stylized lighting, trendy setting',
    automotive:   'automotive photography, dynamic angle, sleek finish, dramatic lighting, motion blur',
    finance:      'corporate finance aesthetic, city skyline, data visualization, professional atmosphere, blue tones',
  },
}
...
export function parseCreativeBrief(userMessage) {
  ...
  const subjectKeywords = {
    product: ['product', 'item', 'merchandise', 'goods', 'showcase', 'retail', 'shop', 'store', 'ecommerce', 'e-commerce', 'marketplace'],
    portrait: ['portrait', 'headshot', 'team', 'people', 'person', 'face', 'profile', 'about us', 'staff', 'employees'],
    landscape: ['landscape', 'scenic', 'vista', 'horizon', 'panorama', 'outdoor'],
    food: ['food', 'restaurant', 'recipe', 'cuisine', 'chef', 'cooking', 'menu', 'cafe', 'bakery', 'coffee', 'roast', 'brew', 'bar', 'bistro', 'diner'],
    architecture: ['building', 'architecture', 'real estate', 'property', 'interior', 'room', 'house', 'apartment', 'office', 'workspace', 'coworking'],
    abstract: ['abstract', 'pattern', 'texture', 'gradient', 'geometric', 'generative', 'creative', 'agency', 'design', 'studio', 'brand', 'branding'],
    lifestyle: ['lifestyle', 'wellness', 'fitness', 'yoga', 'travel', 'adventure', 'health', 'spa', 'meditation', 'retreat', 'coaching', 'consulting'],
    ...
    technology: ['tech', 'technology', 'software', 'app', 'saas', 'startup', 'ai', 'digital', 'innovation', 'code', 'developer', 'platform', 'api', 'data', 'cloud', 'cyber'],
    education: ['education', 'school', 'university', 'course', 'learning', 'training', 'academy', 'tutor', 'teach', 'student'],
    music: ['music', 'band', 'artist', 'concert', 'studio', 'album', 'producer', 'dj', 'sound', 'audio', 'podcast'],
    fashion: ['fashion', 'clothing', 'apparel', 'boutique', 'style', 'wear', 'collection', 'designer', 'accessory', 'jewelry'],
    automotive: ['car', 'auto', 'vehicle', 'motor', 'drive', 'garage', 'racing', 'automotive'],
    finance: ['finance', 'bank', 'investment', 'trading', 'crypto', 'fintech', 'insurance', 'wealth', 'money', 'capital'],
  }
...
export function constructArtDirectedPrompts(brief, count = 2) {
  ...
  // Build base context from the user's raw message — extract business-specific fields
  // The rawMessage starts with a generic instruction preamble; the actual business info
  // comes in structured lines like "Project: ...", "Brand name: ...", etc.
  let contextHint = ''
  const lines = brief.rawMessage.split('\n')
  const businessFields = []
  for (const line of lines) {
    const trimmed = line.trim()
    if (/^(Project|Brand name|Target audience|Headline|Key messaging|Style\/mood|Most important page):/i.test(trimmed)) {
      businessFields.push(trimmed)
    }
  }
  if (businessFields.length > 0) {
    contextHint = businessFields.join('. ').slice(0, 300)
  } else {
    // Fallback: skip the generic preamble and use the meaningful part
    const meaningful = brief.rawMessage.replace(/^Build this project.*?styling[^.]*\./is, '').trim()
    contextHint = (meaningful || brief.rawMessage).slice(0, 300)
  }
  // Always prefix with subject type for clarity
  if (brief.subjects.length > 0 && !contextHint.toLowerCase().includes(brief.subjects[0])) {
    contextHint = `${brief.subjects[0]} photography/imagery. ${contextHint}`
  }
...
export async function generateArtDirectedImages(provider, brief, count = 2) {
  if (typeof provider?.generateImage !== 'function') {
    throw new Error(`Provider does not support image generation (model: ${provider?.model || 'unknown'})`)
  }
