...
import { formatTokensForPrompt, formatBlueprintForPrompt } from './design-tokens.js'
import { formatPrimitivesForPrompt } from './primitives.js'
...
export function buildWaveSystemPrompt({ plan, wave, filesBuiltSoFar }) {
  ...
  const recipeBlock = formatRecipesForPrompt(recipeIds, plan.recipeFamily?.family)
...
export async function* buildWave({
  ...
  const referenceImages = Array.isArray(plan.referenceImages) && plan.referenceImages.length > 0
    ? plan.referenceImages
    : (Array.isArray(plan.imageAssets) ? plan.imageAssets : [])
  const messages = [
  ...
  // Retry once non-streaming if still empty, with a STRICTER reminder.
  // Common failure mode: small/chatty models (Haiku, Flash) print the JSON
  // as conversational text instead of calling the tool. We prepend an
  // assistant-directive nudge that forces tool use.
  if (files.length === 0 && provider.chatWithTools) {
    try {
      const reminderMessages = [
        ...messages,
        { role: 'assistant', content: `I was about to respond with JSON as text, but I must call the \`create_files\` tool instead. Let me use the tool now.` },
        { role: 'user', content: `Yes — call the \`create_files\` tool with the files for this wave. Do NOT print JSON in your text response. Do NOT explain. Just call the tool exactly once with all files for this wave in a single call.` },
      ]
      const retry = await provider.chatWithTools(reminderMessages, CREATE_TOOLS, {
        temperature: 0.4,
        max_tokens: 16384,
        tool_choice: { type: 'function', function: { name: 'create_files' } },
      })
  ...
  if (kept.length === 0) {
    const diag = files.length === 0
      ? (toolArgsAccum.length > 0 ? `tool args truncated (${toolArgsAccum.length} chars, JSON parse failed)` : 'LLM returned no tool calls')
      : `files had unexpected paths; expected ${[...expectedSet].join(', ')}; got ${files.map((f) => f.path).join(', ')}`
    console.error(`[buildWave] ${wave.id} failed: ${diag}`)
    yield {
      event: 'wave_error',
      data: {
        waveId: wave.id,
        message: `wave produced 0 expected files (${diag})`,
      },
    }
