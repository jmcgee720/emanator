...
import { escalateToCoreSystemTool, handleEscalation, sendEscalationMessageTool, handleSendEscalationMessage } from './agent-escalation.js'
...
export function runSqlMigrationTool() {
  return {
    ...
    async execute({ sql, description }) {
      ...
      // Use our internal API endpoint which handles the pg connection
      ...
      if (!response.ok) {
        ...
        throw new Error(`SQL execution failed: ${errorData.error || response.statusText}`)
      }
...
export function buildDefaultToolset(rawScope, writer = null, reader = null, attachments = null, guardCtx = null, projectId = null, screenshotCtx = null, userId = null, chatId = null) {
  ...
  if (projectId && !guardCtx?.isSelfEdit) {
    ...
    if (userId && chatId) {
      ...
      // send_escalation_message: project agent can send messages to the
      // active escalation chat to communicate with Core System.
      const sendMessageTool = {
        ...sendEscalationMessageTool,
        async execute(args) {
          return handleSendEscalationMessage({
