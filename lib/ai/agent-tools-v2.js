...
import { getPreviewLogsTool, getBrowserConsoleTool, getNetworkLogTool, previewDiagnosticsTool, refreshPreviewTool, runCommandInPreviewTool } from './tools/preview-diagnostics.js'
import { screenshotPreviewTool } from './tools/screenshot-preview.js'
import { deployFirebaseFunctionsTool } from './tools/firebase-deploy.js'
import { deployToVercelTool } from './tools/vercel-deploy.js'
import { deployViaGithubTool } from './tools/deploy-via-github.js'
import { escalateToCoreSystemTool, handleEscalation } from './agent-escalation.js'
...
/**
 * Time Machine tools — snapshot & revert for Core System safety.
 * Only exposed in self-edit mode (guardCtx.isSelfEdit === true).
 */
export function createSnapshotTool() {
  return {
    name: 'create_snapshot',
    description: 'Create a snapshot of the current Auroraly source tree. Reads all files from GitHub and stores them in the snapshots table. Use this before making risky changes or when you want to save a known-good state.',
    input_schema: {
      type: 'object',
      properties: {
        name: {
          type: 'string',
          description: 'Optional snapshot name (e.g. "Before auth refactor"). If omitted, auto-generates a timestamped name.',
        },
      },
      required: [],
    },
    async execute({ name }) {
      const { createSnapshot } = await import('./time-machine.js')
      const snapshot = await createSnapshot(name || null)
      return [
        `Snapshot created: ${snapshot.name}`,
        `  ID: ${snapshot.id}`,
        `  Files: ${snapshot.file_count}`,
        `  Commit: ${snapshot.commit_hash || 'unknown'}`,
...
export function listSnapshotsTool() {
  return {
    name: 'list_snapshots',
    description: 'List recent snapshots of the Auroraly source tree. Shows snapshot ID, name, file count, and timestamp. Use this to find a snapshot to revert to.',
    input_schema: {
      type: 'object',
      properties: {
        limit: {
    ...
    async execute({ limit }) {
      const { listSnapshots } = await import('./time-machine.js')
      const snapshots = await listSnapshots(limit || 20)
      
      if (snapshots.length === 0) {
        return 'No snapshots found. Create one with create_snapshot.'
      }
      
      const lines = snapshots.map((s, i) => {
        const autoTag = s.auto ? ' [auto]' : ''
        const kb = Math.round(s.total_size / 1024)
        return `${i + 1}. ${s.name}${autoTag}\n   ID: ${s.id}\n   Files: ${s.file_count} (${kb} KB)\n   Commit: ${s.commit_hash || 'unknown'}\n   Created: ${s.created_at}`
      })
      ...
      return [
        ...
        ...lines,
        '',
        'To revert to a snapshot, call revert_to_snapshot with the ID.',
      ].join('\n')
    },
  }
}

export function revertToSnapshotTool() {
  return {
    name: 'revert_to_snapshot',
    description: 'Revert the Auroraly source tree to a snapshot. DANGER: This overwrites ALL files in the repo with the snapshot contents. Use only when Core System has broken itself and you need to restore a known-good state. Requires explicit user confirmation.',
    input_schema: {
      type: 'object',
      properties: {
        snapshot_id: {
          type: 'string',
          description: 'Snapshot ID to revert to (from list_snapshots)',
        },
        confirmed: {
          type: 'boolean',
      ...
      required: ['snapshot_id', 'confirmed'],
    },
    async execute({ snapshot_id, confirmed }) {
      if (!confirmed) {
        return [
      ...
      const { revertToSnapshot } = await import('./time-machine.js')
      const result = await revertToSnapshot(snapshot_id, true)
      
      return [
        `Reverted to snapshot: ${result.snapshot_name}`,
        `  Files restored: ${result.files_restored}`,
        `  Files failed: ${result.files_failed}`,
        '',
        result.files_failed > 0
          ? `Failed files:\n${result.failed_files.map(f => `  • ${f.path}: ${f.error}`).join('\n')}`
...
export function diffSnapshotTool() {
  return {
    name: 'diff_snapshot',
    description: 'Show the diff between the current state and a snapshot. Returns files that were added, modified, or deleted since the snapshot. Use this to see what changed before deciding whether to revert.',
    input_schema: {
      type: 'object',
      properties: {
        snapshot_id: {
          type: 'string',
          description: 'Snapshot ID to diff against (from list_snapshots)',
        },
      },
      required: ['snapshot_id'],
    },
    async execute({ snapshot_id }) {
      const { diffSnapshot } = await import('./time-machine.js')
      const diff = await diffSnapshot(snapshot_id)
      
      const lines = []
      lines.push(`Diff vs snapshot: ${diff.snapshot_name}`)
      lines.push(`Total changes: ${diff.total_changes}`)
      lines.push('')
      
      if (diff.added.length > 0) {
        lines.push(`Added (${diff.added.length}):`)
        diff.added.slice(0, 20).forEach(p => lines.push(`  + ${p}`))
      ...
      if (diff.modified.length > 0) {
        ...
        diff.modified.slice(0, 20).forEach(p => lines.push(`  ~ ${p}`))
        if (diff.modified.length > 20) lines.push(`  …and ${diff.modified.length - 20} more`)
        lines.push('')
      }
      
      if (diff.deleted.length > 0) {
        lines.push(`Deleted (${diff.deleted.length}):`)
        diff.deleted.slice(0, 20).forEach(p => lines.push(`  - ${p}`))
        if (diff.deleted.length > 20) lines.push(`  …and ${diff.deleted.length - 20} more`)
        lines.push('')
      }
      
      if (diff.total_changes === 0) {
        lines.push('No changes detected. Current state matches the snapshot.')
      }
...
/**
 * run_sql_migration — Execute SQL on Supabase database.
 * Only exposed in self-edit mode (Core System). Allows the agent to run
 * migrations, create tables, add columns, etc. without requiring the user
 * to manually paste SQL into the Supabase dashboard.
 */
export function runSqlMigrationTool() {
  return {
    name: 'run_sql_migration',
    description: 'Execute SQL on the Supabase database. Use this to run migrations, create tables, add columns, update RLS policies, etc. Only available in Core System self-edit mode.',
    input_schema: {
      type: 'object',
      properties: {
        sql: {
          type: 'string',
          description: 'The SQL to execute. Can be a full migration file or a single statement.',
        },
        description: {
          type: 'string',
          description: 'Brief description of what this SQL does (e.g. "Add user_id and metadata columns to chats table")',
        },
      },
      required: ['sql', 'description'],
    },
    async execute({ sql, description }) {
      ...
      if (!supabaseUrl || !supabaseServiceKey) {
        throw new Error('Supabase credentials not configured (NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY missing)')
      }
      
      console.log(`[run_sql_migration] Executing: ${description}`)
      console.log(`[run_sql_migration] SQL length: ${sql.length} chars`)
      
      // Use Supabase Management API to execute SQL
      // The service role key has full access to execute arbitrary SQL
      ...
      // If exec_sql RPC doesn't exist, we need to use the pg library directly
      // via a Supabase Edge Function or create the helper function first
      if (!response.ok) {
        const errorText = await response.text()
        
        // Try to create the exec_sql helper function first
        const helperSql = `
...
`
        ...
        // Use the Supabase client to try creating the function
        const { createClient } = await import('@supabase/supabase-js')
        const supabase = createClient(supabaseUrl, supabaseServiceKey, {
          auth: { persistSession: false },
        })
        
        // Try executing the helper creation via raw query
        // This is a chicken-and-egg problem, so we'll use a different approach:
        // Use the Supabase SQL endpoint directly
        const sqlEndpoint = supabaseUrl.replace('https://', 'https://').replace('.supabase.co', '.supabase.co')
        
        // Actually, the simplest approach: use node-postgres directly
        const { Pool } = await import('pg')
        const connectionString = process.env.DATABASE_URL || process.env.SUPABASE_DB_URL
        
        if (!connectionString) {
          throw new Error([
            'Cannot execute SQL: DATABASE_URL not configured.',
            '',
            'To enable SQL migrations, add DATABASE_URL to your environment variables:',
        ...
        try {
          ...
          return [
            `✅ SQL executed successfully: ${description}`,
            '',
            result.rows && result.rows.length > 0
              ? `Rows affected: ${result.rowCount}\nSample: ${JSON.stringify(result.rows.slice(0, 3), null, 2)}`
              : `Rows affected: ${result.rowCount || 0}`,
            '',
            'The migration has been applied to the Supabase database.',
          ].join('\n')
        } catch (pgError) {
          await pool.end()
          throw new Error(`SQL execution failed: ${pgError.message}`)
        }
      }
      ...
      return [
        `✅ SQL executed successfully: ${description}`,
        '',
        result ? `Result: ${JSON.stringify(result, null, 2)}` : 'No data returned (DDL statement)',
        '',
        'The migration has been applied to the Supabase database.',
      ].join('\n')
    },
 ...
 *   - projectId: when present, exposes preview diagnostic tools (get_preview_logs,
 *     get_browser_console, get_network_log) for project-scoped agents.
 *   - userId: when present with projectId, enables escalate_to_core_system tool.
 *   - chatId: when present with projectId, enables escalate_to_core_system tool.
 */
export function buildDefaultToolset(rawScope, writer = null, reader = null, attachments = null, guardCtx = null, projectId = null, screenshotCtx = null, userId = null, chatId = null) {
  const tools = [
  ...
  // Core System quick reference + self-diagnostic + Time Machine + SQL migrations — only in self-edit mode
  if (guardCtx?.isSelfEdit) {
    tools.push(coreSystemReferenceTool())
    tools.push(selfDiagnosticTool(rawScope, writer, reader))
    tools.push(createSnapshotTool())
    tools.push(listSnapshotsTool())
    tools.push(revertToSnapshotTool())
    tools.push(diffSnapshotTool())
    tools.push(runSqlMigrationTool())
  }
  
  // Preview diagnostic tools — only in project mode (when projectId is provided)
  if (projectId && !guardCtx?.isSelfEdit) {
    tools.push(previewDiagnosticsTool(projectId))   // ← deep one-shot diagnostic (image staleness + WS upgrade probe + verdict)
    tools.push(getPreviewLogsTool(projectId))
    tools.push(getBrowserConsoleTool(projectId))
    tools.push(getNetworkLogTool(projectId))
    tools.push(refreshPreviewTool(projectId))       // ← agent-triggered preview refresh
    tools.push(runCommandInPreviewTool(projectId))  // ← agent-triggered terminal commands (firebase deploy, npm install, etc.)
    tools.push(deployFirebaseFunctionsTool(projectId))  // ← Firebase Functions deployment (full-stack deployments without local filesystem)
    tools.push(deployToVercelTool(projectId))       // ← Vercel deployment (deploy project to production via Vercel API)
    tools.push(deployViaGithubTool(projectId))      // ← GitHub → Vercel deployment (bypasses 10 MB API limit)

    // screenshot_preview: the agent's eyes. Captures the live rendered
    // state of the Fly preview so Claude can SEE what the user sees
    // before/after edits. Closes the "agent confidently edits wrong
    // file because grep matched a keyword" failure mode. Disabled in
    // self-edit chats (no Fly preview to capture there).
    if (screenshotCtx?.appBaseUrl) {
      tools.push(screenshotPreviewTool(projectId, screenshotCtx.appBaseUrl, screenshotCtx.authHeaders || {}))
    }
    
    // escalate_to_core_system: project agent can spawn a joint conversation
    // with Core System when it hits capability gaps. Only exposed when we
    // have userId + chatId to create the escalation chat.
    if (userId && chatId) {
      const escalationTool = {
        ...escalateToCoreSystemTool,
        async execute(args) {
...
export function saveAttachmentTool(rawScope, writer, attachments) {
  ...
  return {
    ...
    async execute({ attachment_index, attachment_filename, path: reqPath }) {
      ...
      let dataUrl = att.preview_data || att.data
      
      // ── Rehydration: fetch binary from project_files if missing ────────
      // When images are uploaded, the client sends them as vision content
      // blocks. The upload endpoint saves the base64 data URL to
      // project_files and returns it in the response, but the client may
      // strip it from metadata.attachments to stay under Vercel's ~4MB
      // request body cap. If preview_data is missing here, rehydrate by
      // reading the file back from project_files (same pattern as
      // attachmentToContentBlock in stream-handler-v2.js line 710-718).
      if (!dataUrl && att.path) {
        try {
          const { db } = await import('@/lib/supabase/db')
          // Extract projectId from scope.rootDirs (format: '/project-<uuid>')
          const projectIdMatch = scope.rootDirs[0]?.match(/\/project-([a-f0-9-]+)$/)
          const projectId = projectIdMatch ? projectIdMatch[1] : null
          if (projectId) {
            const row = await db.projectFiles.findByPath(projectId, att.path)
            if (row?.content) {
              dataUrl = row.content
              console.log(`[save_attachment_to_path] rehydrated binary for ${att.filename} from project_files`)
            }
          }
        } catch (err) {
          console.warn(`[save_attachment_to_path] rehydration failed for ${att.path}: ${err.message}`)
        }
      }
      
      if (!dataUrl || typeof dataUrl !== 'string') {
        throw new Error(`Attachment "${att.filename}" has no binary data on this turn (file_category=${att.file_category}). The file may not have been uploaded successfully, or the binary data was stripped. Ask the user to re-upload the file.`)
      }
  ...
  return {
    ...
    async execute(args) {
      ...
      const STOPWORDS = new Set([
        'page', 'screen', 'view', 'modal', 'tab',
        'button', 'link', 'menu', 'icon',
        'field', 'item', 'list', 'card', 'overview',
        'metrics', 'dashboard', 'settings', 'home', 'main',
        'with', 'and',
        ...
        'clicked', 'hover', 'state', 'states', 'highlighted',
        'text', 'label', 'labels', 'title', 'titles',
        'name', 'names', 'window', 'tabs',
        // Layout & position descriptors — these are LEGITIMATE spatial language
        'below', 'above', 'beside', 'next', 'under', 'over', 'near',
        'inside', 'outside', 'within', 'between', 'around', 'across',
        'top', 'bottom', 'left', 'right', 'center', 'middle',
        // Visual property descriptors — these are LEGITIMATE color/style language
        'purple', 'blue', 'green', 'yellow', 'orange', 'white', 'black',
        'gray', 'grey', 'dark', 'light', 'bright', 'gradient', 'solid',
        'transparent', 'opaque', 'faded', 'bold', 'italic', 'underlined',
        'background', 'color', 'colors',
        // UI state descriptors — these are LEGITIMATE state language
        'open', 'closed', 'expanded', 'collapsed', 'focused', 'blurred',
        'loading', 'loaded', 'empty', 'filled', 'checked', 'unchecked',
        // Size & dimension descriptors — these are LEGITIMATE size language
        'large', 'small', 'wide', 'narrow', 'tall', 'short', 'full',
        'half', 'quarter', 'double', 'triple',
        // Structural descriptors — these are LEGITIMATE layout language
        'section', 'panel', 'area', 'region', 'row', 'column',
        'sidebar', 'header', 'footer', 'headers', 'footers', 'sidebars',
        'dropdown',
      ])
