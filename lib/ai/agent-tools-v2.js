...
export function runSqlMigrationTool() {
  return {
    ...
    async execute({ sql, description }) {
      ...
      if (error) {
        // If exec_sql RPC doesn't exist, fall back to direct query
        if (error.code === '42883') {
          console.log('[run_sql_migration] exec_sql RPC not found, using direct query')
          const { data: directData, error: directError } = await supabase.from('_sql_exec').select('*').limit(0)
          
          // Try raw SQL execution via the REST API
          ...
          if (!response.ok) {
            // Last resort: try to execute via the SQL editor endpoint
            // This requires creating a helper function first
            const helperSql = `
            ...
            const { error: helperError } = await supabase.rpc('exec_sql', { sql_query: helperSql })
            
            if (helperError) {
              throw new Error(`Failed to execute SQL: ${directError?.message || error.message}. The exec_sql helper function does not exist and could not be created. You need to run this SQL manually in the Supabase SQL Editor.`)
            }
            
            // Retry with the helper function
            const { data: retryData, error: retryError } = await supabase.rpc('exec_sql', { sql_query: sql })
            
            if (retryError) {
              throw new Error(`SQL execution failed: ${retryError.message}`)
            }
      ...
      return [
        `✅ SQL executed successfully: ${description}`,
        '',
        data ? `Result: ${JSON.stringify(data, null, 2)}` : 'No data returned (DDL statement)',
        '',
        'The migration has been applied to the Supabase database.',
      ].join('\n')
    },
