 ...
 */
import { containsPlaceholderLanguage } from './file-context-loader.js'
 ...
 */
export function validatePlan(plan, fileContext, previousRejectedHash = null, userMessage = null) {
  const errors = []
  const warnings = []
...
const PATCH_PLACEHOLDER_PATTERNS = [
  /\bassume\b/i,
  /\bexisting code\b/i,
  /\bplaceholder\b/i,
  /\binsert here\b/i,
  /\bwhere metadata is read\b/i,
  /\bexisting imports and component structure\b/i,
  /\.\.\.\s*$/m,
...
export function validatePatchGrounding(diffEntries, filesByPath, planContext) {
  ...
  for (const diff of diffEntries) {
    ...
    for (const pattern of PATCH_PLACEHOLDER_PATTERNS) {
      if (pattern.test(newContent)) {
        errors.push(`"${path}": generated code contains placeholder language: "${newContent.match(pattern)?.[0]}"`)
        break
      }
    }
