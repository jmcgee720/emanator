...
export function validatePlan(plan, fileContext, previousRejectedHash = null, userMessage = null, opts = {}) {
  ...
  // 0. Project ID grounding — plan must target the active project (warning only, stamped by message-stream)
  if (activeProjectId && plan.projectId && plan.projectId !== activeProjectId) {
    warnings.push('Plan projectId mismatch — may target wrong project')
  }
  ...
  // 9. Minimal patch check — warn if many files, hard reject only for very large plans
  if ((plan.file_actions || []).length > 30) {
    errors.push(`Plan touches ${plan.file_actions.length} files — exceeds maximum of 30`)
  } else if ((plan.file_actions || []).length > 15) {
