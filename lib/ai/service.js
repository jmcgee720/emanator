...
import { classifyIntent, getIntentWorkflow, getIntentSystemAddendum, shouldUsePlanMode, resolveTaskMode, classifyRequestMode, isSimpleFrontendEdit, findMainPagePath, isProceedSignal, isLargeAppBuild } from './intents.js'
...
export class AIService {
  ...
  async *processMessageStream({ projectId, chatId, userMessage, userId, scope: requestedScope, designPrefs, executePlan, attachments, selfEditTarget }) {
    ...
    try {
      ...
      if (intent === 'build' && effectiveScope === 'project' && !directEditMode
                ...
                if (directEditMode) {
                  ...
                  if (savedFiles.length === 0) {
                    ...
                    if (directEditRetry < 1) {
                      ...
                      messages.push({ role: 'user', content: 'The file write failed. Please try generating the code again and call the tool.' })
                      fullContent = ''
                      toolCalls = []
                      continue
                    }
                    // Retry exhausted — conversational error
                    fullContent = `I tried to build that but couldn't generate the files. Could you try a more specific request?`
                    yield { event: 'token', data: { content: fullContent } }
                    continue
                  }
