...
_defaultModel(provider) {
  ...
  async *processMessageStream({ projectId, chatId, userMessage, userId, scope: requestedScope, designPrefs, executePlan, attachments, selfEditTarget }) {
    ...
    try {
      ...
      // ── HARD GUARD: plan_patch mode NEVER routes to image generation ──
      if (requestMode === 'plan_patch' && workflow.toolMode === 'image_gen') {
        console.warn('[AIService] Image generation blocked — requestMode is plan_patch, forcing build workflow')
