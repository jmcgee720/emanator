...
export class AIService {
  ...
  _apiKey(provider) {
    // Platform-managed keys ONLY — no user-provided or direct provider keys at runtime
    const emergentKey = process.env.EMERGENT_LLM_KEY
    if (emergentKey) return emergentKey
    // Fallback to server-side env keys (still platform-managed, not user-supplied)
    const keys = {
    ...
    if (!key) {
      console.error(`[AIService] No platform API key available for provider "${provider}"`)
    }
    return key
  }

  /**
   * Returns the direct provider API key (bypassing Emergent proxy).
   * Used ONLY as a last resort when the proxy budget is exceeded
   * but the user still has a direct key configured.
   */
  _directApiKey(provider) {
    const directKeys = {
      openai: process.env.OPENAI_API_KEY,
      anthropic: process.env.ANTHROPIC_API_KEY,
    }
    const key = directKeys[provider]
    // Only return if it's a real provider key, not the emergent key
    if (key && key !== process.env.EMERGENT_LLM_KEY) return key
    return null
  }
  ...
  async *_streamWithFallback(makeStream) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        // Proxy budget errors: skip model fallback (same key = same failure).
        // Instead, try a direct API key bypass if available.
        if (isProxyBudget) {
          console.warn(`[StreamRetry] Proxy budget exceeded — skipping model fallback (same key)`)
          const directKey = this._directApiKey(this.providerName)
          if (directKey) {
            console.warn(`[StreamRetry] Attempting direct API key bypass for ${this.providerName}`)
            this.provider = createProvider(this.providerName, directKey, this.modelName, {})
            this._usedDirectBypass = true
            try {
              for await (const chunk of makeStream()) {
                yield chunk
              }
              yield { event: '_fallback_used', data: { provider: this.providerName, model: this.modelName, directBypass: true } }
              return
            } catch (bypassErr) {
              console.error(`[StreamRetry] Direct key bypass also failed: ${String(bypassErr?.message || '').slice(0, 120)}`)
              lastErr = bypassErr
            }
          }
          throw err
