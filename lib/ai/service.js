  ...
  async *_streamWithFallback(makeStream) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        // Context length errors: auto-trim messages and signal for retry
        const isContextLength = err?.error_type === 'context_length'
        if (isContextLength) {
          console.warn(`[StreamRetry] Context length exceeded — will be handled by caller with message trimming`)
          const trimErr = new Error('context_length_auto_trim')
          trimErr.error_type = 'context_length_auto_trim'
          throw trimErr
        }
