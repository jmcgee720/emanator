...
import { detectImageCategories, hasVisualIntent, getStockPhotos, generateArtDirectedImages, buildImagePromptContext, buildDesignIntelligencePrompt, parseCreativeBrief, analyzeImage } from './image-prefetch.js'
...
export class AIService {
  /**
   * Analyze an image and extract relevant features or metadata.
   * @param {Buffer} imageBuffer - The image data in buffer format.
   * @returns {Object} Analysis results, such as detected objects, colors, etc.
   */
  ...
  async *_streamWithFallback(makeStream) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        // Context length errors: fallback to a smaller model won't help (same/smaller context window)
        const isContextLength = err?.error_type === 'context_length'
        if (isContextLength) {
          console.warn(`[StreamRetry] Context length exceeded — fallback skipped (same context window)`)
          throw err
        }
