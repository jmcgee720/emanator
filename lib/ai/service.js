...
import { classifyIntent, getIntentWorkflow, getIntentSystemAddendum, shouldUsePlanMode, resolveTaskMode, classifyRequestMode, isSimpleFrontendEdit, findMainPagePath, isProceedSignal } from './intents.js'
...
export class AIService {
  ...
  async *processMessageStream({ projectId, chatId, userMessage, userId, scope: requestedScope, designPrefs, executePlan, attachments, selfEditTarget }) {
    ...
    try {
      ...
      if (effectiveScope === 'project' && projectId && isSimpleFrontendEdit(userMessage)) {
        ...
        directEditMode = true
        console.log('[DirectEdit]', JSON.stringify({ detected: true, target: directEditTarget, fileCount: earlyIndex.length }))
        yield { event: 'status', data: { stage: 'direct_edit', detail: `Direct edit mode → ${directEditTarget}` } }
      }

      // 6b. Project Manager conversational mode
      // For build intents: unless the user explicitly says "go ahead", respond conversationally first.
      // This makes Emanator act as a project manager who discusses before building.
      let projectManagerMode = false
      if (intent === 'build' && effectiveScope === 'project' && !directEditMode && !isProceedSignal(userMessage)) {
        projectManagerMode = true
        console.log('[ProjectManager] Conversational mode — user has not given proceed signal')
      }
...
- The plan should feel actionable and customizable — the user should feel they can say "change step 2" or "skip pricing".`
      ...
      if (directEditMode && directEditTarget) {
        ...
        const fileAction = (await db.projectFiles.findByPath(projectId, directEditTarget)) ? 'update_files' : 'create_files'
...
The user will review and approve, revise, or cancel your plan before any files are changed.`
            ...
            try {
              ...
              if (toolName === 'propose_plan') {
                ...
                if (directEditMode) {
                  ...
                  if (!fullContent) {
                    const fileList = savedFiles.map(f => `- \`${f.path}\` (${f.action})`).join('\n')
                    fullContent = `## Page Updated\n\n${fileList}\n\nCheck the **Preview** tab to see your changes.`
                    yield { event: 'token', data: { content: fullContent } }
                  }
                  continue
                }
      ...
      if (!usePlanMode && !proposedPlan && diffFiles.length === 0 && generatedFiles.length === 0 && toolMode !== 'chat_only' && requestMode !== 'plan_only' && requestMode !== 'read_only_report') {
