  ...
  async *processMessageStream({ projectId, chatId, userMessage, userId, scope: requestedScope, designPrefs, executePlan, attachments, selfEditTarget }) {
    ...
    try {
      ...
      if (requestMode === 'apply_pending_diff' && pendingDiffMessage) {
        ...
        // Check for continuation: if plan has next_steps and apply succeeded and verification passed
        const nextStep = (!results.rolledBack && verificationPassed && planData?.next_steps?.length > 0) ? planData.next_steps[0] : null
        const remainingSteps = (!results.rolledBack && verificationPassed && planData?.next_steps?.length > 1) ? planData.next_steps.slice(1) : []
