    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (isTransient && attempt < MAX_RETRIES) {
          ...
          console.warn(`[StreamRetry] Attempt ${attempt + 1} failed (${msg.slice(0, 120)}), retrying in ${delay}ms...`)
        ...
        if (attempt > 0) {
          console.error(`[StreamRetry] All ${attempt + 1} attempts failed: ${msg.slice(0, 120)}`)
        }
  ...
  async *processMessageStream({ projectId, chatId, userMessage, userId, scope: requestedScope, designPrefs, executePlan, attachments, selfEditTarget }) {
    ...
    try {
      ...
      if (requestMode === 'apply_pending_diff' && pendingDiffMessage) {
        ...
        // Check for continuation: from explicit next_steps or synthesized remaining steps
        const effectiveSteps = (planData?.next_steps?.length > 0) ? planData.next_steps : synthesizedSteps
        const nextStep = (!results.rolledBack && verificationPassed && effectiveSteps.length > 0) ? effectiveSteps[0] : null
        const remainingSteps = (!results.rolledBack && verificationPassed && effectiveSteps.length > 1) ? effectiveSteps.slice(1) : []

        yield { event: 'done', data: { content, toolMode: 'apply_pending_diff', scope: requestedScope || 'project', runId, provider: this.providerName, model: this.modelName, appliedFiles: results.written, deletedFiles: results.deleted, rolledBack: results.rolledBack || false, planData: planData || null, continuation: nextStep ? { nextStep, remainingSteps, originalTask: planData?.summary || '' } : null } }
        return
      }
                ...
                if (!validationResult.valid) {
                  ...
                  if (revPlanCall) {
                    try {
                      ...
                      fullContent = this.formatProposedPlanResponse(revArgs)
                      yield { event: 'token', data: { content: fullContent } }
                    } catch (parseErr) {
                    ...
                    if (critiqueContent.trim().toLowerCase().startsWith('issues found: yes')) {
                      ...
                      if (revisedPlanCall) {
                        try {
                          ...
                          fullContent = this.formatProposedPlanResponse(revisedArgs)
                          yield { event: 'token', data: { content: fullContent } }
                        } catch (parseErr) {
