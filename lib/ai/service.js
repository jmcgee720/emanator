...
export class AIService {
  ...
  _apiKey(provider) {
    ...
    const preferProxy = String(process.env.PREFER_EMERGENT_PROXY || '1') !== '0'

    // Routing priority:
    // 1. If the Emergent universal proxy is available AND PREFER_EMERGENT_PROXY
    //    isn't explicitly disabled, use the proxy. This avoids falling back
    //    to direct keys with depleted balances in shared / dev environments.
    // 2. Otherwise fall back to a direct provider key.
    // 3. Last resort: the universal key without proxy (may still work for
    //    OpenAI which accepts it directly, but not for Anthropic/Gemini).
    if (preferProxy && emergentKey && proxyUrl) {
