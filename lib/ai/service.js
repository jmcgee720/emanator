...
export class AIService {
  ...
  static FALLBACK_MAP = {
    openai: { provider: 'anthropic', model: null },
    anthropic: { provider: 'openai', model: null },
  }
  ...
  /**
   * Switch to fallback provider. Returns true if switched, false if no fallback available.
   */
  _switchToFallback() {
    if (this._fellBack) return false
    const fb = AIService.FALLBACK_MAP[this.providerName]
    if (!fb) return false
    const fbKey = this._apiKey(fb.provider)
    if (!fbKey) return false
    this.providerName = fb.provider
    this.modelName = fb.model || this._defaultModel(fb.provider)
    const opts = this._proxyOptions()
    this.provider = createProvider(fb.provider, fbKey, this.modelName, opts)
    this._fellBack = true
    console.log(`[AI FALLBACK] Switched to ${this.providerName}/${this.modelName}`)
    return true
  }

  /**
   * Wrap a streaming call with automatic provider fallback on rate-limit errors.
   * makeStream: () => AsyncIterable — factory that uses this.provider (re-evaluated after switch)
   * Yields a special { type: 'status' } chunk on fallback switch so callers can surface it.
   */
  async *_streamWithFallback(makeStream) {
    try {
      ...
      // Reset rate-limit escalation counter on successful stream completion
      ...
      if (isRateLimit && this._switchToFallback()) {
        // Emit a status chunk so the caller can propagate to UI
        yield { type: 'status', stage: 'provider_fallback', detail: `Rate-limited — switching to ${this.providerName}…` }
        for await (const chunk of makeStream()) {
          yield chunk
        }
      } else {
  ...
  async *processMessageStream({ projectId, chatId, userMessage, userId, scope: requestedScope, designPrefs, executePlan, attachments, selfEditTarget }) {
    ...
    if (executePlan) {
      yield* this.executePlanStream({ projectId, chatId, userMessage, userId, scope: requestedScope, designPrefs, planData: executePlan, runId, startTime })
      return
    }
    ...
    try {
      ...
      const toolMode = requestMode === 'read_only_report'
        ...
        : (effectiveScope !== 'project'
          ? 'chat_only'
          : (workflow.toolMode === 'chat_only' ? 'chat_only' : detectToolMode(userMessage)))
      ...
      if (requestMode === 'read_only_report' && projectId) {
        ...
        // Check if the REQUESTED file was found (not just any file in the project)
        const requestedFilesLoaded = requestedFileFound || directReadFiles.length > 0
        // Also check if fsContext loaded the specifically requested file
        const fsContextHasRequestedFile = candidates?.length > 0 && (fsContext?.relevantFiles || []).some(f => {
...
5. Provide a thorough analysis: structure, key functions, exports, dependencies, patterns, and any issues you notice.`
          ...
          // Requested file NOT found — be explicit and helpful
...
The user will review and approve, revise, or cancel your plan before any files are changed.`
            ...
            try {
              ...
              if (toolName === 'propose_plan') {
                ...
                if (groundedFileContext) {
                  validationResult = validatePlan(args, groundedFileContext, lastRejectedPlanHash, userMessage)
                ...
                if (!validationResult) {
                  validationResult = {
                    valid: true,
                    errors: [],
                    warnings: ['No existing files loaded; validation ran in empty-project mode'],
                    hash: hashPlan(args),
                    mode: 'empty_project',
                  }
                }
      ...
      yield {
        ...
        data: {
          ...
          scope: effectiveScope, intent, runId,
          provider: this.providerName, model: this.modelName,
          canvasUpdated,
          fsStats: fsContext ? { scanned: fsContext.scannedCount, read: fsContext.readCount, matched: fsContext.matchedCount } : null
        }
      }
    } catch (error) {
      ...
      await this.logGenerationRun({
        id: runId, project_id: projectId, chat_id: chatId, user_id: userId,
        tool_mode: 'error', files_generated: 0, duration: Date.now() - startTime,
        success: false, error: error.message, provider: this.providerName, model: this.modelName,
      })
   ...
   */
  async *executePlanStream({ projectId, chatId, userMessage, userId, scope, designPrefs, planData, runId, startTime }) {
    try {
...
Execute ALL file actions listed above. Use create_files for new files, update_files for modifications, and delete_files for removals. Generate complete, production-ready code.`
  ...
  async processMessage({ projectId, chatId, userMessage, userId, scope: requestedScope }) {
    ...
    try {
      ...
      // 5. Determine tool mode from workflow (or fallback to existing detection)
  ...
  async loadContext(projectId, chatId) {
    const [project, chat, messages, files, canvasDoc] = await Promise.all([
  ...
  async *processImageGeneration({ projectId, chatId, userMessage, userId, intent, workflow, runId, startTime }) {
    try {
      // Hard guard: block image generation if prompt contains code/architecture signals
      ...
      if (codeSignals.test(userMessage)) {
        console.warn('[AIService] Image generation blocked — code/architecture prompt detected')
