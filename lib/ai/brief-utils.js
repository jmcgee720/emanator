...
export function buildAssetsFileContent(imageAssets) {
  ...
  // Maintain running indices per role for PHOTO_N / ILLUSTRATION_N.
  let photoIdx = 0
  let illuIdx = 0
  const logoSeen = { v: false }
  const heroSeen = { v: false }

  const lines = []
  for (const a of renderable) {
    let exportName
    if (a.role === 'logo' && !logoSeen.v) { exportName = 'LOGO_URL'; logoSeen.v = true }
    else if (a.role === 'hero' && !heroSeen.v) { exportName = 'HERO_URL'; heroSeen.v = true }
    else if (a.role === 'photo' || a.role === 'logo' || a.role === 'hero') { exportName = `PHOTO_${photoIdx++}` }
    ...
    lines.push(`${note}export const ${exportName} = \`${safe}\``)
  }

  return `// AUTO-GENERATED by Emanator from user-uploaded brand assets.
