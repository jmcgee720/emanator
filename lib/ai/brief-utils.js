...
export function buildAssetsFileContent(imageAssets) {
  ...
  for (const a of renderable) {
    let exportName
    let vfsPath
    if (a.role === 'logo' && !logoSeen.v) {
      exportName = 'LOGO_URL'; logoSeen.v = true
      vfsPath = '/logo.png'
    } else if (a.role === 'hero' && !heroSeen.v) {
      exportName = 'HERO_URL'; heroSeen.v = true
      vfsPath = '/hero.jpg'
    ...
    vfsEntries.push(`  '${vfsPath}': ${exportName},`)
  }
