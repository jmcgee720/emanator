...
export class ImageService {
  constructor() {
    // Direct OpenAI key — required for gpt-image-1 / DALL-E.
    ...
    if (!apiKey) throw new ProviderError({
      ...
      model: 'gpt-image-1',
      status_code: 500,
      raw_error: 'OPENAI_API_KEY not configured for image generation',
      user_message: 'Image generation is temporarily unavailable. Please try again later.',
    })
    this.provider = new OpenAIProvider(apiKey, null, {})
   ...
   * provider. Uses the direct Gemini API key.
   */
  _getGeminiProvider() {
    ...
    if (!apiKey) throw new ProviderError({
      ...
      status_code: 500, raw_error: 'GEMINI_API_KEY not configured for Nano Banana',
      user_message: 'Gemini image generation is not configured.',
    })
    this._geminiProvider = new GeminiProvider(apiKey, 'gemini-2.5-flash-image-preview', {})
