...
import { classifyProviderError, ProviderError } from './errors.js'
...
export class ImageService {
  constructor() {
    // Direct provider key is PRIMARY — bypasses proxy spending cap
    const directKey = process.env.OPENAI_API_KEY
    const emergentKey = process.env.EMERGENT_LLM_KEY
    const apiKey = directKey || emergentKey
    if (!apiKey) throw new ProviderError({
      error_type: 'auth',
      provider: 'openai',
      model: 'gpt-image-1',
      status_code: 500,
      raw_error: 'No platform API key configured for image generation',
      user_message: 'Image generation is temporarily unavailable. Please try again later.',
    })
    // Only route through proxy when using the Emergent key (no direct key available)
    const opts = (!directKey && emergentKey && process.env.EMERGENT_PROXY_URL)
      ? { baseURL: process.env.EMERGENT_PROXY_URL }
      : {}
    this.provider = new OpenAIProvider(apiKey, null, opts)

    // Lazy Gemini (Nano Banana) provider — constructed on first request.
    // Using EMERGENT_LLM_KEY for proxy access OR a direct GEMINI_API_KEY.
    this._geminiProvider = null
  }

  /**
   * Lazily-constructed Nano Banana (`gemini-2.5-flash-image-preview`)
   * provider. Reuses the same env-key + proxy convention as the
   * primary text provider so users on the universal key get image
   * generation for free.
   */
  _getGeminiProvider() {
    if (this._geminiProvider) return this._geminiProvider
    const directKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY
    const emergentKey = process.env.EMERGENT_LLM_KEY
    const apiKey = directKey || emergentKey
    if (!apiKey) throw new ProviderError({
      error_type: 'auth', provider: 'gemini', model: 'gemini-2.5-flash-image-preview',
      status_code: 500, raw_error: 'No API key configured for Nano Banana',
      user_message: 'Gemini image generation is not configured.',
    })
    const opts = (!directKey && emergentKey && process.env.EMERGENT_PROXY_URL)
      ? { baseURL: process.env.EMERGENT_PROXY_URL }
      : {}
    this._geminiProvider = new GeminiProvider(apiKey, 'gemini-2.5-flash-image-preview', opts)
   ...
   */
  async generate({ projectId, prompt, mode, spriteOpts, size, userId, chatId, variation, imageProvider, referenceImages }) {
    const result = { asset: null }
    for await (const evt of this.generateWithProgress({ projectId, prompt, mode, spriteOpts, size, userId, chatId, variation, imageProvider, referenceImages })) {
   ...
   */
  async *generateWithProgress({ projectId, prompt, mode, spriteOpts, size, userId, chatId, variation, imageProvider, referenceImages }) {
    ...
    // Pick the image provider. Defaults to OpenAI gpt-image-1; explicit
    // `imageProvider: 'gemini'` or `GEMINI_MODEL_IMAGE` env selects Nano
    // Banana. Nano Banana is preferred when the user supplies reference
    // images because it supports native image-to-image editing.
    const wantsGemini = imageProvider === 'gemini' || imageProvider === 'nano-banana'
      || (process.env.DEFAULT_IMAGE_PROVIDER === 'gemini')

    yield { type: 'stage', stage: 'generating', progress: 20, label: wantsGemini ? 'Generating with Nano Banana' : 'Generating image' }

    let result
    try {
      if (wantsGemini) {
        const gemini = this._getGeminiProvider()
        result = await gemini.generateImage(finalPrompt, {
          reference_images: referenceImages || [],
        })
      } else {
        result = await this.provider.generateImage(finalPrompt, {
          size: validatedSize,
          quality: 'auto',
        })
      }
      ...
      throw classifyProviderError(err, wantsGemini ? 'gemini' : 'openai', wantsGemini ? 'gemini-2.5-flash-image-preview' : 'gpt-image-1')
    }
    ...
    const imageDataUrl = result.b64_json
      ? `data:${result.mimeType || 'image/png'};base64,${result.b64_json}`
      : result.url
