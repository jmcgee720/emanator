...
export async function* runPhasedPipeline(ctx) {
  const { projectId, chatId, userId, runId, db, imageMode } = ctx
  ...
  try {
    ...
    const imageryDeferred = phaseState.results.images?.deferred === true
    yield { event: 'complete', data: { files_generated: filesSaved, duration: Date.now() - pipelineStart, imagery_deferred: imageryDeferred } }

    // Stamp the project so the UI can show the "Generate brand imagery"
    // CTA when the first build deferred imagery. Best-effort — we don't
    // fail the build if the settings update hits an issue.
    try {
      const projectsCol = db.projects || (await import('../../supabase/db.js')).db.projects
      const cur = await projectsCol.findById(projectId)
      const settings = { ...(cur?.settings || {}) }
      settings.imagery_status = imageryDeferred ? 'deferred' : 'generated'
      settings.imagery_status_at = new Date().toISOString()
      // Fullstack apps need the server-side preview engine to actually
      // execute API routes. Without this hint, PreviewTab would default
      // to Babel mode and the dashboard would 404 on its own /api calls.
      const planArchetype = phaseState.results.plan?.archetype
      if (planArchetype === 'fullstack_app') {
        settings.preview_engine_hint = 'server'
        settings.archetype = 'fullstack_app'
      }
      await projectsCol.update(projectId, { settings })
    } catch (err) {
      console.warn('[PhasedPipeline] could not stamp imagery_status:', err.message)
    }
