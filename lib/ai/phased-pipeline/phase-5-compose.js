...
`
  ...
  try {
    console.log('[Compose] Starting streaming compose call — 8k max_tokens, tool=create_files')
    ...
    // Final fallback: non-streaming API call. Useful when streaming dies
    // silently (happens with some Anthropic SDK combos on complex prompts).
    if (rawFiles.length === 0 && provider.chatWithTools) {
      console.warn('[Compose] attempts 1+2 produced 0 files — falling back to non-streaming chatWithTools')
      try {
        ...
        for (const tc of resp.tool_calls || []) {
          try {
            ...
            console.error('[Compose:attempt-3] tool args parse failed:', err.message)
          }
        }
        console.log(`[Compose:attempt-3] non-streaming done — files=${rawFiles.length}`)
      } catch (err) {
        console.error('[Compose:attempt-3] non-streaming call failed:', err.message)
      }
    }
  } catch (err) {
