...
export async function* runPhaseCompose(ctx) {
  ...
  yield { event: 'status', data: { stage: 'compose', detail: `Writing ${filesToWrite.length} files (one at a time for reliability)...` } }
...
`
  ...
  for (let i = 0; i < filesToWrite.length; i++) {
    const filePath = filesToWrite[i]
    const isPage = /^app\/.*page\.(jsx|tsx)$/.test(filePath) || /^app\/page\.(jsx|tsx)$/.test(filePath) || filePath === 'app/page.jsx'
    const minLines = isPage ? 300 : 60

    yield { event: 'file_start', data: { path: filePath, idx: i + 1, total: filesToWrite.length } }
...
Minimum length: ${minLines} lines of JSX.
...
Call the \`write_file\` tool ONCE with this file's full content. Do NOT print JSON as text. Do NOT explain.`,
    ...
    // Two attempts per file: streaming first, then non-streaming fallback.
    ...
    if (!fileContent) {
      ...
      yield { event: 'file_failed', data: { path: filePath, error: lastError?.message || 'unknown' } }
      continue
    }
    ...
    try {
      ...
      yield { event: 'file_saved', data: { path: filePath, bytes: resolved.length } }
    } catch (err) {
