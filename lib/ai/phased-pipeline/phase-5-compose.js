...
Call the \`write_file\` tool ONCE with this file's full content. Do NOT print JSON as text. Do NOT explain.`,
  ...
  for (const f of scaffoldFiles) {
    ...
    if (existing) {
      // Special case: package.json — merge required deps/scripts even
      // when one already exists. Without this, a Claude-generated
      // package.json missing the Tailwind trio leaves globals.css
      // unprocessable → "Unexpected character '@'" build error.
      if (f.path === 'package.json') {
        try {
          ...
          console.warn(`[Compose] could not heal existing package.json (${err.message}) — leaving as-is`)
        }
      }
      scaffoldSkipped++
      continue
  ...
  console.log(`[Compose] scaffolding pass: wrote ${scaffoldWritten}, healed ${scaffoldHealed}, skipped ${scaffoldSkipped} (LLM-generated)`)
