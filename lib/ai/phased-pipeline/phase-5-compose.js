...
`
  ...
  let rawFiles = []
  try {
    // Anthropic's API rejects requests where the estimated duration exceeds
    // 10 minutes. Their estimate is derived from max_tokens + model speed.
    // We keep max_tokens at 12k (safely under the threshold for Sonnet) and
    // use the streaming transport which also avoids client-side timeouts.
    console.log('[Compose] Calling provider.chatWithToolsStream — streaming transport, max_tokens=12000')
    const stream = provider.chatWithToolsStream(messages, COMPOSE_TOOL, {
      temperature: 0.6,
      max_tokens: 12_000,
