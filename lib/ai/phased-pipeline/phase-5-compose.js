...
export async function* runPhaseCompose(ctx) {
  ...
  // Pull the framework-paths contract eagerly so we can both filter
  // Claude's `plan.files` and force-overwrite later. Single source of truth.
  const { FRAMEWORK_PATHS, FORCE_OVERWRITE_PATHS } = await import('./scaffolding.js')
  const frameworkSet = new Set(FRAMEWORK_PATHS)

  // LAYER 1: strip framework infrastructure from the LLM's authoring
  // surface. Claude is great at React/JSX but consistently produces
  // half-broken postcss.config.js, junk-value globals.css `:root`
  // blocks, and package.json files missing devDependencies. Those are
  // not creative work — they are framework boilerplate, written
  // deterministically by the scaffolding pass below. Pre-emptively
  // dropping them here also saves N LLM calls + ~2k tokens each.
  const rawPlanFiles = Array.isArray(plan.files) ? plan.files : []
  const filesToWrite = rawPlanFiles.filter((p) => !frameworkSet.has(p))
  const droppedFrameworkFiles = rawPlanFiles.filter((p) => frameworkSet.has(p))
  if (droppedFrameworkFiles.length > 0) {
    console.log(`[Compose] dropped ${droppedFrameworkFiles.length} framework files from Claude's plan (deterministic scaffolding owns them): ${droppedFrameworkFiles.join(', ')}`)
  }
  if (filesToWrite.length === 0) {
    throw new Error('Compose phase has no files to write — plan.files contained only framework boilerplate')
  }
...
Call the \`write_file\` tool ONCE with this file's full content. Do NOT print JSON as text. Do NOT explain.`,
    ...
    for (let attempt = 1; attempt <= 2 && !fileContent; attempt++) {
      try {
        if (attempt === 1) {
          fileContent = await withOverloadedRetry(
            () => streamSingleFile(provider, fileMessages, filePath),
            filePath,
          )
        } else {
          fileContent = await withOverloadedRetry(
            () => nonStreamSingleFile(provider, fileMessages, filePath),
            filePath,
          )
        }
      } catch (err) {
  ...
  for (const s of settled) {
    ...
    try {
      ...
      // Per-file event so the wizard / chat UI can show real-time
      // progress instead of a 30s mystery wait. Also lets tests assert
      // the per-file save contract.
      yield { event: 'file_saved', data: { path: filePath, size: resolved.length } }
    } catch (err) {
      console.error(`[Compose] failed to save ${filePath}:`, err.message)
      failed.push({ path: filePath, error: `save failed: ${err.message}` })
      yield { event: 'file_failed', data: { path: filePath, error: err.message } }
    }
  }
  ...
  // ─── Scaffolding pass ────────────────────────────────────────────
  // Write the Next.js + Tailwind framework boilerplate (package.json
  // with scripts.dev, layout.jsx, globals.css, tailwind/postcss/next
  // configs) that Claude is never asked to generate. Without these
  // the project's 67 hand-written JSX files have nothing to mount
  // against — the preview-runner correctly errors with "no
  // package.json with a dev/start script found anywhere in /project".
  //
  // We only write files that don't already exist so any Claude-
  // generated app/layout.jsx etc. wins. saveFile already does an
  // existence check via findByPath → that's the precedence guard.
  // ─── Scaffolding pass ────────────────────────────────────────────
  // LAYER 2: framework files are authoritative — we force-overwrite
  // them every build regardless of what Claude (or a previous run)
  // wrote. package.json is the one exception: it gets MERGED (preserves
  // user-added deps like framer-motion) rather than overwritten.
  // Non-framework scaffold files (jsconfig.json, .gitignore) are written
  // only when missing — they're harmless to skip if a project already
  // has them.
  yield { event: 'status', data: { stage: 'compose', detail: 'Writing framework scaffolding…' } }
  const { buildScaffolding, mergeRequiredPackageDeps } = await import('./scaffolding.js')
  const forceOverwriteSet = new Set(FORCE_OVERWRITE_PATHS)
  const scaffoldFiles = buildScaffolding({
    projectName: plan.brand?.name || 'auroraly-project',
    tokens,
    fullstack: plan.archetype === 'fullstack_app',
  })
  let scaffoldWritten = 0
  let scaffoldOverwritten = 0
  let scaffoldSkipped = 0
  let scaffoldHealed = 0
  for (const f of scaffoldFiles) {
    const existing = await db.projectFiles.findByPath(projectId, f.path)

    // package.json → MERGE deps + scripts, preserve user customizations
    if (f.path === 'package.json' && existing) {
      try {
        const parsed = JSON.parse(existing.content || '{}')
        const { pkg, changed } = mergeRequiredPackageDeps(parsed, {
          fullstack: plan.archetype === 'fullstack_app',
        })
        if (changed) {
          ...
          scaffoldHealed++
          saved.push({ path: f.path, healed: true })
          yield { event: 'file_saved', data: { path: f.path, healed: true } }
        } else {
          scaffoldSkipped++
        }
        continue
      } catch (err) {
        console.warn(`[Compose] could not heal existing package.json (${err.message}) — overwriting`)
        // Fall through to overwrite below.
      }
    }

    // Framework infra (postcss/tailwind/next config, globals.css,
    // layout.jsx) → FORCE overwrite. Claude is not allowed to author
    // these any more.
    if (forceOverwriteSet.has(f.path) && existing) {
      ...
      scaffoldOverwritten++
      saved.push({ path: f.path, overwritten: true })
      yield { event: 'file_saved', data: { path: f.path, overwritten: true } }
      continue
    }

    // Everything else (jsconfig, .gitignore, and any missing infra) →
    // create-if-absent.
    if (existing) { scaffoldSkipped++; continue }
    ...
    scaffoldWritten++
    saved.push({ path: f.path, scaffolded: true })
    yield { event: 'file_saved', data: { path: f.path, scaffolded: true } }
  }
  console.log(`[Compose] scaffolding pass: wrote ${scaffoldWritten}, overwrote ${scaffoldOverwritten}, healed ${scaffoldHealed}, skipped ${scaffoldSkipped}`)

  return {
    files: saved,
    failed: failed.length > 0 ? failed : undefined,
    scaffolded: scaffoldWritten,
...
/**
 * Detect Anthropic 529 overloaded / transient 5xx errors. We classify by
 * status code attached to provider errors AND by message-text matching as
 * a backstop because not every error path in our provider clients carries
 * a structured status.
 */
function isOverloadedError(err) {
  ...
  return msg.includes('529') || msg.includes('overloaded') || msg.includes('overloaded_error') || msg.includes('service unavailable')
}

/**
 * Retry an async operation with exponential backoff when the provider
 * returns 529 / overloaded. Caps at 3 retries with 1s → 2s → 4s waits
 * plus up to 500ms jitter. Non-overloaded errors short-circuit immediately
 * so we don't waste retries on parse/format failures.
 */
async function withOverloadedRetry(fn, label, maxRetries = 3) {
  let lastErr = null
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`[Compose:${label}] 529/overloaded — retry ${attempt + 1}/${maxRetries} in ${delay}ms (${err.message})`)
