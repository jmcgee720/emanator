...
export async function* runPhaseImages(ctx) {
  const { geminiProvider, openaiImageProvider, priorResults, attachments, imageMode } = ctx
  ...
  // Lever 2: deferred imagery. First-build path skips Nano Banana / OpenAI
  // entirely and lets Phase 5 fall back to its built-in SVG/gradient
  // placeholders (see phase-5-compose.js line ~100). The user can later
  // hit `POST /api/projects/<id>/imagery/generate` to run images + recompose.
  // This drops first-paint payload from ~240MB to ~500KB and lets the
  // user iterate on copy/structure before paying the imagery credit cost.
  if (imageMode === 'defer') {
    yield {
      event: 'status',
      data: {
        stage: 'images',
        detail: `Skipping imagery for now — you'll add it on the next step. ${plan.imageManifest.length} image slots will use placeholders.`,
      },
    }
    return { images: [], deferred: true, _ms: Date.now() - phaseStart }
  }
