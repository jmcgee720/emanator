...
async function generateOneImage({ placement, refImages, geminiProvider, openaiImageProvider, plan, genErrors }) {
  ...
  // Attempt 2: OpenAI gpt-image-1
  if (openaiImageProvider) {
    try {
      const res = await openaiImageProvider.generateImage(placement.prompt, {
      ...
      if (res?.b64_json) {
        return {
          role: placement.role,
          dataUrl: `data:image/png;base64,${res.b64_json}`,
          subject: placement.prompt,
          source: 'openai_image',
        }
      }
      ...
      genErrors.push({ role: placement.role, provider: 'openai', message: 'no_image_in_response' })
    } catch (err) {
      const msg = err?.user_message || err?.raw_error || err?.message || String(err)
      genErrors.push({ role: placement.role, provider: 'openai', message: msg })
      console.error(`[PhaseImages] ${placement.role} openai failed:`, msg)
    }
  }
