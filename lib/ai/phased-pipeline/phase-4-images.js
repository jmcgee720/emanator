...
export async function* runPhaseImages(ctx) {
  ...
  // Collect error messages so we can surface them to the UI (so user knows
  // WHY their images are generic stock photos instead of custom-generated).
  const genErrors = []
  ...
  const perImage = prompts.map((p) =>
    (canGenerate
      ...
      : Promise.reject(new Error('gemini_provider_not_configured'))
    ).catch((err) => {
      // Surface the real error — not just 'using stock fallback'.
      const msg = err?.user_message || err?.raw_error || err?.message || String(err)
      genErrors.push({ role: p.role, message: msg })
      console.error(`[PhaseImages] ${p.role} gen failed:`, msg)
