...
import { githubReader } from '@/lib/ai/github-reader'
import { githubWriter } from '@/lib/ai/github-writer'
...
export async function createSnapshot(name = null, metadata = {}) {
  ...
  try {
    // Read all files from the GitHub repo
    const allFiles = await githubReader.listAllFiles()
    ...
    for (let i = 0; i < allFiles.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.allSettled(
        batch.map(async (file) => {
          try {
            const content = await githubReader.readFile(file.path)
            return {
              path: file.path,
              content,
              size: content.length,
            ...
            console.warn('[TimeMachine] Failed to read', file.path, ':', err.message)
            return null
          }
        })
      )
    ...
    let commitHash = null
    try {
      commitHash = await githubWriter.getCurrentCommitHash()
    } catch (err) {
  ...
  try {
    ...
    for (let i = 0; i < filesSnapshot.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map(async (file) => {
          try {
            await githubWriter.writeFile(file.path, file.content, `[TimeMachine] Revert to snapshot: ${snapshot.name}`)
...
export async function diffSnapshot(snapshotId) {
  try {
    ...
    const currentFiles = await githubReader.listAllFiles()
    ...
    for (const file of currentFiles) {
      const snapshotContent = snapshotFiles.get(file.path)
      
      if (!snapshotContent) {
        added.push(file.path)
      } else {
        try {
          const currentContent = await githubReader.readFile(file.path)
          if (currentContent !== snapshotContent) {
            modified.push(file.path)
          }
        } catch (err) {
          console.warn('[TimeMachine] Could not read', file.path, 'for diff:', err.message)
        }
      }
    }
