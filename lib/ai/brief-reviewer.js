...
//
// This is what makes Auroraly "feel agentic" — it notices its own
...
A flow is "wired" only if ALL of these are true:
  ...
  10. Brand copy discipline: scan landing/hero content. If the H1 or subhead reads as generic SaaS boilerplate (e.g. "Welcome to our platform", "The best solution for your needs", "Build amazing things", "Get started today", "Lorem ipsum", "Feature 1/2/3", "Fast · Secure · Scalable" without domain specifics, or untouched recipe defaults) AND the plan has a non-empty brand.description, flag as "<path>: generic-marketing-copy" so the repair wave rewrites it in the brand's own tone.
  11. Theme-token discipline: scan each generated page/component. If it uses hardcoded color classes like \`text-white\`, \`bg-white\`, \`bg-black\`, \`border-white/10\`, \`from-violet-500\`, \`to-indigo-500\`, \`bg-gray-900\`, or any \`bg-<named>-<shade>\` Tailwind color utility for brand-facing UI (navbar, hero, CTAs, cards), flag as "<path>: hardcoded-color-classes-bypass-theme". The codebase ships with components/theme.js exposing CSS variables; all brand-facing UI MUST use the arbitrary-value form (\`bg-[var(--primary)]\`, \`text-[var(--ink)]\`, etc.).
...
Emit ONLY the files that need to be added or changed. Do not touch untouched files.`
  ...
  // Attach the user's reference images to the repair prompt too — if the
  // review flagged "ignored-user-logo" / "generic-marketing-copy", the
  // repair LLM needs to SEE the reference, not a paraphrase of it. Cap at 2.
  ...
  const repairUserContent = referenceImages.length > 0
    ? [
        { type: 'text', text: userPrompt.slice(0, 12000) + '\n\nReference image(s) attached — use them to guide palette/layout/typography decisions in the repair.' },
