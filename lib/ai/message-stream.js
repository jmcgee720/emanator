...
- Keep the response focused on the configuration request only`
      ...
      if (effectiveScope === 'project' && !selfEditTarget?.path && (taskMode === 'build' || taskMode === 'refine_page' || taskMode === 'edit')) {
      ...
      if (selfEditTarget?.path) {
        ...
        try {
          ...
          if (existsSync(targetPath)) {
            ...
            console.warn('[SelfEdit] Target file not found:', targetPath)
          }
        } catch (err) {
...
## YOUR ROLE
You modify a SINGLE existing source file when the user describes improvements. You do NOT create new files, build websites, generate UI components, or produce previews.
...
## RULES (STRICT)
1. Call the \`update_files\` tool with path \`${selfEditTarget.path}\` containing the COMPLETE updated file.
2. Read the file contents above CAREFULLY. Understand the existing code structure before changing anything.
3. Make TARGETED edits — only add or modify what the user asked for. Do NOT rewrite from scratch.
4. NEVER create new files. NEVER generate UI components. NEVER build websites or apps. Your ONLY job is editing \`${selfEditTarget.path}\`.
5. Preserve ALL existing functionality. Only add/modify the specific behavior the user described.
6. If the user's request is vague, add the improvement as new code/comments in the most logical location within the existing file structure.
7. Respond with a brief explanation of what you changed and why, then call the tool.`
      ...
      if (intentAddendum && !selfEditTarget?.path) systemMessage += '\n\n' + intentAddendum

      // ── Grounding Injection: project identity + strict file index ──
      if (effectiveScope === 'project' && projectId && !selfEditTarget?.path) {
      ...
      if (fsContext && !selfEditTarget?.path) {
