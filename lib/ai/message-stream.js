...
import { handleReadFiles, handleVerifyBuild, handleExecCommand, handleEditLines } from '../e2b/agent-tools.js'
...
2. Call \`edit_lines\` with the line numbers you want to change
...
4. If it fails: read the error carefully, call \`read_files\` again to see the reverted file, then try a SMALLER edit that doesn't break JSX structure.
...
4. If verification fails: READ THE ERROR, call \`read_files\` again, then fix with another \`edit_lines\` call. You MUST retry — do not give up after one failed edit.
...
- After your create_files call, follow up with a short conversational summary of what you built. Do NOT ask questions.`
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        if (isSelfEdit && !selfEditIsConversational) {
          effectiveToolSet = AI_TOOLS.filter(t => ['patch_files', 'update_files', 'update_canvas', 'read_files', 'verify_build', 'exec_command', 'update_memory', 'screenshot_verify', 'edit_lines'].includes(t.function?.name))
        }
          ...
          for (const toolCall of toolCalls) {
            try {
              ...
              if (toolName === 'propose_plan') {
                ...
                if (directEditMode || isSelfEdit || (isBriefBuild && existingFiles === 0)) {
                  ...
                  if (isSelfEdit && savedFiles.length > 0) {
                    ...
                    if (buildPassed) {
                      ...
                      // BUILD FAILED — feed error back to AI for retry
                      fullContent = `Updated **${shortFiles}** — ${summaryText}\nBUILD FAILED — changes were reverted.\n\nNote: Build check returned a warning. Click Apply to Live to test the changes.`
                      yield { event: 'replace_content', data: { content: fullContent } }

                      // Inject retry context into messages so agent loop retries
                      const retryMsg = `[SYSTEM: Your edit to ${shortFiles} caused a BUILD ERROR and was auto-reverted. The app is safe.\n\nCompilation error:\n\`\`\`\n${buildError || 'Unknown error — check the file for syntax issues'}\n\`\`\`\n\nDo NOT give up. You MUST fix this:\n1. Call \`read_files\` to see the current (working) version of the file\n2. Look at the error above to understand what went wrong\n3. Call \`edit_lines\` with a SMALLER, CORRECT edit\n4. Common issues: missing closing tags, wrong indentation, undefined variables, breaking JSX expression boundaries\n\nCall read_files NOW.]`
                      messages.push({ role: 'assistant', content: fullContent })
                ...
                if (agentLoopCount >= 1) {
                  messages.push({ role: 'user', content: '[SYSTEM: You have now read the file contents with line numbers. Use `edit_lines` to make changes by specifying line_start/line_end and the new content. Match the existing indentation exactly. After editing, the build is auto-verified — if it breaks, the file is reverted and you\'ll see the error. Do NOT describe what you will do — call the tool now.]' })
                }
                fullContent = ''
                toolCalls = []
                toolOpts.tool_choice = 'auto'
                ...
                // Most reliable method: AI specifies line numbers, system applies edits
                ...
                if (editResult.success) {
                  ...
                  // Auto-verify build to catch syntax errors immediately
                  if (isSelfEdit) {
                    canvasUpdated = true
                    // Wait for Next.js hot reload to process the change
                    await new Promise(r => setTimeout(r, 4000))
                    try {
                      const healthRes = await fetch('http://localhost:3000/api/health', { signal: AbortSignal.timeout(10000) })
                      if (!healthRes.ok) {
                        // Build broke — grab the actual error from logs
                        ...
                        try {
                          ...
                          const lastErrIdx = errLines.findLastIndex(l => l.includes('Error') || l.includes('Syntax') || l.includes('Module build failed') || l.includes('Expected'))
                          if (lastErrIdx >= 0) {
                            const start = Math.max(0, lastErrIdx - 5)
                            const end = Math.min(errLines.length, lastErrIdx + 10)
                            buildError = errLines.slice(start, end).join('\n')
                          }
                        } catch {}
                        // Auto-revert
                        const fs = await import('fs')
                        ...
                        editResult.errors.push(`BUILD BROKEN — your edit caused a compilation error and was auto-reverted.\n\nCompilation error:\n\`\`\`\n${buildError || 'Could not read error log'}\n\`\`\`\n\nTo fix this:\n1. Call \`read_files\` to see the current (reverted) file with line numbers\n2. Identify the problem from the error above\n3. Try a smaller, more targeted \`edit_lines\` call that preserves the existing JSX structure\n4. Call \`verify_build\` after editing to confirm it compiles`)
                      } else {
                      ...
                      await new Promise(r => setTimeout(r, 4000))
                      try {
                        const retryRes = await fetch('http://localhost:3000/api/health', { signal: AbortSignal.timeout(10000) })
                        if (!retryRes.ok) {
                          console.error(`[edit_lines] BUILD STILL BROKEN after retry — reverting ${args.path}`)
                          const fs = await import('fs')
                          const fullPath = (await import('path')).resolve('/app', args.path)
                          if (editResult.originalContent) {
                            fs.writeFileSync(fullPath, editResult.originalContent, 'utf-8')
                            editResult.success = false
                            editResult.errors.push('BUILD BROKEN after retry — edit was auto-reverted. Call read_files to see the file, then try a smaller edit.')
                          }
                        }
                      } catch {}
                    }
                ...
                const resultMsg = editResult.success
                  ? `Successfully applied ${editResult.applied} edit(s) to \`${args.path}\` (${editResult.linesBefore} → ${editResult.linesAfter} lines). ${editResult.errors.length > 0 ? `\nWarnings: ${editResult.errors.join(', ')}` : ''}\n\n${isSelfEdit ? 'File written to disk. Click Apply to Live to promote.' : 'Changes saved.'}`
