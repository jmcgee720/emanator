...
- Keep the response focused on the configuration request only`
      ...
      if (isSelfEdit) {
        ...
        const hasNoEditVerbs = !msgTrimmed.match(/\b(add|change|edit|update|remove|delete|fix|modify|replace|insert|create|refactor|rename|move|implement|build|write|patch)\b/)
        selfEditIsConversational = conversationalPatterns.some(p => p.test(msgTrimmed)) || isShortNonEdit || (msgTrimmed.startsWith('are you') && hasNoEditVerbs)
