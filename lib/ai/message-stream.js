 ...
 * Analyzes the edited file's role and content to suggest relevant follow-ups.
 */
function generateSelfEditSuggestions(savedFiles, userMessage) {
  const suggestions = []
  const paths = savedFiles.map(f => f.path || '').join(' ')
  const content = savedFiles.map(f => f.content || '').join(' ').toLowerCase()
  const msgLower = (userMessage || '').toLowerCase()

  // Prompt Builder suggestions
  if (paths.includes('prompt-builder')) {
    if (!content.includes('accessibility') && !msgLower.includes('accessibility'))
      suggestions.push('Add accessibility guidelines — ensure generated sites include ARIA labels and proper heading hierarchy')
    if (!content.includes('seo') && !msgLower.includes('seo'))
      suggestions.push('Add SEO best practices — meta tags, semantic HTML, and structured data rules')
    if (!content.includes('performance') && !msgLower.includes('performance'))
      suggestions.push('Add performance rules — lazy loading, code splitting hints, image optimization')
    if (!content.includes('mobile') && !msgLower.includes('mobile'))
      suggestions.push('Strengthen mobile-first patterns — touch targets, viewport handling, responsive breakpoints')
  }

  // Design System suggestions
  if (paths.includes('design-system')) {
    if (!content.includes('dark') && !msgLower.includes('dark'))
      suggestions.push('Add dark mode token variants for all color scales')
    if (!content.includes('animation') && !msgLower.includes('animation'))
      suggestions.push('Define animation presets — entrance, exit, hover, and loading state timing functions')
    if (!content.includes('spacing'))
      suggestions.push('Add a spacing scale system for consistent vertical rhythm')
  }

  // Image Generator suggestions
  if (paths.includes('image-prefetch') || paths.includes('image')) {
    if (!content.includes('aspect') && !msgLower.includes('aspect'))
      suggestions.push('Add aspect ratio detection — auto-pick landscape, portrait, or square based on page section')
    if (!content.includes('fallback'))
      suggestions.push('Add fallback image generation for when primary generation fails')
  }

  // Plan Validator suggestions
  if (paths.includes('plan-validator')) {
    suggestions.push('Add validation for file dependency order — imports should be resolved before files that use them')
    suggestions.push('Add a complexity score threshold — reject plans with too many simultaneous file changes')
  }

  // Service/Router suggestions
  if (paths.includes('service') || paths.includes('request_router')) {
    suggestions.push('Add intent confidence thresholds — route to clarification dialog when confidence < 0.6')
    suggestions.push('Add request pattern caching — remember successful prompt patterns for faster routing')
  }

  // Generic fallbacks
  if (suggestions.length === 0) {
    suggestions.push('Add error handling for edge cases in this file')
    suggestions.push('Add JSDoc comments to exported functions for better self-documentation')
    suggestions.push('Consider extracting complex logic into helper functions for readability')
  }
...
- After your create_files call, follow up with a short conversational summary of what you built. Do NOT ask questions.`
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        // Force the AI to use patch_files in self-edit mode
        if (isSelfEdit) {
        ...
        // Self-edit: only allow patch_files tool
        if (isSelfEdit) {
                ...
                if (directEditMode || isSelfEdit || (isBriefBuild && existingFiles === 0)) {
                  ...
                  if (savedFiles.length > 0 && !fullContent) {
                    if (isBriefBuild) {
                      ...
                      // Self-edit: generate context-aware enhancement suggestions
                      const editedPaths = savedFiles.map(f => f.path).join(', ')
                      const { text: seText } = buildVerifiedResponseWithRuntime(savedFiles, userMessage, true, true)
                      fullContent = seText
                      // Append enhancement suggestions based on what was edited
