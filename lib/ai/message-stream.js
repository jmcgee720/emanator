...
- Keep the response focused on the configuration request only`
      ...
      if (isSelfEdit) {
        ...
        systemMessage = `You are **Emanator**, an AI-powered website builder platform. You edit your own internal source code when users request improvements.
...
4. Do NOT mention "preview". Say: "Review the changes in the Code tab. Click Apply to Live when ready."
5. Do NOT say "file(s) written successfully" — the system adds its own summary.
6. **CRITICAL: READ THE USER'S MESSAGE CAREFULLY.** If they ask a pure question (e.g. "what is X?" "how does Y work?"), ANSWER it. For everything else — feature requests, improvements, explorations, implementations — CALL patch_files and make the edit. When in doubt, DO THE EDIT.
7. When the user says "proceed", "continue", "next step", "go ahead", "do it", "explore", "implement", or "keep going" — EXECUTE an edit immediately using patch_files. Do NOT respond with a plan, outline, or bullet-point list.
8. **NEVER WRITE PLANS OR OUTLINES.** You are a code editor, not a consultant. If the user asks to "explore integrations" → add integration config code to the file. If they ask to "improve performance" → add performance optimizations to the code. ALWAYS translate requests into actual code changes.
9. You are editing EMANATOR'S OWN CODEBASE (Next.js, React, Node.js). Features like "CSV export", "deploy integration" etc. are features TO ADD TO EMANATOR — not user website features.
10. Never suggest editing API routes, database logic, or authentication flows — those are outside your scope.
11. After applying patches, give a 1-line summary and end with: "Click **Apply to Live** when ready, or tell me what to do next."
12. If the user asks "is X done?" or "what's the status?" — answer based on what you know from the conversation. Do NOT randomly edit unrelated files.
`}`
...
- After your create_files call, follow up with a short conversational summary of what you built. Do NOT ask questions.`
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        // Force the AI to use patch_files in self-edit mode with 3-tier intent system:
        // 1. Conversational (greetings, identity questions) → no tools
        // 2. Status questions ("is X done?", "what happened?") → auto (AI chooses text or tool)
        // 3. Action requests (add, explore, implement) → FORCE patch_files
        if (isSelfEdit) {
          if (selfEditIsConversational) {
            console.log('[SelfEdit-Chat] Conversational mode — no tool_choice forcing')
          } else {
            const msgLower = userMessage.trim().toLowerCase()
            const isStatusQuestion = /\b(is .+ done|what happened|what.?s the status|did (it|you)|how did|was that|is that)\b/.test(msgLower)
            if (isStatusQuestion) {
              toolOpts.tool_choice = 'auto'
              console.log('[SelfEdit] Status question — tool_choice: auto')
            } else {
              toolOpts.tool_choice = { type: 'function', function: { name: 'patch_files' } }
              console.log('[SelfEdit] Action request — tool_choice: forced patch_files')
            }
          }
        }
