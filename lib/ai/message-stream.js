...
- After your create_files call, follow up with a short conversational summary of what you built. Do NOT ask questions.`
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        while (true) {
          ...
          // If the AI promised action ("I'll build that", "Let me create...") but didn't call any tool,
          // force a retry. This mirrors the Core System broken promise detector.
          if (!isSelfEdit && !directEditMode && !usePlanMode && effectiveScope === 'project' && toolCalls.length === 0 && fullContent.length > 0) {
            const promisedAction = /\b(let'?s (get started|do|implement|add|begin|proceed|start|build|work on|make|address|fix|resolve|create)|let me (implement|add|create|build|start|proceed|get started|fix|address)|i'?ll (add|implement|create|update|modify|do|build|make|start|proceed|work on|fix|address|generate)|here'?s (what|how|the)|implementing|starting|proceeding|creating|building|generating)\b/i.test(fullContent)
            if (promisedAction && selfEditPatchRetry < MAX_SELF_EDIT_RETRIES) {
              selfEditPatchRetry++
              console.log(`[BuildRetry] AI promised action but called no tool — forcing retry ${selfEditPatchRetry}/${MAX_SELF_EDIT_RETRIES}`)
              yield { event: 'status', data: { stage: 'retrying', detail: 'Executing...' } }
