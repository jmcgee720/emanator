      ...
      if (isSelfEdit && toolMode !== 'chat_only') {
        ...
        if (needsSynthesis) {
          ...
          try {
            ...
            // in assistant turns when no `tools` array is provided. We strip
            // tool_calls, drop role:'tool' messages (converting them to user
            // notes), and coalesce same-role neighbors to keep the API happy.
            const cleanForSynthesis = (msgs) => {
              const out = []
              for (const m of (msgs || [])) {
                ...
                if (m.role === 'assistant' && Array.isArray(m.tool_calls) && m.tool_calls.length > 0) {
                  const toolSummary = m.tool_calls.map(tc => '[Used tool: ' + (tc?.function?.name || 'unknown') + ']').join(' ')
