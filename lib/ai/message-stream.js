...
**WHEN TO USE WHICH:**
- If the file is < 500 lines → use \`patch_files\` with precise search/replace pairs
- If the file is > 500 lines → use \`patch_files\` with small, targeted patches (NOT update_files — you'll lose existing code)
- ONLY use \`update_files\` for NEW files that don't exist yet, or when explicitly told to replace a file
- NEVER use \`update_files\` on an existing file unless you include ALL the original code plus your changes — partial rewrites will be blocked by the regression guard
          ...
          for (const toolCall of toolCalls) {
            try {
              ...
              if (toolName === 'propose_plan') {
                ...
                if (agentLoopCount >= 1) {
                  messages.push({ role: 'user', content: '[SYSTEM: You have now read the file contents. Make the changes using update_files or patch_files. Do NOT describe what you will do — call the tool now.]' })
                }
                fullContent = ''
                toolCalls = []
                toolOpts.tool_choice = 'auto'
