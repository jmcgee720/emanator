...
HERO (two-column): \`grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-32\`. Left: pill badge → gradient headline (text-5xl md:text-7xl) → subtitle → 2 CTA buttons (contextual labels, NOT "Get Started"/"Learn More" — use "Launch Dashboard", "See How It Works" etc). Right: floating glass card mockup of the product. ${heroHeadline ? `Headline: "${heroHeadline}".` : ''} ${keyMessaging ? `Subtitle: ${keyMessaging}.` : ''}
...
- NEVER use "Get Started" or "Learn More" as button text. Use contextual labels: "Launch Dashboard", "See How It Works", "View Pricing", "Start Free Trial".
...
- Year in footer: 2026, not 2023.`
        ...
        // Add attachments context — reference images but DON'T embed base64 (too large, blows context)
        // Instead, save images to project files and reference them by path
        let artDirectionNote = ''
        if (attachments?.length > 0) {
          const imageAssets = attachments.filter(a => a.type === 'image' && a.data)
          if (imageAssets.length > 0) {
            // Save images as project files so they're available
            const savedImages = []
            for (const asset of imageAssets) {
              try {
                ...
                savedImages.push({ name: asset.name, path: imgPath, isLogo: /logo/i.test(asset.name), isHero: /hero|banner/i.test(asset.name) })
              } catch (e) { console.warn('[CreativeBrief] Failed to save image:', e.message) }
            }
            // Tell GPT about the images without embedding base64
            const logoAsset = savedImages.find(a => a.isLogo)
            const heroAsset = savedImages.find(a => a.isHero)
            artDirectionNote = `\nArt direction: User uploaded ${savedImages.length} image(s).`
            if (logoAsset) artDirectionNote += ` Use an inline SVG logo inspired by the brand name "${brandName}" — make it a unique geometric icon.`
            if (heroAsset) artDirectionNote += ` Include a visually rich hero section with decorative elements.`
