...
import { describeScreenshot } from '../e2b/screenshot-service.js'
...
2. Call \`update_files\` (or \`patch_files\` for small files <500 lines) to make changes
...
- After your create_files call, follow up with a short conversational summary of what you built. Do NOT ask questions.`
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        if (isSelfEdit && !selfEditIsConversational) {
          effectiveToolSet = AI_TOOLS.filter(t => ['patch_files', 'update_files', 'update_canvas', 'read_files', 'verify_build', 'exec_command', 'update_memory', 'screenshot_verify'].includes(t.function?.name))
        }
          ...
          for (const toolCall of toolCalls) {
            try {
              ...
              if (toolName === 'propose_plan') {
                ...
                console.log(`[AgentLoop] screenshot_verify: ${url}`)
