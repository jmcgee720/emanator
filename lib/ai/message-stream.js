        ...
        while (true) {
          ...
          if (isSelfEdit) {
            ...
            if (!selfEditIsConversational && toolCalls.length === 0 && fullContent.length > 0) {
              const promisedAction = /\b(let'?s (get started|do|implement|add|begin)|i'?ll (add|implement|create|update|modify|do)|here'?s (what|how)|implementing|starting|proceeding)\b/i.test(fullContent)
              if (promisedAction && selfEditPatchRetry < MAX_SELF_EDIT_RETRIES) {
                ...
                messages.push({ role: 'user', content: 'You said you would do it but you did not call any tool. Call patch_files NOW to make the edit. Do not respond with text.' })
