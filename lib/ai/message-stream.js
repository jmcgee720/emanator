...
- Keep the response focused on the configuration request only`
      ...
      // When the user submits a Creative Brief OR makes the first build
      // request on a fresh project, bypass ALL complex machinery
      ...
      if (!isBriefBuildEarly && taskMode === 'build' && projectId && effectiveScope === 'project') {
        try {
          const earlyFiles = await db.projectFiles.findIndexByProjectId(projectId)
          // 0 files means a brand-new project — anyone's first message is an initial scaffold
      ...
      if ((isBriefBuildEarly || isFreshBuildEarly) && projectId) {
        ...
        console.log(`[CreativeBrief-FastPath] Detected ${trigger} build — routing to new pipeline`)
