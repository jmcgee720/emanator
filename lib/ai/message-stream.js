...
**Part 1 — Brief acknowledgment** (optional, 1 sentence max)
> "Looking now."
...
- Call \`update_files\` with the COMPLETE modified file content.`
      ...
      if (isSelfEdit && toolMode !== 'chat_only') {
        ...
        // anything else.
        const danglingIntent = hasIntentVerb && !hasCompletionVerb && isTooShort
        ...
        if (needsSynthesis) {
          console.log('[SelfEdit-Synthesis] Triggering final follow-up pass:', JSON.stringify({ usedAnyTool, danglingIntent, asksUserForHelp, hasIntentVerb, hasCompletionVerb, isTooShort, fullContentLength: (fullContent || '').length }))
          yield { event: 'status', data: { stage: 'synthesizing', detail: 'Generating follow-up response...' } }
          const synthesisDirective = asksUserForHelp
            ? 'You asked the user for help / told them to select a file / said you were stuck. THIS IS A FAILURE RESPONSE. The user has no idea where files live — that is your job. STOP asking for help. Now use exec_command IMMEDIATELY to find the file yourself: try `find /app -type f -name "*<keyword>*" | head -20` or `grep -rln "<unique-string>" /app/lib /app/app /app/components 2>/dev/null | head -10` to locate the real path. Once found, use cat or read_files on the real path. Then respond with concrete findings in past tense (file paths, line numbers, what the code does). Do NOT ask the user any more questions.'
            : danglingIntent
            ? 'You said "' + tailText.slice(-150).trim() + '" but did not actually do the work or produce the answer the user asked for. STOP. The user needs a real, concrete answer NOW. Use any tool you need (read_files, exec_command, etc.) to find the actual information, then respond with: specific file paths, specific line numbers, specific findings. Speak in past tense ("I found X in Y at line Z") not future tense ("I\'ll look at"). Do NOT promise again — deliver.'
            : 'You ran tool(s) and got results. Now respond to the user with a clear, concrete synthesis: what did you find, what does it mean, and what would you do next? Reference specific file paths and line numbers. Speak in past tense about findings. Be confident and direct.'
