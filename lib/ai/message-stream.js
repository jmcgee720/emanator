...
## TOOLS
You have TWO tools:
1. **patch_files** — Edit code files. Use for any code changes.
2. **update_canvas** — Update the Project Canvas (the checklist/notes panel on the right). Use when the user asks to update the checklist, mark items done, add tasks, or modify notes.

When the user mentions "checklist", "canvas", "next steps", "mark as done", or "update the status" — use \`update_canvas\`, NOT \`patch_files\`.
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        if (isSelfEdit) {
          if (selfEditIsConversational) {
            ...
            const isStatusQuestion = /\b(is .+ done|what happened|what.?s the status|did (it|you)|how did|was that|is that|what should|what.?s next|work on next|applied to live|good next step)\b/.test(msgLower)
            const isCanvasRequest = /\b(canvas|checklist|next steps|mark.*(done|complete)|update.*(status|checklist|canvas))\b/.test(msgLower)
            if (isStatusQuestion || isCanvasRequest) {
              toolOpts.tool_choice = 'auto'
              console.log('[SelfEdit] Status/canvas question — tool_choice: auto')
              toolOpts.tool_choice = 'auto'
              console.log('[SelfEdit] Status/canvas question — tool_choice: auto')
            } else {
        ...
        if (isSelfEdit && !selfEditIsConversational) {
          effectiveToolSet = AI_TOOLS.filter(t => t.function?.name === 'patch_files' || t.function?.name === 'update_canvas')
        }
              ...
              if (toolName === 'propose_plan') {
                ...
                if (directEditMode || isSelfEdit || (isBriefBuild && existingFiles === 0)) {
                  ...
                  if (savedFiles.length > 0 && !fullContent) {
                    if (isSelfEdit) {
                      // Self-edit: use AI's own summary from the patch_files tool call
