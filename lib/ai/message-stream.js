...
import { mapImageAssets, buildAssetsFileContent, buildBrandVfsMap } from './brief-utils.js'
import { runPostRepair } from './post-repair.js'
import { analyzeDesignTokens, buildThemeFile, FALLBACK_TOKENS, analyzeLayoutBlueprint, classifyRecipeFamily } from './design-tokens.js'
import { buildManifest } from './build-observatory.js'
import { verifyBuild, formatVerifyForRepairPrompt, findingsToReviewShape, shouldContinueVisualLoop } from './screenshot-verify.js'
import { buildPrimitiveFiles, formatPrimitivesForPrompt } from './primitives.js'
...
- Keep the response focused on the configuration request only`
      ...
      // ══════════════════════════════════════════════════════════════════════
      // ── CREATIVE BRIEF FAST-PATH (archetype-aware, multi-wave pipeline) ──
      ...
      // to building. Runs: classify archetype → plan → build waves → self-review → repair.
      // ══════════════════════════════════════════════════════════════════════
      const isBriefBuildEarly = userMessage.includes('Build this project now with COMPLETE')
      if (isBriefBuildEarly && projectId) {
        console.log('[CreativeBrief-FastPath] Detected brief build — routing to new pipeline')
        ...
        // New pipeline aborted (rare — typically provider failure in classify/plan).
        // Emit a graceful error instead of the deprecated single-file fallback.
        console.warn('[NewPipeline] Aborted without handled=true — emitting error')
        yield { event: 'error', data: { message: 'Build failed at planning stage. Please try again.', error_type: 'pipeline_abort' } }
        await this.logGenerationRun({
          id: runId, project_id: projectId, chat_id: chatId, user_id: userId,
          tool_mode: 'new_pipeline_abort', files_generated: 0,
...
async function* runNewBriefPipeline({ aiService, projectId, chatId, userId, userMessage, attachments, runId, startTime }) {
  ...
  // ── Step 1.5: Art direction + image-asset prep from reference images ──
  // Three categories, each feeds a different layer of the pipeline:
  //   'brand'      → assets.js (rendered in generated site)
  //   'aesthetic'  → design tokens (palette/fonts/vibe)
  //   'structural' → layout blueprint (composition/section-order/nav-style)
  // The UI now tags attachments explicitly; we honor it and fall back to the
  // legacy filename-guessing path only when role is missing.
  ...
  if (imageAttachments.length > 0) {
    ...
    // Fallback: if nothing is explicitly tagged aesthetic, use brand uploads as aesthetic
    // references too (legacy UX — one slot for everything).
    const aestheticForTokens = aestheticAttachments.length > 0 ? aestheticAttachments : brandAttachments

    yield { event: 'status', data: { stage: 'art_direction', detail: `Analyzing ${imageAttachments.length} reference image${imageAttachments.length > 1 ? 's' : ''}...` } }

    // ── Art direction prose (semantic summary for the planner) ──
    const __artT = Date.now()
    try {
      const { analyzeArtDirection } = await import('./art-direction.js')
      artDirection = await analyzeArtDirection(aestheticForTokens, aiService.provider)
      if (artDirection) {
        yield { event: 'art_direction', data: { summary: artDirection, imageCount: aestheticForTokens.length } }
      }
    } catch (e) {
      console.warn('[NewPipeline] Art direction failed, continuing without:', e.message)
    }
    tick('art_direction', __artT)

    // ── Structured design tokens (palette/fonts/radius/vibe) ──
    const __tokT = Date.now()
    try {
      designTokens = await analyzeDesignTokens(aestheticForTokens, aiService.provider)
      if (designTokens) {
        yield { event: 'design_tokens', data: { tokens: designTokens } }
        console.log('[NewPipeline] Design tokens extracted: vibe=' + designTokens.vibe + ' primary=' + designTokens.primary)
      }
    } catch (e) {
      console.warn('[NewPipeline] Design token extraction failed:', e.message)
    }
    tick('design_tokens', __tokT)

    // ── Recipe family classifier ──
    // Picks one of saas-clean / editorial-serif / brutalist-raw / luxury-minimal /
    // playful-illustrated. The builder's recipe block swaps to variant recipes
    // for the chosen family (navbar + landing get a structurally different
    // composition, not just a recolor). Silent fallback to saas-clean on failure.
    const __famT = Date.now()
    try {
      recipeFamily = await classifyRecipeFamily(aestheticForTokens, aiService.provider)
      if (recipeFamily) {
        yield { event: 'recipe_family', data: recipeFamily }
        console.log('[NewPipeline] Recipe family:', recipeFamily.family, `(${recipeFamily.confidence.toFixed(2)})`, '—', recipeFamily.reason)
      }
    } catch (e) {
      console.warn('[NewPipeline] Recipe family classifier failed:', e.message)
    }
    tick('recipe_family', __famT)

    // ── Layout blueprint (from structural screenshots) ──
    if (structuralAttachments.length > 0) {
      const __bpT = Date.now()
      try {
        layoutBlueprint = await analyzeLayoutBlueprint(structuralAttachments, aiService.provider)
        if (layoutBlueprint) {
          yield { event: 'layout_blueprint', data: { blueprint: layoutBlueprint, sourceCount: structuralAttachments.length } }
          console.log('[NewPipeline] Layout blueprint extracted:', layoutBlueprint.sections_order.join(' → '))
        }
      } catch (e) {
        console.warn('[NewPipeline] Layout blueprint extraction failed:', e.message)
      }
      tick('layout_blueprint', __bpT)
    }

    // ── Role-tag brand assets (logo / hero / photo / illustration) ──
    // UI-supplied role wins; mapImageAssets handles sub-classification and
    // preserves the user's per-image placement note on each output.
    imageAssets = mapImageAssets(brandAttachments)
  }
  ...
  // Emit components/theme.js with design tokens as CSS variables. Every
  // themed recipe reads `bg-[var(--primary)]`, `text-[var(--ink)]` etc.
  // Deterministic — guarantees the tokens reach the browser even when
  // the LLM ignores the aesthetic brief.
  try {
    ...
    console.log('[NewPipeline] Emitted theme.js (vibe:', plan.designTokens?.vibe || 'default', ')')
  } catch (e) {
    console.warn('[NewPipeline] Failed to emit theme.js:', e.message)
  }

  // Emit components/assets.js with FULL data URLs — bypasses the LLM entirely
  // so we don't truncate base64 strings. Builder prompt tells the LLM to import
  // from this file. This is the single source of truth for "use my logo here".
  if (imageAssets.length > 0) {
    try {
      ...
      console.log('[NewPipeline] Emitted assets.js with', imageAssets.length, 'data URLs')

      // Also emit the VFS path→dataUrl map over SSE so the preview iframe can
      // resolve `<img src="/logo.png">` style paths (the LLM's natural HTML
      // syntax) in addition to the `import { LOGO_URL }` module syntax.
      const vfsMap = buildBrandVfsMap(imageAssets)
      if (vfsMap.length > 0) {
        yield { event: 'generated_images_map', data: { images: vfsMap, source: 'brand_vfs' } }
        console.log('[NewPipeline] Emitted brand VFS map with', vfsMap.length, 'paths')
      }
    } catch (e) {
      console.warn('[NewPipeline] Failed to emit assets.js:', e.message)
    }
  }

  // ── Step 3c: Emit primitive files (Session 30, 6/7) ──
  // When the layout blueprint is present, pre-compose <Hero /> and
  // <FeatureGrid /> as actual files the builder can import. Parameters
  // (layout, text alignment, columns, card style) map directly from
  // blueprint enum values — no LLM guessing, no hand-tuning.
  if (plan.layoutBlueprint) {
    try {
      ...
      if (primitiveFiles.length > 0) {
        ...
        console.log('[NewPipeline] Emitted', primitiveFiles.length, 'primitive file(s) from blueprint')
        plan.primitivesEmitted = primitiveFiles.map((f) => f.path)
      }
    } catch (e) {
      console.warn('[NewPipeline] Failed to emit primitive files:', e.message)
    }
  }
  ...
  if (allSavedFiles.length > 0) {
    ...
    // Runs AFTER the LLM's review/repair wave. Catches the three
    // structural regressions that prompt rules alone can't guarantee:
    // duplicate router navbar, ignored LOGO_URL, ignored HERO_URL.
    const __postRepairT = Date.now()
    try {
      ...
      if (updates.length > 0) {
        ...
        console.log('[NewPipeline] Post-repair applied to', modifiedPaths.length, 'file(s):', modifiedPaths.join(', '))
        yield { event: 'files_saved', data: { files: modifiedPaths.map((p) => ({ path: p, action: 'post_repair', id: p })) } }
      }
    } catch (err) {
      console.warn('[NewPipeline] Post-repair skipped:', err.message)
    }
    tick('post_repair', __postRepairT)

    // ── Step 5c+d: Visual-fidelity loop (Session 32, N-round re-verify) ──
    // Unified verify → repair → re-verify cycle. Up to N rounds (default 3);
    // short-circuits early when Vision signals MATCH or confidence drops. Each
    // round emits its own `screenshot_verify` event so the observatory can show
    // round-by-round progress (e.g. "Round 1: 4 findings → Round 2: 1 finding
    // → Round 3: MATCH").
    //
    // Why N rounds matters:
    //   - Session 29 capped at 1 round for cost control. Deep mismatches
    //     (palette + composition + typography all at once) often need 2
    //     passes because the first repair focuses on the biggest gap.
    //   - Re-verify between rounds prevents "repair churn" — if round 1
    //     fixed everything, round 2 isn't wasted Vision budget.
    const __visualRepairT = Date.now()
    const MAX_ROUNDS = Number(process.env.VISUAL_REPAIR_MAX_ROUNDS || 3)
    const visualLoopSummary = { rounds: [], totalFilesRepaired: 0, finalMatches: false, initialFindings: 0 }

    if (plan.referenceImages?.length > 0) {
      try {
        for (let round = 0; round < MAX_ROUNDS; round++) {
          // Verify current project state against references
          ...
          if (!verifyResult) {
            console.log(`[NewPipeline] Visual verify round ${round + 1}: no verdict (skipping loop)`)
            break
          }

          plan.verifyResult = verifyResult // final verdict surfaced to manifest
          if (round === 0) visualLoopSummary.initialFindings = verifyResult.findings.length
          yield { event: 'screenshot_verify', data: { ...verifyResult, round: round + 1 } }
          console.log(
            `[NewPipeline] Visual verify round ${round + 1}/${MAX_ROUNDS}:`,
            verifyResult.matches ? 'MATCH' : `${verifyResult.findings.length} finding(s)`,
            `(confidence ${Math.round(verifyResult.confidence * 100)}%)`,
          )

          visualLoopSummary.rounds.push({
            round: round + 1,
          ...
          // Check loop stop conditions (matches / no-findings / max-rounds).
          const gate = shouldContinueVisualLoop(verifyResult, round, MAX_ROUNDS)
          if (gate.stop) {
            visualLoopSummary.finalMatches = verifyResult.matches
            if (gate.reason === 'max-rounds') {
              console.log(`[NewPipeline] Visual repair loop: max rounds reached with ${verifyResult.findings.length} unresolved finding(s)`)
            } else {
              console.log(`[NewPipeline] Visual repair loop: stopping (${gate.reason})`)
            }
            break
          }

          // Synthesize a review-shaped payload and run the targeted repair wave.
          const synthesizedReview = findingsToReviewShape(verifyResult)
          if (synthesizedReview.broken.length === 0) {
            console.log(`[NewPipeline] Round ${round + 1}: no repairable findings (skipping)`)
            break
          }
          ...
          const repairGen = repairBuild({
            plan, review: synthesizedReview, filesBuilt: repairFiles,
            provider: aiService.provider,
            saveFiles: async (files) => aiService.saveFiles(projectId, files, false),
          })
          let repairResult = { filesRepaired: [] }
          ...
          visualLoopSummary.rounds[visualLoopSummary.rounds.length - 1].filesRepaired = repairResult.filesRepaired.length
          visualLoopSummary.totalFilesRepaired += repairResult.filesRepaired.length
          yield { event: 'visual_repair_complete', data: { round: round + 1, filesRepaired: repairResult.filesRepaired } }

          // If repair didn't actually change anything, don't burn another
          // Vision call re-verifying the same thing.
          if (repairResult.filesRepaired.length === 0) {
            console.log(`[NewPipeline] Round ${round + 1}: repair made zero changes, exiting loop`)
            break
          }
        }

        if (visualLoopSummary.rounds.length > 0) {
          yield { event: 'visual_loop_summary', data: visualLoopSummary }
          console.log('[NewPipeline] Visual loop summary:', JSON.stringify(visualLoopSummary))
        }
      } catch (err) {
        console.warn('[NewPipeline] Visual fidelity loop skipped:', err.message)
      }
    }
    tick('visual_repair', __visualRepairT)

    // ── Step 6: Build observatory manifest ──
    // Emits the full state of the build (what's in assets.js / theme.js,
    // integrity check results, timing per phase) so the UI can answer
    // "why didn't my logo render?" in 3 seconds instead of 3 screenshots.
    try {
      const dbFilesFinal = await db.projectFiles.findByProjectId(projectId)
      ...
      const manifest = buildManifest({
        ...
        timings: __buildTimings,
        projectFiles,
      })
      yield { event: 'build_manifest', data: manifest }

      // Emit a project.integrity.check.json file the Preview can introspect.
      // Not a test runner — just a JSON snapshot the UI can read on demand.
      ...
      console.warn('[NewPipeline] Observatory manifest failed:', err.message)
    }
  }
  ...
  // Auto-snapshot this build so the user can rollback later.
  // Best-effort: snapshots table may be unavailable; never fail the build.
  try {
    const dbFilesForSnap = await db.projectFiles.findByProjectId(projectId)
    if (dbFilesForSnap && dbFilesForSnap.length > 0) {
      const shortBrief = (brief.summary || brief.rawBrief || '').slice(0, 60).trim() || plan.brand.name
      const snapName = `Build · ${new Date().toISOString().slice(0, 16).replace('T', ' ')} · ${shortBrief}`
      await db.snapshots.create({
        project_id: projectId,
        name: snapName,
        files_snapshot: dbFilesForSnap.map((f) => ({
          path: f.path,
          content: f.content || '',
        ...
        metadata: {
          file_count: dbFilesForSnap.length,
      ...
      console.log('[NewPipeline] Auto-snapshot created:', snapName)
    }
  } catch (e) {
    console.warn('[NewPipeline] Auto-snapshot failed (non-critical):', e.message)
  }
