...
COLORS: ${colorDirection}. Derive accent from this. NEVER use pink/rose unless brief says pink. ${referencesSites ? `Match quality of: ${referencesSites}.` : ''}
...
NAVBAR: sticky, \`bg-gray-900/60 backdrop-blur-xl border-b border-white/[0.06]\`. Left: SVG logo + "${brandName}". Center: max 5 links. Right: CTA button. Active page highlighted.

HERO (two-column): \`grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-32\`. Left: pill badge → gradient headline (text-5xl md:text-7xl) → subtitle → 2 CTA buttons. Right: floating glass card mockup of the product. ${heroHeadline ? `Headline: "${heroHeadline}".` : ''} ${keyMessaging ? `Subtitle: ${keyMessaging}.` : ''}
CTA LOGIC: Think about the user journey. Visitors on the marketing site are NOT logged in.
- Primary CTA: "Start Free Trial", "Sign Up Free", or "Try ${brandName} Free" → navigates to 'login'
- Secondary CTA: "See How It Works" or "View Pricing" → navigates to 'features' or 'pricing'
- Navbar far-right button: "Login" or "Sign Up" → navigates to 'login'
- NEVER use "Launch Dashboard" as a public CTA — Dashboard is only for logged-in users.

SOCIAL PROOF: after hero. "Trusted by" logos OR metrics bar ("10,000+ users", "4.9/5 rating"). \`grid grid-cols-2 md:grid-cols-4\`.

FEATURES: ${featuresList.length > 0 ? `\`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6\`. For EACH feature, create a glass card with unique SVG icon + actual name + 2-line description. Features: ${featuresList.join(', ')}. NEVER use "Feature 1" — use actual names.` : 'Build compelling features grid with real content.'}

PRICING (if needed): 3-tier cards (\`grid grid-cols-1 md:grid-cols-3 gap-8\`). Free/$0, Pro/$49 (highlighted, "Popular" badge), Enterprise/Custom. Use actual feature names in checklists.

${appPages.length > 0 ? `APP PAGES (${appPages.join(', ')}): Show sidebar nav (w-60, dark) + main content. Content: "Welcome to ${brandName}" + onboarding cards ("Connect a Product", "Set Up SEO", "Create Campaign") with action buttons. Empty states with dashed borders. NO fake metrics — user just signed up.` : ''}

FOOTER: 4-column grid (\`grid grid-cols-2 md:grid-cols-4 gap-8 py-16\`). Col1: "${brandName}" + tagline + social SVG icons. Col2: Product links. Col3: Resources (Blog, Docs, Help). Col4: Legal (Privacy, Terms, Contact). Bottom: "© 2026 ${brandName}".

DESIGN: Glass cards: \`bg-white/[0.02] border border-white/[0.06] rounded-2xl p-8\`. Buttons: \`rounded-full px-8 py-3.5\`. Section spacing: py-24 minimum. Card hover: \`hover:-translate-y-1 transition-all duration-300\`. Button hover: \`hover:scale-105 active:scale-95\`. Alternate layouts — never 2 identical grid sections in a row.

CRITICAL RULES:
- EVERY button MUST have onClick={() => onNavigate('pagename')}. No dead buttons.
- Think about user journey: public pages → CTA goes to 'login'. App pages → CTA goes to relevant app page.
- Primary hero CTA: "Start Free Trial" or "Sign Up Free" → goes to 'login'. NEVER "Launch Dashboard" on a public page.
- Navbar CTA: "Login" or "Sign Up" → goes to 'login'. 
- Pricing MUST have 3 tiers (Free, Pro/$49, Enterprise/Custom) in \`grid grid-cols-1 md:grid-cols-3\`. NEVER a single card.
- Footer MUST be 4-column grid with real links. NEVER a single copyright line.
- Use actual feature names from the brief. NEVER "Feature 1" or generic descriptions.
- Year in footer: 2026, not 2023.`
        ...
        // Add attachments context — describe images to GPT without embedding base64
        let artDirectionNote = ''
          ...
          if (imageAssets.length > 0) {
            ...
            for (const asset of imageAssets) {
              try {
                ...
                savedImages.push({ name: asset.name, path: imgPath, isLogo: /logo/i.test(asset.name), isHero: /hero|banner/i.test(asset.name), isStyle: /style|mood|ref|direction|brand/i.test(asset.name) })
              } catch (e) { console.warn('[CreativeBrief] Failed to save image:', e.message) }
            }
            // Build a description of what was uploaded so GPT matches the aesthetic
            const descriptions = savedImages.map(a => {
              if (a.isLogo) return `- BRAND LOGO: The user has a custom logo for "${brandName}". Create a sophisticated SVG logo: use the first letter "${brandName.charAt(0)}" inside a geometric shape (hexagon, rounded square, or circle) with a gradient fill using the brand's accent colors. This logo MUST appear in the navbar (left side, h-8) and footer.`
              if (a.isStyle) return `- STYLE REFERENCE ("${a.name}"): The user included a mood/style reference image. This means they want a PREMIUM, polished aesthetic. Double down on: glassmorphism, generous whitespace (py-32), subtle gradients, refined typography. Match the sophistication level of linear.app.`
              if (a.isHero) return `- HERO VISUAL: Create an impressive hero section with a floating dashboard mockup on the right side (glass card with mini stats and chart elements inside).`
              return `- DESIGN ASSET ("${a.name}"): User uploaded this for design inspiration. Make the overall design more polished and intentional.`
            }).join('\n')
            artDirectionNote = `\n\nART DIRECTION — The user uploaded ${savedImages.length} design asset(s):\n${descriptions}\nHonor these assets by matching the visual style, colors, and aesthetic they imply. The logo should appear in both the navbar and footer.`
            briefMessages[0].content += artDirectionNote
          }
        }
        ...
        if (briefGeneratedFiles.length === 0) {
          ...
          if (briefToolArgsAccum && briefToolArgsAccum.length > 200) {
            ...
            try {
              ...
              if (!recovered) {
                const contentMatch = briefToolArgsAccum.match(/"content"\s*:\s*"([\s\S]+?)(?:"\s*[,}\]]|$)/)
