...
import { verifyPatchResult, buildVerifiedPatchResponse } from './patch-verification.js'
...
The user will review and approve, revise, or cancel your plan before any files are changed.`
                    ...
                    try {
                      ...
                      // Always auto-execute revised plans
                      {
                        ...
                        if (revSaved.length > 0) {
                          const vResult = verifyPatchResult(revSaved, userMessage)
                        ...
                        try {
                          ...
                          // Always auto-execute self-critique revised plans
                          {
                            ...
                            fullContent = revSaved2.length > 0
                              ? buildVerifiedPatchResponse(verifyPatchResult(revSaved2, userMessage), refinementMode)
                ...
                if (!patchResult.valid) {
                  ...
                  if (rewriteFiles.length > 0) {
                    ...
                    try {
                      ...
                      if (saved.length > 0) {
                        const vResult = verifyPatchResult(saved, userMessage)
                        fullContent = buildVerifiedPatchResponse(vResult, true)
                        yield { event: 'token', data: { content: fullContent } }
                      }
                    } catch (saveErr) {
      ...
      if (directEditMode && generatedFiles.length === 0 && fullContent) {
        ...
        if (parsed.files?.length > 0) {
          ...
          if (savedFiles.length > 0) {
            const vResult = verifyPatchResult(savedFiles, userMessage)
            fullContent = buildVerifiedPatchResponse(vResult, false)
          }
        }
      ...
      if (generatedFiles.length === 0 && diffFiles.length === 0 && fullContent) {
        ...
        if (leakedJSON?.files && Array.isArray(leakedJSON.files) && leakedJSON.files.length > 0) {
          ...
          fullContent = sanitizedSaved.length > 0
            ? buildVerifiedPatchResponse(verifyPatchResult(sanitizedSaved, userMessage), true)
