...
2. Call \`edit_lines\` with the line numbers you want to change (or \`patch_files\` for small files <500 lines)
3. Call \`verify_build\` to confirm it compiled
          ...
          for (const toolCall of toolCalls) {
            try {
              ...
              if (toolName === 'propose_plan') {
                ...
                if (agentLoopCount >= 1) {
                  messages.push({ role: 'user', content: '[SYSTEM: You have now read the file contents with line numbers. Use `edit_lines` (preferred — specify line numbers to replace/insert/delete) or `patch_files` for small targeted edits. Do NOT describe what you will do — call the tool now.]' })
                }
                fullContent = ''
                toolCalls = []
                toolOpts.tool_choice = 'auto'
