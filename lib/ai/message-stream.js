...
/**
 * Identify the most likely self-edit target file from text content.
 * Used both for pre-identification in "All Core System" mode and for broken-promise retry.
 * Returns the matching SELF_EDIT_TARGET entry or null.
 */
function identifyTargetFile(text, targets) {
  ...
  const keywordMap = [
    ...
    { keywords: ['dashboard', 'csv export', 'export button', 'project files section', 'download csv'], target: 'Dashboard.jsx' },
    ...
    { keywords: ['stream engine', 'sse event', 'tool routing', 'message stream'], target: 'message-stream.js' },
    { keywords: ['canvas', 'checklist', 'project management', 'markdown editor'], target: 'CoreCanvas.jsx' },
    { keywords: ['diff view', 'code tab', 'apply to live', 'rollback', 'patch history'], target: 'CodeTab.jsx' },
    { keywords: ['tool definition', 'patch_files tool', 'update_canvas tool', 'create_files tool'], target: 'tools.js' },
    { keywords: ['plan valid', 'plan reject'], target: 'plan-validator.js' },
    { keywords: ['constant', 'self edit target', 'chat type'], target: 'constants.js' },
    { keywords: ['right panel', 'preview tab', 'tab switch'], target: 'RightPanel.jsx' },
 ...
 */
function applyPatchContent(originalContent, patchContent) {
 ...
 */
function validateExportsPreserved(originalContent, newContent) {
 ...
 */
function buildVerifiedResponseWithRuntime(savedFiles, userMessage, isRefinement, isSelfEdit = false) {
 ...
 */
function generateSelfEditSuggestions(savedFiles, userMessage) {
...
- Keep the response focused on the configuration request only`
      ...
      if (isSelfEdit) {
        ...
        if (userMessage.startsWith('[SYSTEM:')) {
          ...
          const isActionSignal = actionSignals.test(msgTrimmed)
...
- **Safety net**: After Apply to Live, the system runs a health check. If your edit breaks the app, it auto-reverts from a snapshot. So be bold but precise.
...
You have two tools for code changes:
- **\`patch_files\`**: For SMALL targeted edits. Each file has a \`patches\` array of \`{ search, replace }\` pairs.
- **\`update_files\`**: For LARGE changes or when the target file is very big (1000+ lines). Provide the COMPLETE updated file content.

**WHEN TO USE WHICH:**
- If the file shown above is < 500 lines → use \`patch_files\` (search/replace is precise on small files)
- If the file shown above is > 500 lines → use \`update_files\` with the ENTIRE file content (patches are unreliable on large files)
- If your first patch_files attempt fails → the system will automatically retry with update_files
...
1. Call \`read_files\` to understand the relevant code
2. Call \`update_files\` (or \`patch_files\` for small files) to make changes
3. Call \`verify_build\` to confirm it compiled
...
- After your create_files call, follow up with a short conversational summary of what you built. Do NOT ask questions.`
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        // Self-edit mode: ALWAYS use 'auto' — let the AI decide whether to respond or call a tool
        // The system prompt guides behavior, not forced tool_choice
        if (isSelfEdit) {
          if (selfEditIsConversational) {
        ...
        if (isSelfEdit && !selfEditIsConversational) {
          effectiveToolSet = AI_TOOLS.filter(t => ['patch_files', 'update_files', 'update_canvas', 'read_files', 'verify_build'].includes(t.function?.name))
        }
          ...
          if (isSelfEdit) {
            ...
            if (!selfEditIsConversational && toolCalls.length === 0 && fullContent.length > 0) {
              ...
              if (promisedAction && selfEditPatchRetry < MAX_SELF_EDIT_RETRIES) {
                ...
                let retryDirective = 'You said you would do it but you did not call any tool. Call patch_files NOW to make the edit. Do not respond with text.'
                  ...
                  try {
                    ...
                    if (bestTarget) {
                      ...
                      if (existsSync(targetPath)) {
                        ...
                        retryDirective = `You promised to act but called no tool. Here is the file you need to edit:\n\n## TARGET FILE: \`${bestTarget.path}\`\n\`\`\`javascript\n${fileContent}\n\`\`\`\n\nCall patch_files NOW with search/replace patches against this EXACT content. The \`path\` in your tool call MUST be \`${bestTarget.path}\`. Do NOT respond with text — ONLY call the tool.`
                      }
                    } else {
                ...
                toolOpts.tool_choice = { type: 'function', function: { name: 'patch_files' } }
                continue
              }
            }
          }
        ...
        if (toolCalls.length > 0) {
              ...
              if (toolName === 'propose_plan') {
                ...
                if (saveTool === 'patch_files' && args.files) {
                  ...
                  if (totalFailed > 0 && totalApplied === 0 && selfEditPatchRetry < MAX_SELF_EDIT_RETRIES && isSelfEdit) {
                    ...
                    // ── Fallback: when patch_files completely fails, retry with update_files ──
                    // For large files, search/replace patches are unreliable. Fall back to full-file update.
                    if (saveTool === '__blocked__' && isSelfEdit && selfEditPatchRetry < MAX_SELF_EDIT_RETRIES) {
                      selfEditPatchRetry++
                      console.log(`[patch_files→update_files] All patches failed — falling back to update_files (attempt ${selfEditPatchRetry})`)
                      yield { event: 'status', data: { stage: 'retrying', detail: 'Patches failed — retrying with full file update...' } }

                      // Load the file the AI was trying to patch
                      const targetFile = failedPatchDetails[0]?.filePath
                      if (targetFile) {
                        const fullPath = path.resolve('/app', targetFile)
                        if (fs.existsSync(fullPath)) {
                          let currentContent = fs.readFileSync(fullPath, 'utf-8')
                          if (currentContent.length > 60000) currentContent = currentContent.slice(0, 60000) + '\n// ... [truncated at 60K chars]'

                          const fallbackMessages = [
                            ...messages,
                            { role: 'assistant', content: fullContent || '(patch attempt failed)' },
                            { role: 'user', content: `Your patch_files call failed because the search strings didn't match. Instead, use the \`update_files\` tool to provide the COMPLETE updated file.\n\nHere is the current content of \`${targetFile}\`:\n\`\`\`\n${currentContent}\n\`\`\`\n\nApply the changes you intended and return the ENTIRE file using update_files. The path must be \`${targetFile}\`. Do NOT use patch_files — use update_files with the complete content.` }
                          ]

                          let fallbackToolCalls = []
                          let fallbackContent = ''
                          try {
                            const fallbackToolOpts = { ...toolOpts, tool_choice: { type: 'function', function: { name: 'update_files' } } }
                            ...
                            console.error('[patch_files→update_files] Fallback stream error:', fallbackErr.message)
                          }

                          if (fallbackToolCalls.length > 0) {
                            const fbTC = fallbackToolCalls.find(tc => {
                              try { return tc.function.name === 'update_files' || tc.function.name === 'create_files' } catch { return false }
                            })
                            if (fbTC) {
                              try {
                                const fbArgs = JSON.parse(fbTC.function.arguments)
                                if (fbArgs.files?.length > 0) {
                                  console.log(`[patch_files→update_files] Fallback succeeded: ${fbArgs.files.length} file(s)`)
                                  args.files = fbArgs.files
                                  saveTool = 'update_files'
                                }
                              } catch (fbParseErr) {
                                console.error('[patch_files→update_files] Parse error:', fbParseErr.message)
                              }
                            }
                          }
                        }
                    ...
                    // Partial success: some patches applied, some failed — log but proceed
                    console.log(`[SelfEdit-Partial] ${totalApplied} patches applied, ${totalFailed} failed — saving partial result`)
                    yield { event: 'status', data: { stage: 'partial_patch', detail: `${totalApplied} of ${totalApplied + totalFailed} patches applied successfully. Saving partial result.` } }
                  ...
                  for (let i = 0; i < args.files.length; i++) {
                    ...
                    if (file.content.trim().startsWith('<<<PATCHES>>>')) {
                      ...
                      if (fs.existsSync(fullPath)) {
                        ...
                        } else if (originalSize > 500 && newSize < originalSize * 0.4) {
                ...
                // ── Agent Loop: read_files ──
                // AI wants to read project files before making changes.
                // Load the files and feed content back as a tool result, then continue the loop.
                const paths = args.paths || []
                const reason = args.reason || ''
                console.log(`[AgentLoop] read_files: ${paths.length} file(s) — ${reason}`)
                yield { event: 'status', data: { stage: 'reading', detail: `Reading ${paths.length} file(s)...` } }

                let fileResults = []
                for (const reqPath of paths.slice(0, 5)) { // Max 5 files per call
                  // Try project files in DB first
                  const dbFile = allProjectFiles.find(f => f.path === reqPath || f.path.endsWith(reqPath))
                  if (dbFile?.content) {
                    const content = dbFile.content.length > 30000 ? dbFile.content.slice(0, 30000) + '\n// ... [truncated at 30K]' : dbFile.content
                    fileResults.push(`## \`${dbFile.path}\` (${dbFile.content.split('\n').length} lines)\n\`\`\`\n${content}\n\`\`\``)
                  } else if (isSelfEdit) {
                    // Self-edit mode: read from disk
                    try {
                      const { readFileSync, existsSync } = await import('fs')
                      const { resolve } = await import('path')
                      const fullPath = resolve('/app', reqPath)
                      if (existsSync(fullPath)) {
                        let content = readFileSync(fullPath, 'utf-8')
                        const lineCount = content.split('\n').length
                        if (content.length > 30000) content = content.slice(0, 30000) + '\n// ... [truncated at 30K]'
                        fileResults.push(`## \`${reqPath}\` (${lineCount} lines)\n\`\`\`\n${content}\n\`\`\``)
                      } else {
                        fileResults.push(`## \`${reqPath}\` — FILE NOT FOUND`)
                      }
                    } catch (readErr) {
                      fileResults.push(`## \`${reqPath}\` — ERROR: ${readErr.message}`)
                    }
                  } else {
                    fileResults.push(`## \`${reqPath}\` — FILE NOT FOUND in project`)
                  }
                }

                const toolResult = fileResults.length > 0
                  ? `Here are the file contents you requested:\n\n${fileResults.join('\n\n')}`
                  : 'No files found matching the requested paths.'

                // Feed tool result back to AI and continue the agent loop
                messages.push({ role: 'assistant', content: fullContent || null, tool_calls: [{ id: toolCall.id || 'read_' + Date.now(), type: 'function', function: { name: 'read_files', arguments: toolCall.function.arguments } }] })
                messages.push({ role: 'tool', tool_call_id: toolCall.id || 'read_' + Date.now(), content: toolResult })
                fullContent = ''
                toolCalls = []
                toolOpts.tool_choice = 'auto'
                agentLoopCount = (agentLoopCount || 0) + 1
                if (agentLoopCount > 6) { break } // Safety: max 6 agent loop iterations
                agentLoopContinue = true
                break // Break out of for-loop, continue the while-loop

              } else if (toolName === 'verify_build') {
                // ── Agent Loop: verify_build ──
                // AI wants to check if its changes compiled successfully.
                const checkType = args.check_type || 'quick'
                console.log(`[AgentLoop] verify_build: ${checkType}`)
                yield { event: 'status', data: { stage: 'verifying', detail: 'Checking compilation...' } }

                let verifyResult = ''
                try {
                  const healthUrl = isSelfEdit ? 'http://localhost:3000/api/health' : 'http://localhost:3000/api/health'
                  const healthRes = await fetch(healthUrl + (checkType === 'full' ? '?compile=full' : ''), { signal: AbortSignal.timeout(10000) })
                  const isHealthy = healthRes.ok

                  if (isHealthy) {
                    verifyResult = 'BUILD VERIFIED: The app compiled successfully. Health check returned 200 OK. Your changes are working.'
                  } else {
                    // Try to get error details
                    let errorDetail = `Health check returned HTTP ${healthRes.status}.`
                    try {
                      const { readFileSync } = await import('fs')
                      const errLog = readFileSync('/var/log/supervisor/nextjs_api.err.log', 'utf-8')
                      const recentErrors = errLog.split('\n').slice(-20).filter(l => l.includes('Error') || l.includes('error') || l.includes('Module build failed')).join('\n')
                      if (recentErrors) errorDetail += `\n\nRecent compilation errors:\n${recentErrors}`
                    } catch {}
                    verifyResult = `BUILD FAILED: ${errorDetail}\n\nYou need to fix the error and try again. Look at the error message to determine what's wrong.`
                  }
                } catch (verifyErr) {
                  verifyResult = `BUILD CHECK ERROR: Could not reach the health endpoint — ${verifyErr.message}. The server may be restarting.`
                }

                // Feed result back to AI and continue the agent loop
                messages.push({ role: 'assistant', content: fullContent || null, tool_calls: [{ id: toolCall.id || 'verify_' + Date.now(), type: 'function', function: { name: 'verify_build', arguments: toolCall.function.arguments } }] })
                messages.push({ role: 'tool', tool_call_id: toolCall.id || 'verify_' + Date.now(), content: verifyResult })
                fullContent = ''
                toolCalls = []
                toolOpts.tool_choice = 'auto'
                agentLoopCount = (agentLoopCount || 0) + 1
                if (agentLoopCount > 6) { break }
                agentLoopContinue = true
                break // Break out of for-loop, continue the while-loop
          ...
          // (handled inline — tools set agentLoopContinue and break out of for-loop)
        }
      }
