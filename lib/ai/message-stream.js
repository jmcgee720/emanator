...
import { reviewBuild, repairBuild } from './brief-reviewer.js'
...
- Keep the response focused on the configuration request only`
      ...
      if (isBriefBuildEarly && projectId) {
        ...
        console.log('[CreativeBrief-FastPath] Attachments:', attachments ? `${attachments.length} items: ${attachments.map(a => `${a.name}(${a.type},${a.data ? a.data.length + 'chars' : 'no-data'})`).join(', ')}` : 'NONE')

        // ── NEW PIPELINE BRANCH (behind feature flag) ──
        // When EMANATOR_NEW_PIPELINE=1, run archetype inference + plan + wave build
        // instead of the legacy single-file fast-path. Legacy path runs when flag is unset.
        const USE_NEW_PIPELINE = process.env.EMANATOR_NEW_PIPELINE === '1'
        if (USE_NEW_PIPELINE) {
          const newPipelineResult = yield* runNewBriefPipeline({
            aiService: this, projectId, chatId, userId, userMessage, attachments, runId, startTime,
          })
          if (newPipelineResult?.handled) return
          // If new pipeline returned handled=false, fall through to legacy fast-path
          console.log('[NewPipeline] Aborted — falling through to legacy fast-path')
        }
        ...
        const marketingPages = pagesList.filter(p => /^(home|features|pricing|about|contact|blog|faq|testimonials)$/i.test(p))
        const appPages = pagesList.filter(p => !(/^(home|features|pricing|about|contact|blog|faq|testimonials|login)$/i.test(p)))
        const hasLogin = pagesList.some(p => /login/i.test(p))

        const briefSystemPrompt = `You build production-ready React apps. Brand: "${brandName}". ${projectDesc ? `Description: ${projectDesc}.` : ''} ${targetAudience ? `Audience: ${targetAudience}.` : ''} Tone: ${toneOfVoice}. ${thingsToAvoid ? `AVOID: ${thingsToAvoid}.` : ''}

RULES: One file \`app/page.jsx\`. All components inline. No imports from local files. No react-router. No \`import React\`. Use only standard Tailwind classes. useState for navigation. Default page: 'home'. 800-1000 lines. Call create_files immediately — no explanations.

COLORS: ${colorDirection || 'Dark mode with accent colors'}. Derive ALL accent colors from this direction — buttons, badges, gradients, active states. If no colors specified, use a dark theme (gray-950) with one accent color. NEVER default to pink/rose unless the brief explicitly says pink. ${referencesSites ? `Match quality of: ${referencesSites}.` : ''}

PAGES: ${marketingPages.length > 0 ? `Marketing (public): ${marketingPages.join(', ')}` : 'Home, Features, Pricing'}. ${hasLogin ? 'Login page: glass card, email+password, sign in button.' : ''} ${appPages.length > 0 ? `App pages (behind login, show onboarding/empty state — NO fake data): ${appPages.join(', ')}.` : ''}

NAVBAR: sticky, glassmorphism (backdrop-blur-xl, semi-transparent bg, subtle border-b). Left: SVG logo (unique geometric shape with accent gradient) + "${brandName}". Center: max 5 links. Right: CTA button → 'login'. Active page highlighted with accent color.

HERO (two-column on desktop): \`grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-32\`. Left: pill badge → gradient headline (text-5xl md:text-7xl, use accent colors for gradient) → subtitle → 2 CTA buttons. Right: floating glass card mockup hinting at the product. ${heroHeadline ? `Headline: "${heroHeadline}".` : ''} ${keyMessaging ? `Subtitle: ${keyMessaging}.` : ''}
CTA LOGIC: Visitors are NOT logged in. Primary: "Start Free Trial" or "Sign Up Free" → 'login'. Secondary: "See How It Works" → 'features'. Navbar right: "Login" → 'login'. NEVER "Launch Dashboard" on public pages.

SOCIAL PROOF: after hero. Metrics bar or "Trusted by" logos. \`grid grid-cols-2 md:grid-cols-4\`.

FEATURES: ${featuresList.length > 0 ? `For EACH feature below, create a glass card with unique SVG icon + the exact feature name + 2-line description specific to ${brandName}. Features: ${featuresList.join(', ')}. NEVER use "Feature 1" — use actual names. Layout: ${featuresList.length <= 3 ? 'grid grid-cols-1 md:grid-cols-' + featuresList.length : featuresList.length <= 6 ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3'} gap-6.` : 'Build a compelling features section with real content relevant to the product.'}

PRICING (if page exists): 3-tier cards (\`grid grid-cols-1 md:grid-cols-3 gap-8\`). Free, Pro (highlighted, "Popular"), Enterprise/Custom. Use feature names from the brief in checklists.

${appPages.length > 0 ? `APP PAGES (${appPages.join(', ')}): sidebar nav (w-60, dark) + main content. Show ONBOARDING state specific to "${brandName}": welcome message, 3 setup cards relevant to what this product does (e.g., "${projectDesc ? projectDesc.split(',')[0].trim().slice(0, 40) : 'Get started'}"), empty data areas with dashed borders. NO fake metrics.` : ''}

FOOTER: 4-column grid (\`grid grid-cols-2 md:grid-cols-4 gap-8 py-16\`). Col1: "${brandName}" + tagline + social SVG icons. Col2: Product links. Col3: Resources. Col4: Legal + contact. Bottom: "© 2026 ${brandName}".

DESIGN: Glass cards: semi-transparent bg + subtle border + rounded-2xl + p-8. Section spacing: py-24 minimum. Card hover: -translate-y-1. Button hover: scale-105. Alternate layouts between sections. ALL interactive elements have transitions.

CRITICAL RULES:
- EVERY button MUST have onClick={() => onNavigate('pagename')}. No dead buttons.
- Primary hero CTA → goes to 'login'. NEVER "Launch Dashboard" on public pages.
- Pricing: 3 tiers in a row, NEVER a single card.
- Footer: 4-column grid, NEVER a single copyright line.
- Use ACTUAL feature names. NEVER "Feature 1" or generic descriptions.
- Year: 2026.`
        ...
        // Add art direction images as GPT-4o VISION input so GPT can actually SEE them
        if (attachments?.length > 0) {
          const imageAssets = attachments.filter(a => a.type === 'image' && a.data)
          if (imageAssets.length > 0) {
            console.log(`[CreativeBrief-FastPath] Processing ${imageAssets.length} art direction image(s) for vision...`)

            // Save images as project files
            for (const asset of imageAssets) {
              try {
                const imgPath = `assets/${asset.name || ('image-' + Date.now() + '.png')}`
                await this.saveFiles(projectId, [{ path: imgPath, content: asset.data }], false)
              } catch (e) { console.warn('[CreativeBrief] Failed to save image:', e.message) }
            }

            // Resize and prepare for vision
            const visionImages = await prepareVisionImages(imageAssets)

            if (visionImages.length > 0) {
              // Build a multipart user message with text + images
              const imageDescriptions = imageAssets.map(a => {
                const isLogo = /logo/i.test(a.name)
                const isStyle = /style|mood|ref|direction|brand/i.test(a.name)
                if (isLogo) return `Image 1 is the brand LOGO for "${brandName}". Match its colors, shapes, and style in the navbar logo SVG and throughout the design.`
                if (isStyle) return `This image is a STYLE REFERENCE. Match its color palette, spacing, and overall aesthetic in the generated UI.`
                return `This image is a design asset for "${brandName}". Use its visual style as inspiration.`
              }).join(' ')

              // Replace the user message with a multipart content array (text + images)
              briefMessages[briefMessages.length - 1] = {
                role: 'user',
                content: [
                  { type: 'text', text: `${userMessage}\n\nART DIRECTION: I've attached ${visionImages.length} image(s). ${imageDescriptions} Study these images carefully and match their aesthetic in the generated code.` },
                  ...visionImages
                ]
              }
              console.log(`[CreativeBrief-FastPath] Added ${visionImages.length} vision image(s) to user message`)
            } else {
              // Images too large or processing failed — fall back to text description
              const logoName = imageAssets.find(a => /logo/i.test(a.name))
              briefMessages[0].content += `\n\nArt direction: User uploaded ${imageAssets.length} image(s)${logoName ? ` including a logo. Create an SVG logo using the first letter "${brandName.charAt(0)}" in a geometric shape with brand accent colors.` : '. Make the design premium and polished.'}`
            }
          }
        }
        ...
        console.log(`[CreativeBrief-FastPath] Tool calls received: ${briefToolCalls.length}, toolArgsAccum: ${briefToolArgsAccum.length} chars, textContent: ${briefContent.length} chars`)
        ...
        // If no files were generated via tool call, try fallbacks
        if (briefGeneratedFiles.length === 0) {
          // Fallback 1: recover from accumulated tool args (GPT streamed the tool call but it wasn't assembled)
          if (briefToolArgsAccum && briefToolArgsAccum.length > 200) {
            console.log(`[CreativeBrief-FastPath] 0 tool calls but ${briefToolArgsAccum.length} chars of tool_args_delta — recovering`)
            try {
              // Try direct JSON parse first (most common: complete JSON that just wasn't yielded as tool_calls)
              let recovered = false
              try {
                const parsed = JSON.parse(briefToolArgsAccum)
                const files = parsed.files || (parsed.path ? [parsed] : [])
                if (files.length > 0 && files[0].content?.length > 200) {
                  yield { event: 'status', data: { stage: 'saving_files', detail: `Recovering ${files.length} file(s)...` } }
                  const saved = await this.saveFiles(projectId, files, false)
                  for (const f of saved) briefGeneratedFiles.push(f)
                  recovered = true
                  console.log(`[CreativeBrief-Recovery] JSON parse succeeded: ${saved.length} file(s)`)
                }
              } catch {}

              // If JSON parse failed, extract content via regex (incomplete JSON)
              if (!recovered) {
                // Match content that may be truncated (no closing quote)
                // The content field in the JSON looks like: "content":"...code..."
                // When truncated, it may end without closing quote/brace
                const contentMatch = briefToolArgsAccum.match(/"content"\s*:\s*"((?:[^"\\]|\\.)*)/)
                const pathMatch = briefToolArgsAccum.match(/"path"\s*:\s*"([^"]+)"/)
                console.log(`[CreativeBrief-Recovery] Regex: contentMatch=${contentMatch ? contentMatch[1].length + 'chars' : 'null'}, pathMatch=${pathMatch ? pathMatch[1] : 'null'}`)
                if (contentMatch && contentMatch[1].length > 200) {
                  ...
                  yield { event: 'status', data: { stage: 'saving_files', detail: 'Recovering build...' } }
                  const saved = await this.saveFiles(projectId, [{ path: filePath, content }], false)
                  for (const f of saved) briefGeneratedFiles.push(f)
                  console.log(`[CreativeBrief-Recovery] Regex extract: ${filePath} (${content.length} chars)`)
                }
              }
            } catch (parseErr) {
              console.warn('[CreativeBrief-Recovery] Failed:', parseErr.message)
            }
          }

          // Fallback 2: try parsing from text content (code blocks)
          if (briefGeneratedFiles.length === 0 && briefContent && briefContent.length > 200) {
            console.log('[CreativeBrief-FastPath] Trying text-parse fallback')
          ...
          // Fallback 3: RETRY with the SAME full prompt using non-streaming API
          if (briefGeneratedFiles.length === 0) {
            console.log('[CreativeBrief-FastPath] All recovery failed — retrying with non-streaming API')
            yield { event: 'status', data: { stage: 'retrying', detail: 'Retrying build...' } }
            try {
              const retryResult = await this.provider.chatWithTools(
                briefMessages,
                AI_TOOLS.filter(t => ['create_files', 'update_files'].includes(t.function?.name)),
                { temperature: 0.8, max_tokens: 16384, tool_choice: { type: 'function', function: { name: 'create_files' } } }
              )
              // Non-streaming returns { content, tool_calls }
              const retryToolCalls = retryResult.tool_calls || []
              console.log(`[CreativeBrief-Retry] Non-streaming returned ${retryToolCalls.length} tool calls`)
              for (const tc of retryToolCalls) {
                try {
                  ...
                  if (files.length > 0) {
                    ...
                    const saved = await this.saveFiles(projectId, files, false)
                    for (const f of saved) briefGeneratedFiles.push(f)
                    console.log(`[CreativeBrief-Retry] Saved ${saved.length} file(s)`)
                  }
                } catch (e) { console.warn('[CreativeBrief-Retry] Parse error:', e.message) }
              }
              if (retryResult.content) {
                briefContent += retryResult.content
                yield { event: 'token', data: { content: retryResult.content } }
              }
            } catch (retryErr) {
              console.error('[CreativeBrief-Retry] Failed:', retryErr.message)
            }
          }
        }
        ...
        // Emit files_saved event so the preview refreshes immediately — don't wait for done
        if (briefGeneratedFiles.length > 0) {
          yield { event: 'files_saved', data: { files: briefGeneratedFiles.map(f => ({ path: f.path, action: f.action || 'created', id: f.id })), projectId } }
        }
...
async function* runNewBriefPipeline({ aiService, projectId, chatId, userId, userMessage, attachments, runId, startTime }) {
  ...
  const brief = {
    brandName: extractField('Brand name (MUST use this exact name throughout the UI)') || extractField('Brand name') || extractField('Project name') || 'My App',
    colorDirection: extractField('Color direction') || extractField('Colors') || 'Dark mode, violet-blue gradients',
    ...
    mostImportantPage: extractField('Most important page (build this with the most detail)') || extractField('Most important page') || 'Dashboard',
    ...
    toneOfVoice: extractField('Tone of voice') || extractField('Tone') || 'Professional',
    targetAudience: extractField('Target audience') || '',
    projectDesc: extractField('Project description') || extractField('Project') || '',
  }
  ...
  let plan
  try {
    plan = await generatePlan({ brief, archetype, provider: aiService.provider })
    ...
    console.log('[NewPipeline] Plan:', plan.routes.length, 'routes,', plan.components.length, 'components,', plan.waves.length, 'waves')
  } catch (err) {
    console.error('[NewPipeline] Planner failed:', err.message)
    return { handled: false }
  }

  // ── Step 3: Save art direction images as project files (reuse existing logic) ──
  if (attachments?.length > 0) {
    const imageAssets = attachments.filter((a) => a.type === 'image' && a.data)
    for (const asset of imageAssets) {
      try {
  ...
  // ── Step 5: Self-review + auto-repair (one pass max) ──
  let reviewResult = null
  if (allSavedFiles.length > 0) {
    yield { event: 'status', data: { stage: 'reviewing', detail: 'Reviewing build for missing flows...' } }
    try {
      // Note: allSavedFiles from saveFiles() returns {path, id, action} without content.
      // For the review, we need content — reconstruct from what saveFiles received.
      // Since saveFiles was called per-wave, we lost content. Fetch from Supabase.
      ...
      try {
        ...
        // Fallback: shell-only review (no content samples) — reviewer handles gracefully
      ...
      console.log('[NewPipeline] Review:', reviewResult.ok ? 'OK' : `missing=${reviewResult.missing.length} broken=${reviewResult.broken.length}`)

      if (!reviewResult.ok) {
        yield { event: 'status', data: { stage: 'repairing', detail: `Auto-repairing ${reviewResult.missing.length + reviewResult.broken.length} issue(s)...` } }
        ...
        console.log('[NewPipeline] Repair:', repairResult.filesRepaired.length, 'file(s)')
      }
    } catch (err) {
      console.warn('[NewPipeline] Review/repair skipped:', err.message)
    }
  }
  ...
  await aiService.logGenerationRun({
    id: runId, project_id: projectId, chat_id: chatId, user_id: userId,
    tool_mode: 'new_pipeline', files_generated: allSavedFiles.length,
