...
DESIGN: Glass cards: \`bg-white/[0.02] border border-white/[0.06] rounded-2xl p-8\`. Buttons: \`rounded-full px-8 py-3.5\`. Section spacing: py-24 minimum. Card hover: \`hover:-translate-y-1 transition-all duration-300\`. Button hover: \`hover:scale-105 active:scale-95\`. Alternate layouts — never 2 identical grid sections in a row.`
        ...
        if (briefGeneratedFiles.length === 0) {
          // Fallback 1: try parsing from accumulated tool args (GPT may have sent incomplete tool call)
          if (briefToolArgsAccum && briefToolArgsAccum.length > 100) {
            console.log(`[CreativeBrief-FastPath] No tool calls but have ${briefToolArgsAccum.length} chars of tool args — trying to parse`)
            try {
              // Try to extract files from incomplete JSON
              const fileMatch = briefToolArgsAccum.match(/"content"\s*:\s*"([\s\S]+?)(?:"\s*[,\}]|$)/)
              const pathMatch = briefToolArgsAccum.match(/"path"\s*:\s*"([^"]+)"/)
              if (fileMatch && pathMatch) {
                const content = fileMatch[1].replace(/\\n/g, '\n').replace(/\\"/g, '"').replace(/\\\\/g, '\\')
                if (content.length > 200) {
                  const saved = await this.saveFiles(projectId, [{ path: pathMatch[1], content }], false)
                  for (const f of saved) briefGeneratedFiles.push(f)
                  console.log(`[CreativeBrief-FastPath] Recovered file from incomplete tool args: ${pathMatch[1]} (${content.length} chars)`)
                }
              }
            } catch (parseErr) {
              console.warn('[CreativeBrief-FastPath] Failed to parse incomplete tool args:', parseErr.message)
            }
          }
          ...
          // Fallback 3: if still nothing, RETRY once with a simpler prompt
          if (briefGeneratedFiles.length === 0) {
            console.log('[CreativeBrief-FastPath] All fallbacks failed — retrying with simplified request')
            yield { event: 'status', data: { stage: 'retrying', detail: 'Retrying build...' } }
            try {
              const retryMessages = [
                { role: 'system', content: `Generate a single React file called app/page.jsx for "${brandName}". Dark theme, Tailwind CSS. Include navbar, hero, features, pricing, footer. All components inline. 600+ lines. Call create_files NOW.` },
                { role: 'user', content: `Build ${brandName}: ${projectDesc || briefContent || userMessage}` }
              ]
              for await (const chunk of this._streamWithFallback(() => this.provider.chatWithToolsStream(
                retryMessages,
                AI_TOOLS.filter(t => ['create_files', 'update_files'].includes(t.function?.name)),
                  ...
                  for (const tc of (chunk.tool_calls || [])) {
                    try {
                      ...
                      if (args.files?.length > 0) {
                        const saved = await this.saveFiles(projectId, args.files, false)
                        for (const f of saved) briefGeneratedFiles.push(f)
