        ...
        if (attachments?.length > 0) {
          ...
          if (imageAssets.length > 0) {
            ...
            const descriptions = savedImages.map(a => {
              if (a.isLogo) return `- LOGO uploaded ("${a.name}"): The user has a custom logo. Create an SVG logo in the navbar and footer that matches the brand "${brandName}" — use a geometric icon with the brand's accent color gradient.`
              if (a.isStyle) return `- STYLE REFERENCE uploaded ("${a.name}"): The user included mood/style direction. Match this aesthetic — pay close attention to the color preferences and mood described in the brief.`
              if (a.isHero) return `- HERO IMAGE uploaded ("${a.name}"): Create a visually rich hero with decorative gradient elements and floating UI cards.`
              return `- IMAGE uploaded ("${a.name}"): Use as design inspiration for the overall aesthetic.`
            }).join('\n')
