...
Include:
- Small pill badge above headline (\`rounded-full bg-violet-500/10 border border-violet-500/20 px-4 py-1.5 text-sm\`) with a relevant micro-copy like "AI-Powered" or "Now in Beta"
- Large gradient headline (\`text-5xl md:text-7xl font-bold bg-gradient-to-r bg-clip-text text-transparent\`) — use the brand's gradient colors
- 1-2 line subtitle in muted text
- Two CTA buttons — VARY THE LABELS based on the product:
  - Primary (filled, glow shadow): use action-specific text like "Launch Dashboard", "Start Free Trial", "Try ${brandName} Free" — NOT just "Get Started"
  - Secondary (glass/outline): "See How It Works", "Watch Demo", "View Pricing" — NOT just "Learn More"
  - BOTH buttons must call onNavigate() to go to a real page (dashboard, pricing, features)
- Decorative background: gradient blur orbs (\`absolute w-96 h-96 rounded-full blur-3xl opacity-20\`)
...
- Every interactive element needs hover states
- Glass cards: \`bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6\`
- Spacing: generous padding (\`py-24 px-6\` between sections, \`gap-6\` or \`gap-8\` in grids)
- Typography hierarchy: hero headline 5xl-7xl, section headings 3xl-4xl, card titles xl, body text base/lg
...
[ ] Section spacing is py-20 or py-24 (not py-8 or py-12 — too cramped)
[ ] All CTA buttons call onNavigate() to a real page
[ ] ${mostImportantPage} page has stats cards, chart, data table (not just text)
[ ] Social proof section exists between hero and features
[ ] Active nav link is highlighted
[ ] All cards have hover:-translate-y-1 transition
...
- Massive whitespace between sections (py-24 to py-32)
- Headlines are short, punchy, left-aligned or centered with text-5xl to text-7xl
- Feature cards are MINIMAL: small colored icon, bold title, 1-2 lines of description. NOT busy.
- Color is used SPARINGLY — mostly grayscale with ONE accent color for CTAs and highlights
- Backgrounds: solid dark (not gradient-everything). Use gradient ONLY on text and accent elements.
- Typography: large size contrast between heading and body. Hero might be 72px, body 16px.
- Navigation: logo left, 4-5 links center, CTA button right. Nothing else.`
