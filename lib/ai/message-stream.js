...
- Call \`update_files\` with the COMPLETE modified file content.`
      ...
      // ── CONTEXT TRIMMING: cap history BEFORE building messages array ──
      ...
      const MAX_HISTORY_MESSAGES = 40 // Keep last 40 messages (20 turns) — raised from 6 to allow longer conversations
      ...
      const FORK_THRESHOLD_CHARS = 150000 // Suggest fork at ~37K tokens (~150K chars) — raised from 100K
      
      // Check if we should suggest forking the chat
      // Only suggest fork when BOTH conditions are met: high character count AND many messages
      const shouldSuggestFork = totalChars > FORK_THRESHOLD_CHARS && chatHistory.length > 30
      ...
      if (totalChars > MAX_CONTEXT_CHARS) {
        ...
        // Notify user they should fork the chat
        if (shouldSuggestFork) {
          const estimatedTokens = Math.round(totalChars / 4)
          yield { event: 'fork_suggested', data: { 
            reason: 'context_limit',
            totalChars,
            estimatedTokens,
            messageCount: chatHistory.length,
            suggestion: `This conversation is getting long (~${estimatedTokens.toLocaleString()} tokens, ${chatHistory.length} messages). Fork to a new chat to continue with a fresh context — I'll summarize everything so you can pick up where we left off.`
          }}
        }
      }
