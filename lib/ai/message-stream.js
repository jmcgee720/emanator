          ...
          for (const toolCall of toolCalls) {
            try {
              ...
              if (toolName === 'propose_plan') {
                ...
                // EXCEPT: For new projects (no existing files), always show the plan card
                // so the user can review and approve before building.
                ...
                const isSafePlan = !isNewProject && planActions.length <= 3 && !isLargeAppBuild(userMessage)
                ...
                // ── New project or large plan: emit plan for user approval (PlanCard) ──
                if (isNewProject || planActions.length > 3) {
                  console.log('[propose_plan] Emitting plan for user approval:', args.summary)
                  // The plan will be stored in message metadata and PlanCard will render
                  // with "Proceed" / "Change" / "Cancel" buttons
                  planOutput = {
                ...
                // Risky plan on existing project — suppressed
