...
- Keep the response focused on the configuration request only`
      ...
      // ── Self-Edit Mode: inject target file content so the AI grounds edits in real code ──
      if (selfEditTarget?.path) {
        try {
          ...
          if (existsSync(targetPath)) {
            ...
            systemMessage += `\n\n` +
              `## CORE SYSTEM SELF-EDIT MODE\n` +
              `You are editing an EXISTING source file in the Emanator codebase.\n` +
              `**Target file**: \`${selfEditTarget.path}\`\n` +
              `**Description**: ${selfEditTarget.description || 'Core system file'}\n\n` +
              `### CURRENT FILE CONTENTS\n` +
              `\`\`\`javascript\n${truncated}\n\`\`\`\n\n` +
              `### SELF-EDIT RULES\n` +
              `1. You MUST use the \`update_files\` tool with path \`${selfEditTarget.path}\` to apply changes.\n` +
              `2. Read the current file contents above CAREFULLY before making changes.\n` +
              `3. Make TARGETED, MINIMAL edits — do NOT rewrite the entire file unless explicitly asked.\n` +
              `4. NEVER create new standalone files. All changes go into the existing file at \`${selfEditTarget.path}\`.\n` +
              `5. Preserve all existing functionality — only add/modify what the user requested.\n` +
              `6. Return the COMPLETE updated file content in the tool call (the system replaces the file wholesale).\n`
            console.log('[SelfEdit] Injected target file context:', selfEditTarget.path, `(${fileContent.length} chars)`)
          } else {
            console.warn('[SelfEdit] Target file not found on disk:', targetPath)
          }
        } catch (err) {
