...
import { analyzeDesignTokens, buildThemeFile, FALLBACK_TOKENS } from './design-tokens.js'
...
async function* runNewBriefPipeline({ aiService, projectId, chatId, userId, userMessage, attachments, runId, startTime }) {
  ...
  if (imageAttachments.length > 0) {
    ...
    try {
      ...
      artDirection = await analyzeArtDirection(imageAttachments, aiService.provider)
      if (artDirection) {
        yield { event: 'art_direction', data: { summary: artDirection, imageCount: imageAttachments.length } }
        console.log('[NewPipeline] Art direction extracted:', artDirection.slice(0, 120) + '…')
      }
    } catch (e) {
      console.warn('[NewPipeline] Art direction failed, continuing without:', e.message)
    }

    // Extract STRUCTURED design tokens (palette, fonts, radius, vibe) via a
    // second Vision call. These get written to components/theme.js as CSS
    // vars every themed recipe reads from — the deterministic path for
    // "make the output actually look like the references".
    try {
      designTokens = await analyzeDesignTokens(imageAttachments, aiService.provider)
      if (designTokens) {
    ...
    // ALSO store each image as a concrete asset the builder can reference via
    // <img src="..."> — with a guessed role (logo / hero / reference) so the
    // LLM knows which image to use where. Fixes the regression where the new
    // pipeline used images for aesthetic extraction only, ignoring them as
    // actual logo/hero assets.
    imageAssets = mapImageAssets(imageAttachments)
  }
