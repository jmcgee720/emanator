...
## CRITICAL — DO NOT NARRATE INTENT WITHOUT ACTING
When you need to read, search, find, check, or look at files, you MUST invoke the corresponding tool **in the SAME response**. Forbidden patterns:
- ❌ "Let me read the file..." (without calling read_files)
- ❌ "I'll check what files exist..." (without calling read_files / exec_command)
- ❌ "Let me look at..." / "Let me see..." (without a tool call)
- ❌ "I'll do that right now:" (followed by no tool call)

Required pattern: if you say you'll read/check/look at something, the next thing in your response MUST be a tool invocation. Talk = act. The user can see whether tools fire — claiming you'll do something and then not doing it is the most frustrating experience possible. Always actually call the tool.
...
- Call \`update_files\` with the COMPLETE modified file content.`
      ...
      // If the AI used tools (read_files, exec_command, search_replace, etc.)
      // OR ended its text with intent narration ("Let me check...", "Let me
      // find...", etc.) without a closing user-visible answer, force ONE
      // final LLM call with tool_choice='none' so the user always gets a
      // proper follow-up response. Without this, conversations end with
      // dangling intent like "Let me find the correct file:" and no synthesis.
      if (isSelfEdit && toolMode !== 'chat_only') {
        const trimmedTail = (fullContent || '').trim().slice(-200).toLowerCase()
        const endsWithIntentDangling = /(let me|i'?ll|i will|going to|about to|checking|reading|looking|finding|searching|investigating)[^.!?]*[:\s]*$/.test(trimmedTail) && !trimmedTail.endsWith('.') && !trimmedTail.endsWith('!') && !trimmedTail.endsWith('?')
        const usedAnyTool = (messages || []).some(m => m.role === 'tool')
        const needsSynthesis = (usedAnyTool && fullContent.length < 500) || endsWithIntentDangling
        if (needsSynthesis) {
          console.log('[SelfEdit-Synthesis] Triggering final follow-up pass:', JSON.stringify({ usedAnyTool, endsWithIntentDangling, fullContentLength: fullContent.length }))
          yield { event: 'status', data: { stage: 'synthesizing', detail: 'Generating follow-up response...' } }
          // Force the LLM to summarize what it did/found and answer the user
          // in plain text. No more tools — just close out the conversation.
          const synthesisDirective = endsWithIntentDangling
            ? 'You started saying "' + trimmedTail.slice(-80).trim() + '" but didn\'t actually finish. Now respond with a clear, concrete answer to the user\'s original question. Reference any file contents you read. Be specific. Do NOT say "let me check" or "I will" — speak in past tense about what you found.'
            : 'You ran tool(s) and got results. Now respond to the user with a clear synthesis: what did you find, what does it mean, and what would you do next? Reference specific file paths and line numbers if relevant. Be confident and concise.'
          const synthMessages = [...messages, { role: 'user', content: '[SYSTEM: ' + synthesisDirective + ']' }]
          try {
            let synthText = ''
            ...
            console.error('[SelfEdit-Synthesis] Failed:', synthErr.message)
            // Fall through with the original fullContent
          }
        }
      }
