...
async function* runNewBriefPipeline({ aiService, projectId, chatId, userId, userMessage, attachments, runId, startTime }) {
  ...
  try {
    ...
    // Load project settings to decide whether to wire Supabase recipes.
    // User opts in by setting project.settings.supabase = { url, anonKey }.
    try {
    ...
    console.log('[NewPipeline] Plan:', plan.routes.length, 'routes,', plan.components.length, 'components,', plan.waves.length, 'waves, supabase:', plan.useSupabase)
  } catch (err) {
