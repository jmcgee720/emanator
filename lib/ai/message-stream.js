...
- Keep the response focused on the configuration request only`
      ...
      // direct edit mode and read_only_report force no plan
          ...
          for (const toolCall of toolCalls) {
            try {
              ...
              if (toolName === 'propose_plan') {
                    ...
                    try {
                      ...
                      if (revNewProj || revSafe) {
                        ...
                        if (revSaved.length > 0) {
                          fullContent = revNewProj
                            ? `Your project is live! The preview is updating on the right.\n\nHere are some ideas for what to do next:\n- Review the preview and tell me what to change\n- Add new pages or sections\n- Customize the design, colors, or layout\n- Add features like forms, auth, or payments`
                            : `Done — I've updated the preview.`
                        } else {
                          fullContent = `I tried a revised approach but couldn't complete it. Could you try rephrasing?`
                        }
                        yield { event: 'token', data: { content: fullContent } }
                      } else {
                        // Large revised plan — surface for user approval (PlanCard)
                        console.log('[propose_plan] Revised plan too large for auto-exec — surfacing for approval')
                        planOutput = {
                          summary: revArgs.summary,
                          file_actions: revArgs.file_actions,
                          constraints_checked: revArgs.constraints_checked,
                        }
                        fullContent = revArgs.summary || fullContent
                        yield { event: 'token', data: { content: fullContent } }
                      }
                    } catch (parseErr) {
                      ...
                      if (revisedPlanCall) {
                        try {
                          ...
                          if (critNewProj || revSafe2) {
                            ...
                            // Large self-critique plan — surface for user approval (PlanCard)
                            console.log('[propose_plan] Self-critique plan too large — surfacing for approval')
                            planOutput = {
                              summary: revisedArgs.summary,
                              file_actions: revisedArgs.file_actions,
                              constraints_checked: revisedArgs.constraints_checked,
                            }
                            fullContent = revisedArgs.summary || fullContent
                            yield { event: 'token', data: { content: fullContent } }
                          }
                        } catch (parseErr) {
                ...
                // Tag plan: auto-execute for new projects and safe plans, PlanCard for everything else
                ...
                if (isNewProject || isSafePlan) {
                  ...
                  if (savedFiles.length > 0) {
                    ...
                    if (isNewProject) {
                      fullContent = `Your project is live! The preview is updating on the right.\n\nHere are some ideas for what to do next:\n- Review the preview and tell me what to change\n- Add new pages or sections\n- Customize the design, colors, or layout\n- Add features like forms, auth, or payments`
                    } else {
                      fullContent = `Done — I've updated the preview.`
                    }
                    yield { event: 'token', data: { content: fullContent } }
                ...
                // ── Large plan on existing project: surface for user approval (PlanCard) ──
                console.log('[propose_plan] Emitting plan for user approval:', args.summary)
                planOutput = {
                  summary: args.summary,
                  file_actions: args.file_actions,
                  constraints_checked: args.constraints_checked,
                }
                fullContent = args.summary || fullContent
                yield { event: 'token', data: { content: fullContent } }
