...
- Call \`update_files\` with the COMPLETE modified file content.`
      ...
      if (isSelfEdit && toolMode !== 'chat_only') {
        ...
        const needsSynthesis = (usedAnyTool && (fullContent || '').length < 500) || danglingIntent
        if (needsSynthesis) {
          console.log('[SelfEdit-Synthesis] Triggering final follow-up pass:', JSON.stringify({ usedAnyTool, danglingIntent, hasIntentVerb, hasCompletionVerb, isTooShort, fullContentLength: (fullContent || '').length }))
          yield { event: 'status', data: { stage: 'synthesizing', detail: 'Generating follow-up response...' } }
          const synthesisDirective = danglingIntent
