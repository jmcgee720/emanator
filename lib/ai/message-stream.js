        ...
        while (true) {
          ...
          if (isSelfEdit) {
            ...
            if (!selfEditIsConversational && toolCalls.length === 0 && fullContent.length > 0) {
              const promisedAction = /\b(let'?s (get started|do|implement|add|begin|proceed|start|build|work on|make)|let me (implement|add|create|build|start|proceed|get started)|i'?ll (add|implement|create|update|modify|do|build|make|start|proceed|work on)|here'?s (what|how|the)|implementing|starting|proceeding)\b/i.test(fullContent)
