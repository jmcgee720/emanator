...
function buildVerifiedResponseWithRuntime(savedFiles, userMessage, isRefinement, isSelfEdit = false) {
  ...
  const text = buildVerifiedPatchResponse(vResult, isRefinement, isSelfEdit)
...
function generateSelfEditSuggestions(savedFiles, userMessage) {
  ...
  if (paths.includes('plan-validator')) {
    pool.push('Add file dependency graph validation — ensure imports are resolved before files that use them')
    pool.push('Add a complexity ceiling — reject plans with >8 simultaneous file changes and suggest breaking into phases')
    pool.push('Add rollback safety scoring — flag plans that modify >50% of a file without clear search/replace anchors')
  }

  if (paths.includes('service') || paths.includes('request_router')) {
    pool.push('Add intent confidence routing — send to clarification dialog when confidence < 0.6')
    pool.push('Cache successful prompt→action patterns for instant routing on repeat requests')
    pool.push('Add conversation memory — detect when user references "the thing I asked about earlier"')
  }

  if (paths.includes('feature_planner') || paths.includes('safe_apply')) {
    pool.push('Add pre-flight checks — validate that plan file paths exist and imports resolve before execution')
    pool.push('Add incremental apply — write files one at a time with rollback on first failure')
    pool.push('Add execution time estimation — show "~10s" or "~30s" based on file count and complexity')
  }

  // If no file-specific suggestions, generate based on the edit content
  if (pool.length === 0) {
    pool.push('Add unit tests for the functions you just modified')
    pool.push('Add input validation to exported functions — guard against undefined/null parameters')
    pool.push('Add structured logging (console.log with [Tag]) for easier debugging of this module')
  }

  // Shuffle and pick 3 (so repeated edits don't always show the same suggestions)
...
- Keep the response focused on the configuration request only`
      ...
      if (isSelfEdit) {
        ...
        if (selfEditTarget.path) {
          ...
          // "All Core System" selected — provide file index
          ...
          targetDesc = `## AVAILABLE TARGET FILES\nThe user selected "All Core System". Identify which file(s) to edit based on their request, then call \`update_files\` for each.\n\n${fileIndex}\n\nIf the user's request maps to a specific file above, edit ONLY that file. If it spans multiple files, edit each one separately.`
        }

        systemMessage = `You are a **code editor** for the Emanator AI builder platform. You are NOT building a website or app. You are editing Emanator's own internal source code to improve how it works.

## YOUR ROLE
You modify existing source files when the user describes improvements. You do NOT create new files, build websites, generate UI components, or produce previews. No images. No HTML pages. No React components. Just targeted code edits to the files listed below.

## CONVERSATIONAL AWARENESS
If the user sends a greeting, question, compliment, or casual message (not a code edit request), respond conversationally. Be friendly, warm, and helpful. You are Emanator — an AI builder platform. You have personality. Introduce yourself, answer questions about your capabilities, and engage naturally. Do NOT force a code edit when the user is just talking to you.

${targetDesc}

## RULES (STRICT — USE patch_files TOOL)
You MUST use the \`patch_files\` tool (NOT \`update_files\` or \`create_files\`). Each file has a \`patches\` array of \`{ search, replace }\` pairs.
...
- To ADD new code after a line, set \`search\` to that line (plus context), and \`replace\` to that same line plus your new code appended below.
- To INSERT at end of file, search for the last few lines and replace with themselves plus the new code.
...
### OTHER RULES
1. NEVER use \`update_files\` or \`create_files\`. ONLY \`patch_files\`.
2. NEVER create new files. NEVER generate UI components, websites, or apps.
3. Preserve ALL existing functionality.
4. Do NOT mention "preview". Say: "Updated [filename]. Review the changes in the Code tab."
5. Do NOT say "file(s) written successfully" — the system adds its own summary.
6. Be ACTION-ORIENTED. Do NOT explain what you "would" do — just DO it. Call the tool immediately.
7. Do NOT lecture about approaches, strategies, or steps. The user wants code changes, not essays.
8. You are editing EMANATOR'S OWN CODEBASE — not a user's website project. Never confuse the two.
9. Never suggest editing API routes, database logic, or authentication flows — those are outside your scope.
10. After applying patches, briefly note what changed and suggest 1-2 related improvements the user could make next.`
...
- After your create_files call, follow up with a short conversational summary of what you built. Do NOT ask questions.`
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        // Force the AI to use patch_files in self-edit mode — BUT only for actual edit requests
        // Conversational messages (greetings, questions, thanks) should get text responses, not forced tool calls
        let selfEditIsConversational = false
        if (isSelfEdit) {
          const msgTrimmed = userMessage.trim().toLowerCase()
          const conversationalPatterns = [
            // Greetings
            /^(hey|hi|hello|howdy|yo|sup|greetings|good\s+(morning|afternoon|evening))/,
            // Thanks / praise
            /^(thanks|thank you|thx|great job|good job|nice|awesome|amazing|love it|looks? (good|great|amazing))/,
            // Questions about capabilities / identity
            /^(who are you|what (can|do) you|will you|can you|do you|are you|how do you|what('s| is) your)/,
            // Casual chat
            /^(how('s| is) it|what('s| is) up|just (wanted|checking|saying)|i('m| am)|my name|nice to meet)/,
            // Meta questions about memory/context
            /(remember|recall|forget|memory|history|previous (chat|conversation|session))/,
            // Short non-edit messages (< 30 chars with no code keywords)
          ]
          const isShortNonEdit = msgTrimmed.length < 40 && !msgTrimmed.match(/\b(add|change|edit|update|remove|delete|fix|modify|replace|insert|create|refactor|rename|move)\b/)
          selfEditIsConversational = conversationalPatterns.some(p => p.test(msgTrimmed)) || isShortNonEdit
          
          if (selfEditIsConversational) {
            console.log('[SelfEdit-Chat] Conversational message detected, skipping tool_choice forcing')
          } else {
        ...
        // Self-edit: only allow patch_files tool (unless conversational)
        if (isSelfEdit && !selfEditIsConversational) {
          effectiveToolSet = AI_TOOLS.filter(t => t.function?.name === 'patch_files')
        }
              ...
              if (toolName === 'propose_plan') {
                ...
                if (directEditMode || isSelfEdit || (isBriefBuild && existingFiles === 0)) {
                  ...
                  if (savedFiles.length > 0 && !fullContent) {
                    if (isBriefBuild) {
                      // PM-style summary for brief builds
                  ...
                  // ── Self-edit enhancement suggestions (always append, regardless of fullContent) ──
                  if (isSelfEdit && savedFiles.length > 0) {
                    ...
                    try {
                      ...
                      if (canvasContent && canvasContent.includes('## Recent Edits')) {
      ...
      // ── Self-edit completion summary ──
      if (isSelfEdit && generatedFiles.length > 0) {
        const fileList = generatedFiles.map(f => `\`${f.path}\``).join(', ')
        fullContent += `\n\nUpdated ${generatedFiles.length} file(s): ${fileList}.\nCheck the **Code** tab to review changes. Click **Apply to Live** to deploy them to the running system.`
        contentWasSanitized = true  // Force contentOverride so the UI shows the summary
      }
