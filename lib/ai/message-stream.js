...
- Keep the response focused on the configuration request only`
      ...
      // BYPASS: If the project has a creative brief, the user explicitly wants building — never chat-only.
      let projectManagerMode = false
      const hasCreativeBrief = context?.canvas?.creative_brief && (
        context.canvas.creative_brief.elevator_pitch || context.canvas.creative_brief.must_have_features
      )
      if (taskMode === 'build' && intent === 'build' && effectiveScope === 'project' && !directEditMode
          && isLargeAppBuild(userMessage) && !isProceedSignal(userMessage) && !hasCreativeBrief) {
      ...
      if (projectManagerMode) {
        systemMessage += buildProjectManagerPrompt()
      }
