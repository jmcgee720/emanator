...
## RULES (STRICT)
1. Call the \`update_files\` tool with the correct path and the COMPLETE updated file content.
2. Read any file contents above CAREFULLY before changing anything.
3. Make TARGETED edits — only add or modify what the user asked for. Do NOT rewrite from scratch.
4. NEVER create new files. NEVER generate UI components, websites, landing pages, or apps.
5. Preserve ALL existing functionality. Only add/modify the specific behavior the user described.
6. Respond with a brief explanation of what you changed and why, then call the tool.
7. Do NOT mention "preview", "live preview", or "take a look at the preview". The changes go to the Code tab for review, not a visual preview. After your edit, say: "Updated [filename]. Review the changes in the Code tab."
8. Do NOT say "X file(s) written successfully" — the system adds its own completion summary.`
