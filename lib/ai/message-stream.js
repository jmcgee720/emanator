...
## RULES (STRICT — PATCH FORMAT)
You MUST use the \`update_files\` tool. But instead of rewriting the entire file, use the **PATCH FORMAT** below in the \`content\` field. The server will merge your patches into the original file.

### PATCH FORMAT
Your \`content\` field MUST start with \`<<<PATCHES>>>\` followed by one or more search/replace blocks:
\`\`\`
<<<PATCHES>>>
<<<SEARCH>>>
exact existing code to find (copy-paste from the file above)
<<<REPLACE>>>
the replacement code (modified version of the search block)
<<<END>>>
\`\`\`

### PATCH RULES
- Each \`<<<SEARCH>>>\` block must contain EXACT text from the current file (whitespace-sensitive).
- To ADD new code, use a \`<<<SEARCH>>>\` block that matches a nearby line, then include the original line PLUS your new code in \`<<<REPLACE>>>\`.
- To INSERT at the end of the file, search for the last few lines and replace them with themselves plus the new code.
- You can include MULTIPLE search/replace blocks in one content field.
- NEVER output the complete file. ONLY output patches.
- Keep patches small and focused — only the lines that change plus ~2 lines of surrounding context.

### OTHER RULES
1. Read the file contents above CAREFULLY before writing patches.
...
5. Do NOT say "X file(s) written successfully" — the system adds its own summary.`
          ...
          for (const toolCall of toolCalls) {
            try {
              ...
              if (toolName === 'propose_plan') {
                ...
                // ── Direct Edit Mode OR Brief Build: save files immediately, skip diff pipeline ──
                if (directEditMode || (isBriefBuild && existingFiles === 0)) {
                  ...
                  for (const f of savedFiles) {
                    generatedFiles.push(f)
                    yield { event: 'file', data: { path: f.path, action: toolName === 'update_files' ? 'updated' : 'created' } }
                  ...
                  console.log(directEditMode ? '[DirectEdit]' : '[BriefBuild]', 'Saved', savedFiles.length, 'file(s):', savedFiles.map(f => f.path).join(', '))
