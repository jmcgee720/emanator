...
async function* runNewBriefPipeline({ aiService, projectId, chatId, userId, userMessage, attachments, runId, startTime }) {
  ...
  if (imageAttachments.length > 0) {
    ...
    imageAssets = imageAttachments.slice(0, 4).map((att, idx) => {
      ...
      else if (idx === 0 && imageAttachments.length <= 2) role = 'logo' // first image is usually the logo when few uploads
  ...
  if (imageAssets.length > 0) {
    try {
      const lines = imageAssets.map((a) => {
        ...
        // Escape backticks + dollar signs in the data URL for safety inside a template literal
        const safe = a.dataUrl.replace(/`/g, '\\`').replace(/\$/g, '\\$')
        return `export const ${exportName} = \`${safe}\``
      })
      const assetsContent = `// AUTO-GENERATED by Emanator from user-uploaded reference images.\n// Reference as: import { LOGO_URL, HERO_URL, REFERENCE_0 } from '../components/assets'\n// DO NOT EDIT — regenerated on every build.\n\n${lines.join('\n\n')}\n`
