...
Most important: ${mostImportantPage} — build this with the MOST detail (mock data, charts, tables, stats cards)

Use \`useState('home')\` for navigation. Navbar links and ALL buttons/CTAs must use the \`onNavigate\` function to switch pages. Every "Get Started", "Learn More", "Try Free" button should navigate to a relevant page.
...
## ${mostImportantPage.toUpperCase()} PAGE (MOST DETAIL)
Since "${mostImportantPage}" is the most important page, build it as a REAL app interface, not a marketing page:
- Stats overview row: 4 glass cards showing metrics SPECIFIC to ${brandName}: ${projectDesc.includes('marketing') ? '"Active Campaigns: 24", "Total Reach: 1.2M", "Conversion Rate: 4.8%", "Ad Spend: $12,400"' : '"Total Users: 12,847", "Revenue: $34,200", "Growth: +24.5%", "Satisfaction: 89%"'}
- Each stat card: number in large bold text, label below, small green/red percentage change indicator
- A mock chart area: use an SVG with bars or a line path to simulate a real chart (simple colored rectangles or polyline — NOT just a gradient div)
- A data table with 5-6 rows of realistic data with columns (Name, Status, Metric, Date, Actions)
- Filter bar at top: 2-3 filter buttons + a search input
- This page must feel like opening a REAL SaaS dashboard
...
- Section heading pattern: \`<div className="text-center mb-16"><p className="text-sm text-violet-400 font-medium mb-4">SECTION LABEL</p><h2 className="text-3xl md:text-4xl font-bold">Section Title</h2></div>\`
...
- Call \`update_files\` with the COMPLETE modified file content.`
      ...
      // GPT-4o has 128k tokens but we need ~16k for output. Safe input limit: ~100k tokens (~400k chars).
      // If we exceed this, drop oldest messages (keep system + last 10 turns + current user message).
      const totalChars = messages.reduce((s, m) => s + (m.content?.length || 0), 0)
      const MAX_CONTEXT_CHARS = 350000 // ~87k tokens, leaves plenty for output
      if (totalChars > MAX_CONTEXT_CHARS) {
        console.log(`[ContextTrim] Total ${totalChars} chars exceeds ${MAX_CONTEXT_CHARS} — trimming old messages`)
        const systemMsg = messages[0]
        const userMsg = messages[messages.length - 1]
        const history = messages.slice(1, -1)
        // Keep the last 10 messages (5 turns of user/assistant)
        ...
        // Add a note so the AI knows context was trimmed
        if (trimmed.length > 0) {
