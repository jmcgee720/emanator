              ...
              if (toolName === 'propose_plan') {
                ...
                if (editResult.success) {
                  ...
                  if (isSelfEdit) {
                    canvasUpdated = true
                    try {
                      const healthRes = await fetch('http://localhost:3000/api/health', { signal: AbortSignal.timeout(8000) })
                      if (!healthRes.ok) {
                        // Build broke — auto-revert
                        ...
                        // editResult.originalContent has the pre-edit content
                        if (editResult.originalContent) {
                          ...
                          editResult.errors.push('BUILD BROKEN: The edit caused a compilation error. File was auto-reverted to the previous version. Re-read the file with read_files, then try a smaller, safer edit.')
                        }
                      } else {
                        console.log(`[edit_lines] Build verified OK after editing ${args.path}`)
                      }
                    } catch (verifyErr) {
                      // Server might be restarting — wait and retry once
                      await new Promise(r => setTimeout(r, 3000))
                      try {
                        const retryRes = await fetch('http://localhost:3000/api/health', { signal: AbortSignal.timeout(8000) })
                        if (!retryRes.ok) {
                          ...
                          if (editResult.originalContent) {
                            fs.writeFileSync(fullPath, editResult.originalContent, 'utf-8')
                            editResult.success = false
                            editResult.errors.push('BUILD BROKEN: Compilation failed after edit. File was auto-reverted.')
                          }
                        }
                      } catch {}
                    }
