...
Read the color direction carefully. If it says "violet to blue gradients" → use \`from-violet-500 to-blue-500\`. If it says "no white backgrounds" → never use bg-white. Match the mood exactly.
...
Use \`useState('home')\` for navigation. Navbar links switch pages. Each page is a separate function component.
...
- A unique inline SVG icon (different for each feature — use paths, not emoji)
- The actual feature name as the card title
- A 2-sentence description explaining what it does for the user
...
Include:
- Small pill badge above headline (\`rounded-full bg-violet-500/10 border border-violet-500/20 px-4 py-1.5 text-sm\`)
- Large gradient headline (\`text-5xl md:text-7xl font-bold bg-gradient-to-r bg-clip-text text-transparent\`)
- 1-2 line subtitle in muted text
- Two CTA buttons: primary (filled, glow shadow) and secondary (glass/outline)
...
- Left: SVG logo icon + "${brandName}" text
- Center/Right: show MAX 5 nav links (pick the most important pages). If there are more pages, group the rest under a "More" dropdown or just omit lesser pages from the nav.
- Far right: CTA button ("Get Started" or "Login")
- The SVG logo must be a UNIQUE design — a simple geometric shape with gradient fill, not just text

## FOOTER
Build a REAL footer with 4 columns in a grid:
\`\`\`
| Brand + tagline  | Product links    | Resources       | Legal/Contact    |
| description      | (page links)     | Blog, Docs,     | Privacy, Terms   |
| social icons     |                  | Help Center     | Email, Twitter   |
\`\`\`
- Use \`grid grid-cols-2 md:grid-cols-4 gap-8\`
- Brand column: "${brandName}" + 1-line tagline + row of social SVG icons (Twitter, LinkedIn, GitHub)
- Product column: link list of the main pages/features
- Resources column: Blog, Documentation, Help Center, API Reference
- Legal column: Privacy Policy, Terms of Service, contact email
- Bottom bar: "© 2026 ${brandName}. All rights reserved."
- Background: slightly lighter than page (\`bg-gray-900/50 border-t border-white/[0.06]\`)

## ${mostImportantPage.toUpperCase()} PAGE (MOST DETAIL)
Since "${mostImportantPage}" is the most important page, build it with:
- Stats overview row: 4 glass cards showing metrics (e.g., "12,847 Visitors", "+24.5%", "$34,200 Revenue", "89% Satisfaction")
- A mock chart area (use a div with gradient background simulating a chart, or SVG bars/lines)
- A data table with 5-6 realistic rows (e.g., campaigns, products, or whatever fits the brand)
- Action buttons and filters
- This page should feel like a REAL working dashboard/app, not a marketing page

## PRICING SECTION (if Pricing is a page)
3-tier pricing cards in a row:
- Starter (free/$0), Pro ($29-49/mo), Enterprise (custom)
- Each card: glass background, feature checkmark list (6+ items), CTA button
- Pro card highlighted: slightly brighter border, "Popular" badge, primary-colored CTA
- Use actual feature names from the brief for the checklist items

## QUALITY BAR
- 800-1100 lines target. Under 600 = lazy, rejected.
- Every section must have REAL content — actual feature names, actual descriptions, actual pricing tiers.
- ZERO placeholder text. No "Feature 1", "Description of feature", "Lorem ipsum", "Your Company".
- Every interactive element needs hover states (\`hover:bg-white/[0.06] transition-all duration-300\`)
