...
import { verifyBuild, formatVerifyForRepairPrompt, findingsToReviewShape } from './screenshot-verify.js'
...
async function* runNewBriefPipeline({ aiService, projectId, chatId, userId, userMessage, attachments, runId, startTime }) {
  ...
  if (allSavedFiles.length > 0) {
    ...
    // ── Step 5c: Screenshot-verify (Session 28, 4/7) ──
    // Non-blocking Vision comparison: the generated landing + navbar
    // source files are sent to GPT-4o alongside the user's reference
    // images. Emits structured {matches, findings[]} that the repair
    // loop (Session 29) will consume. For now we surface it through the
    // observatory so users can see "Vision says palette matches but
    // hero composition is off." as a single quality signal.
    const __verifyT = Date.now()
    if (plan.referenceImages?.length > 0) {
      try {
        ...
        if (verifyResult) {
          ...
          yield { event: 'screenshot_verify', data: verifyResult }
          console.log(
            '[NewPipeline] Screenshot-verify:',
            verifyResult.matches ? 'MATCH' : `${verifyResult.findings.length} finding(s)`,
            `(confidence ${Math.round(verifyResult.confidence * 100)}%)`,
          )
        }
      } catch (err) {
        console.warn('[NewPipeline] Screenshot-verify skipped:', err.message)
      }
    }
    tick('screenshot_verify', __verifyT)

    // ── Step 5d: Visual-diff repair loop (Session 29, 5/7) ──
    // When the verifier flagged mismatches, synthesize a review-shaped
    // payload from the findings and run the existing repairBuild() wave
    // once. The repair LLM receives the user's reference images (via
    // plan.referenceImages) + the per-file mismatch list + the
    // suggested fixes. Capped at 1 extra round here to keep Vision cost
    // predictable — a future iteration can widen this to N rounds with
    // a re-verify between rounds.
    const __visualRepairT = Date.now()
    const verifyOutcome = plan.verifyResult
    if (verifyOutcome && !verifyOutcome.matches && verifyOutcome.findings?.length > 0) {
      try {
        const dbFilesForRepair = await db.projectFiles.findByProjectId(projectId)
        const repairFiles = (dbFilesForRepair || []).map((f) => ({
          path: f.path, content: f.content || '', file_type: f.file_type,
        }))
        const synthesizedReview = findingsToReviewShape(verifyOutcome)
        if (synthesizedReview.broken.length > 0) {
          yield { event: 'status', data: { stage: 'visual_repair', detail: `Repairing ${synthesizedReview.broken.length} visual mismatch(es)...` } }
          const repairGen = repairBuild({
          ...
          console.log('[NewPipeline] Visual repair:', repairResult.filesRepaired.length, 'file(s)')
          yield { event: 'visual_repair_complete', data: { filesRepaired: repairResult.filesRepaired } }
        }
      } catch (err) {
        console.warn('[NewPipeline] Visual repair skipped:', err.message)
      }
    }
