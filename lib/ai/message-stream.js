...
- After your create_files call, follow up with a short conversational summary of what you built. Do NOT ask questions.`
      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        // selfEditIsConversational is computed early (before system prompt) at line ~543
        if (isSelfEdit) {
          if (selfEditIsConversational) {
            console.log('[SelfEdit-Chat] Conversational mode — no tool_choice forcing')
          } else {
            // Use 'auto' instead of forcing — AI can respond with text when the user asks
            // questions, but patch_files is the ONLY tool available so it will use it for edits
