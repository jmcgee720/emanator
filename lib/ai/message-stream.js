      ...
      if (toolMode === 'chat_only' && !usePlanMode) {
        ...
        if (isSelfEdit) {
          if (selfEditIsConversational) {
            ...
            const isStatusQuestion = /\b(is .+ done|what happened|what.?s the status|did (it|you)|how did|was that|is that|what should|applied to live|good next step)\b/.test(msgLower)
            const isCanvasRequest = /\b(update.*(canvas|checklist)|mark.*(done|complete)|check.?off|canvas.*(update|edit|change))\b/.test(msgLower)
