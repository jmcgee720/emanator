                  ...
                  if (isSelfEdit && savedFiles.length > 0) {
                    ...
                    try {
                      ...
                      for (const f of savedFiles) {
                        ...
                        backups.push({ path: fullPath, content: backup })
                      ...
                      if (healthRes.ok) {
                        buildStatus = 'Build verified — changes compile successfully.'
                        console.log(`[SelfEdit] Build verified OK for ${shortFiles}`)
                      } else {
                        // Build failed — revert disk files
                        let buildError = ''
                        try {
                          const errLog = fs.readFileSync('/var/log/supervisor/nextjs_api.err.log', 'utf-8')
                          const errLines = errLog.split('\n')
                          const lastErrIdx = errLines.findLastIndex(l => l.includes('Error') || l.includes('Syntax') || l.includes('Expected'))
                          if (lastErrIdx >= 0) {
                            buildError = errLines.slice(Math.max(0, lastErrIdx - 3), Math.min(errLines.length, lastErrIdx + 8)).join('\n')
                          }
                        } catch {}
                        for (const b of backups) {
                          if (b.content != null) fs.writeFileSync(b.path, b.content, 'utf-8')
                        }
                        buildStatus = `BUILD FAILED — changes were reverted.\n\nError:\n\`\`\`\n${buildError}\n\`\`\``
                        console.error(`[SelfEdit] Build FAILED for ${shortFiles} — reverted`)
                      }
                    } catch (verifyErr) {
                      buildStatus = 'Build verification skipped (server restarting).'
                    }
