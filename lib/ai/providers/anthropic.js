 ...
 */
/**
 * Direct call to Anthropic API (non-streaming, simple wrapper)
 * Used by fork-summary and other utilities that need a quick one-shot call
 */
export async function callAnthropicDirect({ model, max_tokens, messages, system }) {
  ...
  _convertMessages(messages, { cacheSystem = true } = {}) {
    ...
    for (const msg of messages) {
      ...
      if (msg.role === 'tool') {
        ...
        //
        // Content shapes we accept:
        //   • string  → wrap as Anthropic text content
        //   • array   → pass through as-is; lets tools return vision
        //              blocks (e.g. screenshot_preview returns
        //              [{type:'text', text:...}, {type:'image', source:...}])
        //              so Claude can SEE captured pixels.
        //   • other   → JSON.stringify (legacy fallback)
        let toolContent
