...
export async function saveFiles(projectId, files, isUpdate, provider, prefetchedImages = []) {
  ...
  // NOTE: Generated image placeholders (emanator-generated.img) are intentionally NOT replaced here.
  // They stay as short URLs in source files. The preview resolves them to actual data URLs at render time.
  if (prefetchedImages.length > 0) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // Replace generic placeholders with stock photos (only non-data-URL stock photos)
      const replaced = original.replace(placeholderPattern, () => {
        const img = prefetchedImages[imgIndex % prefetchedImages.length]
        imgIndex++
        // For stock photos use their URL; for generated use placeholder
    ...
    // Save generated images as separate asset files (NOT embedded in source code)
    for (const img of prefetchedImages) {
      if (img._placeholderUrl && img.url?.startsWith('data:')) {
        const assetFilename = img._placeholderUrl.split('/').pop() // e.g., __gen_img_1.png
        const assetPath = `_assets/${assetFilename}`
        files.push({
          path: assetPath,
          content: img.url, // data:image/png;base64,...
          file_type: 'image',
        })
        console.log(`[ImagePostProcessor] Saved generated image as asset: ${assetPath} (${Math.round(img.url.length / 1024)}KB)`)
      }
    }
  }
