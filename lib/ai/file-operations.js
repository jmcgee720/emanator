...
export async function saveFiles(projectId, files, isUpdate, provider, prefetchedImages = []) {
  ...
  if (prefetchedImages.length > 0) {
    ...
    // These are saved ASYNCHRONOUSLY (fire-and-forget) to avoid blocking the SSE stream.
    // The frontend gets image data via the SSE generated_images_map event (sent earlier),
    // and _assets/ files provide persistence across page reloads.
    const generatedImages = prefetchedImages.filter(img => img._placeholderUrl && img.url?.startsWith('data:'))
    for (const img of generatedImages) {
      const filename = img._placeholderUrl.split('/').pop()
      const assetPath = `_assets/${filename}`
      assetFilesToSave.push({
        path: assetPath,
        content: img.url,
        file_type: 'image',
  ...
  // Save _assets/ image files ASYNCHRONOUSLY — don't block the SSE stream
  // The frontend already has image data via the generated_images_map SSE event.
  // These DB entries provide persistence across page reloads.
  if (assetFilesToSave && assetFilesToSave.length > 0) {
    const assetSavePromise = Promise.all(assetFilesToSave.map(async (assetFile) => {
      try {
        const existing = await db.projectFiles.findByPath(projectId, assetFile.path)
        if (existing) {
          await db.projectFiles.update(existing.id, {
            content: assetFile.content,
            file_type: assetFile.file_type
          })
          console.log(`[ImageAssets] Updated ${assetFile.path} (${Math.round(assetFile.content.length / 1024)}KB)`)
        } else {
          await db.projectFiles.create({
            project_id: projectId,
            path: assetFile.path,
            content: assetFile.content,
            file_type: assetFile.file_type || 'image',
            version: 1,
            change_source: 'ai_generation'
          })
          console.log(`[ImageAssets] Saved ${assetFile.path} (${Math.round(assetFile.content.length / 1024)}KB)`)
        }
      } catch (err) {
        console.error(`[ImageAssets] Failed to save ${assetFile.path}:`, err.message)
      }
    }))
    // Fire-and-forget: don't await, just log when done
    assetSavePromise.then(() => {
      console.log(`[ImageAssets] All ${assetFilesToSave.length} asset(s) saved in background`)
      invalidateCache(projectId)
    }).catch(err => {
      console.error('[ImageAssets] Background save failed:', err.message)
    })
  }
