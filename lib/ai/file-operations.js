...
export async function saveFiles(projectId, files, isUpdate, provider, prefetchedImages = []) {
  ...
  // ── Placeholder Image Replacement — swap placeholder URLs with real stock/generated photos ──
  if (prefetchedImages.length > 0) {
    // Build a map of generated image placeholder URLs → actual base64 data URLs
    const genImageMap = new Map()
    for (const img of prefetchedImages) {
      if (img._placeholderUrl && img.url?.startsWith('data:')) {
        genImageMap.set(img._placeholderUrl, img.url)
      }
    }
    ...
    // Also match generated image placeholder URLs
    const genPlaceholderPattern = /https:\/\/emanator-generated\.img\/__gen_img_\d+\.png/g
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // Replace generic placeholders with stock photos
      let replaced = original.replace(placeholderPattern, () => {
      ...
      // Replace generated image placeholders with actual base64 data URLs
      if (genImageMap.size > 0) {
        replaced = replaced.replace(genPlaceholderPattern, (match) => {
          return genImageMap.get(match) || match
        })
      }

      if (replaced !== original) {
        const genCount = [...original.matchAll(genPlaceholderPattern)].length
        const stockCount = imgIndex
        console.log(`[ImagePostProcessor] Replaced ${genCount} generated + ${stockCount} stock placeholder(s) in ${files[i].path}`)
