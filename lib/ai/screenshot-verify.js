...
/**
 * Given a Vision verify result, decide whether the loop should continue
 * into another repair round. Pure function — separates the decision from
 * the pipeline's I/O so the stop conditions can be unit-tested.
 *
 * Stop when:
 *   - verifyResult is null (no verdict available)
 *   - verifyResult.matches === true (Vision says we're done)
 *   - verifyResult.findings.length === 0 (Nothing concrete to repair)
 *   - round >= maxRounds - 1 (we'd have no re-verify pass after this repair)
 *
 * @param {import('./screenshot-verify.js').VerifyResult|null} verifyResult
 * @param {number} round - 0-indexed round number
 * @param {number} maxRounds - cap from config (default 3)
 * @returns {{stop: boolean, reason: string}}
 */
export function shouldContinueVisualLoop(verifyResult, round, maxRounds) {
  if (!verifyResult) return { stop: true, reason: 'no-verdict' }
  if (verifyResult.matches) return { stop: true, reason: 'matches' }
  if (!Array.isArray(verifyResult.findings) || verifyResult.findings.length === 0) {
    return { stop: true, reason: 'no-findings' }
  }
  if (round >= maxRounds - 1) return { stop: true, reason: 'max-rounds' }
