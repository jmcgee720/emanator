...
const REFINEMENT_PATTERNS = [
  ...
  /\b(remove|delete|hide|get\s*rid\s*of|drop|strip|kill)\b.{0,30}\b(the\s+)?(section|button|image|text|header|footer|nav|sidebar|card|border|shadow|background|element|block|row|column|icon|link|spacing)\b/i,
  // "move X to the top/bottom/left/right"
  /\b(move|shift|reorder|rearrange|relocate|reposition|swap)\b.{0,40}\b(to\s+the\s+)?(top|bottom|left|right|center|above|below|before|after)\b/i,
  ...
  /\b(use|try|switch\s+to|go\s+with)\b.{0,20}\b(a\s+)?(different|new|another|better)\s+(color|font|style|layout|theme|background|palette|scheme|design|approach)\b/i,
  // Direct style instructions: "center the text", "align left", "set bg to..."
  /\b(center|align|set|apply|give\s+it)\b.{0,30}\b(the\s+)?(text|heading|title|content|background|bg|border|shadow|color|font|style|theme|padding|margin)\b/i,
]
