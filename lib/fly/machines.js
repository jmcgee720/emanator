...
export function isMachineConfigStale(machine) {
  ...
  // Also flag machines whose port-443 service handler chain still includes
  // the broken-for-WebSocket-Upgrade `http` handler. Vite HMR over WSS
  // 502s through Fly's HTTP handler — we now use `['tls']` only and let
  // our runner's Node HTTP server handle the upgrade. Existing pre-fix
  // machines must be recycled so they pick up the new service config.
  ...
  for (const svc of services) {
    ...
    for (const p of svc?.ports || []) {
      if (p?.port === 443) {
        ...
        // Stale if handlers includes 'http' (the broken-for-WS chain).
        // Fresh config has only ['tls'].
...
// Detect machines running an OLDER runner image — e.g. after a
// `preview-runner` deploy bumps the image SHA. Fly Machines pin to a
// specific image at create time and do NOT auto-update when a new
// version is pushed, so without this check, deployed runner-side
// fixes (e.g. Vite alias injection, CRA compile-ready probe) never
// reach existing user preview machines until the user clicks "Hard
// Reset" — which is unacceptable UX after we ship a bugfix.
//
// `deployedImage` is the canonical "what should we be running right
// now?" image, fetched via resolveDeployedImage(). We compare it to
// the machine's current `config.image`. If they differ, the start
// route will destroy + recreate the machine so the new image (and
// therefore the new runner code) takes effect on the next preview boot.
//
// We split this into a pure function so the start route can decide
// whether to recreate, and so we can unit-test the comparison logic
// without hitting the Fly API.
export function isMachineImageStale(machine, deployedImage) {
  const current = machine?.config?.image
  if (!current || !deployedImage) return false // can't compare → safer not to recycle
...
// In-place env update for an existing machine. This is the disk-preserving
// alternative to destroyMachine(): Fly's Machine Update API rolls the
// machine through stop → reconfigure → start while keeping the rootfs
// (and therefore node_modules, .auroraly-install-hash, .next caches)
// intact. We use this when the only thing the machine needs is updated
// runtime env (e.g. SUPABASE_URL was added to createMachineForProject
// after this machine was first provisioned). Saves the user a full
// reinstall on every config tweak.
//
// Caveats:
//   - This DOES restart the user's dev server (the machine cycles)
//   - It does NOT pull a fresh image — image-only updates still need
//     destroy/recreate. Use isMachineImageStale() for that case.
export async function updateMachineEnv(machineId, machine, additionalEnv) {
  const { app } = getEnv()
  const newEnv = { ...(machine?.config?.env || {}), ...additionalEnv }
  // Fly requires the FULL config in the update body — partial-merge
  // is not supported. We send back the machine's existing config with
  // only the env replaced.
  const updateBody = {
    config: { ...machine.config, env: newEnv },
  }
...
// Build the env block we'd inject into a FRESH machine right now. Used
// by the in-place update path to figure out which keys are missing on
// an older machine and inject them. Keep this in lockstep with the
// `env: { ... }` block in createMachineForProject().
      ...
      services: [
        {
          ports: [
            ...
            // CRITICAL: handler chain is `['tls']` (TLS termination only),
            // NOT `['tls', 'http']`. The `http` handler in Fly's edge
            // multiplexes via HTTP/2 internally and breaks WebSocket
            // Upgrade requests with a 502 Bad Gateway. Vite's HMR relies
            // on WSS over this port, so without this fix the user sees an
            // infinite "[vite] server connection lost. Polling for
            // restart…" loop → page reloads → blank iframe. Letting our
            // runner's Node HTTP server handle protocol parsing directly
            // is the standard Fly recommendation for WS-heavy apps.
        ...
        {
          // Control plane (orchestrator → runner). Kept on the full
          // ['tls','http'] handler chain because we don't run WebSockets
          // over this port — all traffic is plain HTTP JSON.
