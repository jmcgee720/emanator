...
async function listMachinesInApp(appName) {
  try {
    const { res, body } = await flyFetch(`/apps/${appName}/machines`)
    if (!res.ok) {
      // 404 → app doesn't exist yet; treat as "no machines".
      if (res.status === 404) return []
      throw new Error(
        `fly GET /apps/${appName}/machines → ${res.status}: ` +
        (typeof body === 'string' ? body : JSON.stringify(body))
      )
    }
    if (!Array.isArray(body)) return []
    return body.map(m => ({ ...m, _appName: appName }))
  } catch (err) {
...
export async function createMachineForProject(projectId, sharedSecret) {
  ...
  // Try to attach a persistent volume so /project/node_modules survives
  // machine destroy/recreate. If Fly rejects the volume (region capacity
  // hiccups, org quota, transient API errors) we DO NOT block the boot —
  // fall back to an ephemeral rootfs machine so users still get a working
  // preview (just slower cold-boots). This is the safer default; adding
  // a hard dependency on volume creation would turn every transient Fly
  // API glitch into a 500 on the preview-start path.
  let volumeId = null
  try {
  ...
  // Fly 412 "insufficient resources to create new machine with existing
  // volume 'vol_…'" means the volume was placed in an availability zone
  // that has no machine capacity right now. This is common when a volume
  // survives from a previous session in a zone that Fly has since drained.
  // Recovery: destroy the volume, drop the mounts field, retry once. The
  // user loses the node_modules cache for this boot but gets a working
  // preview.
  if (!created.res.ok && created.res.status === 412 && volumeId) {
    const errMsg = typeof created.body === 'string' ? created.body : JSON.stringify(created.body)
    if (/existing volume/i.test(errMsg)) {
      console.warn(
        `[createMachineForProject] Fly 412 for volume ${volumeId} — deleting and retrying without volume. Original error: ${errMsg}`
      )
      try {
      ...
      if (!retried.res.ok) {
        throw new Error(
          `fly POST /apps/${appName}/machines (retry without volume) → ${retried.res.status}: ` +
          (typeof retried.body === 'string' ? retried.body : JSON.stringify(retried.body))
        )
      }
      return { ...retried.body, _appName: appName }
    }
  }

  if (!created.res.ok) {
    throw new Error(
      `fly POST /apps/${appName}/machines → ${created.res.status}: ` +
      (typeof created.body === 'string' ? created.body : JSON.stringify(created.body))
    )
  }
  return { ...created.body, _appName: appName }
}
