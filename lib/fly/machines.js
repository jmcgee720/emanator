...
export async function createMachineForProject(projectId, sharedSecret) {
  ...
  // Ensure the project's persistent volume exists — mounted at /project
  // so npm install artifacts (node_modules, .npmrc, .auroraly-install-hash)
  // survive machine destroy/recreate cycles. Without this, every cold
  // boot after a destroy runs a full 3-6+ min reinstall.
  const volumeId = await ensureProjectVolume(appName, region)
  ...
  const body = {
    ...
    config: {
      ...
      // Mount the per-project volume at /project. The runner writes
      // user files here on sync and runs `npm install` here too, so
      // both source AND node_modules persist across machine lifecycle.
      // On first boot the volume is empty and the runner's sync
      // populates it from the DB (same as before). On every subsequent
      // boot the sync is diff-based ("wrote 0 changed, skipped 109
      // identical") and the install-hash cache-hit skips npm install
      // entirely → dev server up in <10 seconds.
      mounts: [
        {
          volume: volumeId,
          path: '/project',
