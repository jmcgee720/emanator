...
import { ensurePreviewApp, ensureProjectVolume, deleteProjectVolume, previewAppName } from './apps.js'
...
import { flyFetchOrThrow as flyFetch } from './fly-api.js'
// This file uses the "throws on !ok, returns body" contract. Aliased
// to `flyFetch` so we don't have to rename every caller in this file.

// ─── machine lookup ──────────────────────────────────────────────────
async function listMachinesInApp(appName) {
  try {
    // flyFetch throws on !res.ok and returns the parsed body on success.
    // Fly's /machines endpoint returns a JSON array of machine objects
    // directly, so we can map over it as-is.
...
export async function createMachineForProject(projectId, sharedSecret) {
  ...
  // Persistent Fly volume for /project is DISABLED right now. First
  // rollout of this feature triggered filesystem ENOENT errors during
  // `npm install` inside the volume:
  //   ENOENT: no such file or directory, mkdir '/project/node_modules/.bin'
  // The root cause looks like a race between the mount and the runner's
  // initial writes, but the crash-loops it created blocked EVERY user's
  // preview. Reverting to ephemeral rootfs restores working previews
  // (slower cold boots, but working) while we investigate the mount
  // interaction offline. Re-enable by flipping VOLUME_CACHE_ENABLED to
  // true once we have a repro fixture.
  const VOLUME_CACHE_ENABLED = false
  let volumeId = null
  if (VOLUME_CACHE_ENABLED) {
    try {
      volumeId = await ensureProjectVolume(appName, region)
    } catch (err) {
      console.warn(
        `[createMachineForProject] volume creation failed for ${appName}, ` +
        `booting WITHOUT persistent volume (cold boots will be slower). ` +
  ...
  const body = {
    ...
    config: {
      ...
      // Mount the per-project volume at /project ONLY when volumeId
      // resolved successfully above. If it didn't (transient Fly API
      // error), we omit the mounts field entirely — machine boots on
      // the ephemeral rootfs and users get a normal-but-slow-cold-boot
      // preview. Better UX than a hard 500.
      ...(volumeId ? {
        mounts: [
          {
            volume: volumeId,
  ...
  // flyFetch throws on !res.ok and returns the parsed body on success.
  // Machine creation returns the newly-created machine object directly.
  //
  // We catch the specific 412 "insufficient resources … existing volume"
  // failure so we can auto-recover: delete the stuck volume, then retry
  // the POST without any mount. User loses the node_modules cache for
  // this boot but gets a working preview instead of a hard 500.
  let created
  try {
    created = await flyFetch(`/apps/${appName}/machines`, {
      method: 'POST',
      body: JSON.stringify(body),
    })
  } catch (err) {
    const msg = err?.message || String(err)
    if (volumeId && / → 412:/.test(msg) && /existing volume/i.test(msg)) {
      console.warn(
        `[createMachineForProject] Fly 412 for volume ${volumeId} — deleting and retrying without volume. Original error: ${msg}`
      )
      try {
        await deleteProjectVolume(appName, volumeId)
      } catch (delErr) {
        console.warn(`[createMachineForProject] volume delete failed (continuing anyway): ${delErr?.message}`)
      }
      // Retry without the mounts field. flyFetch will throw again if
      // the retry ALSO fails — that error surfaces to the user unchanged.
      const bodyNoVol = { ...body, config: { ...body.config } }
      delete bodyNoVol.config.mounts
      const retried = await flyFetch(`/apps/${appName}/machines`, {
        method: 'POST',
        body: JSON.stringify(bodyNoVol),
      })
      return { ...retried, _appName: appName }
    }
    throw err
  }
