 ...
 *
 * Uses the project's dedicated app's :8443 service which proxies
 * into the runner's :8080. No need for Fly-Force-Instance-Id since
 * each app has only one machine.
 */
export function machineControlUrl(machine) {
  ...
  if (typeof machine === 'object' && machine?._appName) {
    return { url: `https://${machine._appName}.fly.dev:8443`, headers: {} }
  }
