 ...
 * (~200ms); subsequent calls hit a cheap GET that returns immediately.
...
export async function ensurePreviewApp(projectId) {
  ...
  if (check.res.ok) return name
  if (check.res.status !== 404) {
    // Some other transient failure — fall through to create which will
    // give us a clearer error (or succeed if it was a flake).
  }
  ...
  if (create.res.ok || create.res.status === 201) return name
  // Race: another concurrent call created the app between our GET + POST.
  const msg = String(create.body?.error || create.body || '').toLowerCase()
  if (create.res.status === 422 && (msg.includes('taken') || msg.includes('already'))) return name
  if (create.res.status === 409) return name
  throw new Error(
