 ...
 * @param {string} projectId
 * @param {object} [opts]
 * @param {string[]} [opts.changedPaths] Paths of files just modified by
 *   the agent. If `package.json` is among them, the response sets
 *   `requiresRestart: true` so the caller can surface a "Stop → Start
 *   to install new deps" hint in the UI — the runner only runs
 *   `npm install` on cold boot, so a sync alone won't pick up new
 *   dependencies.
 * @returns {Promise<{notified: boolean, reason?: string, machineId?: string, requiresRestart?: boolean}>}
 */
export async function notifyPreviewOfFileChange(projectId, opts = {}) {
  if (!projectId) return { notified: false, reason: 'no-project-id' }

  const requiresRestart = Array.isArray(opts.changedPaths) && opts.changedPaths.some(
    (p) => typeof p === 'string' && /(^|\/)package\.json$/.test(p)
  )

  // If Fly isn't configured (local dev, test runs), no-op silently.
  if (!process.env.FLY_API_TOKEN || !process.env.FLY_PREVIEW_APP_NAME) {
    return { notified: false, reason: 'fly-not-configured', requiresRestart }
  }
  ...
  try {
    ...
    return { notified: false, reason: 'machine-lookup-failed: ' + (err?.message || 'unknown'), requiresRestart }
  }
  ...
  if (!machine) return { notified: false, reason: 'no-machine', requiresRestart }
  if (machine.state !== 'started') {
    return { notified: false, reason: 'machine-' + machine.state, machineId: machine.id, requiresRestart }
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { notified: false, reason: `runner-${res.status}: ${text.slice(0, 200)}`, machineId: machine.id, requiresRestart }
    }
    return { notified: true, machineId: machine.id, requiresRestart }
  } catch (err) {
    return { notified: false, reason: 'fetch-failed: ' + (err?.message || 'unknown'), machineId: machine.id, requiresRestart }
  }
}
