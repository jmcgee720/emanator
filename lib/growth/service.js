...
export const growthDb = {
  ...
  async getPages(userId) {
    ...
    const docs = await db.collection('growth_pages')
      .find({ user_id: userId })
      .sort({ created_at: -1 })
      .project({ _id: 1, user_id: 1, url: 1, created_at: 1, updated_at: 1, 'extracted_data.title': 1, 'extracted_data.meta_description': 1, 'extracted_data.word_count': 1, opportunities: 1, fixes: 1, drafts: 1, drafts_generated_at: 1 })
      .toArray()
...
export const personaDb = {
  ...
  async getPersonas(userId) {
    ...
    const docs = await db.collection('persona_profiles')
      ...
      .project({ _id: 1, user_id: 1, project_id: 1, name: 1, description: 1, interests: 1, platforms: 1, content_types: 1, performance_score: 1, created_at: 1 })
      .toArray()
    return docs.map(d => ({ id: d._id.toString(), ...d, _id: undefined }))
  },
