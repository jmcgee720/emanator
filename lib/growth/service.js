...
export const growthDb = {
  ...
  async getPages(userId) {
    ...
    const docs = await db.collection('growth_pages')
      .find({ user_id: userId })
      .sort({ created_at: -1 })
      .project({ _id: 1, user_id: 1, url: 1, created_at: 1, updated_at: 1, 'extracted_data.title': 1, 'extracted_data.meta_description': 1, 'extracted_data.word_count': 1, opportunities: 1, fixes: 1 })
      .toArray()
