...
export const monitorDb = {
  async addMonitor(userId, data) {
    const db = await getDb()
    const existing = await db.collection('growth_monitors').findOne({ user_id: userId, url: data.url })
    if (existing) return { id: existing._id.toString(), ...existing, _id: undefined, already_exists: true }
    const doc = {
      user_id: userId,
      url: data.url,
    ...
    const result = await db.collection('growth_monitors').insertOne(doc)
    return { id: result.insertedId.toString(), ...doc, _id: undefined }
  },

  async getMonitors(userId) {
    const db = await getDb()
    const docs = await db.collection('growth_monitors')
      .find({ user_id: userId })
      .sort({ created_at: -1 })
      .project({ _id: 1, url: 1, label: 1, checks: 1, enabled: 1, created_at: 1, last_checked_at: 1, 'baseline.title': 1, 'baseline.score': 1, 'latest.title': 1, 'latest.score': 1, changes: 1, counter_moves: 1 })
      .toArray()
    return docs.map(d => ({ id: d._id.toString(), ...d, _id: undefined }))
  },
  ...
  async getMonitor(monitorId, userId) {
    const db = await getDb()
    let oid
    try { oid = new ObjectId(monitorId) } catch { return null }
    const doc = await db.collection('growth_monitors').findOne({ _id: oid, user_id: userId })
    if (!doc) return null
    return { id: doc._id.toString(), ...doc, _id: undefined }
  },

  async updateMonitorCheck(monitorId, userId, checkData) {
    const db = await getDb()
    let oid
    try { oid = new ObjectId(monitorId) } catch { return null }
    const update = {
      $set: {
        ...
        counter_moves: checkData.counter_moves || null,
        last_checked_at: new Date().toISOString(),
      },
      $inc: { checks: 1 },
    }
    // On first check, also set baseline
    const existing = await db.collection('growth_monitors').findOne({ _id: oid, user_id: userId })
    if (existing && !existing.baseline) {
      update.$set.baseline = checkData.latest
    }
    const result = await db.collection('growth_monitors').findOneAndUpdate(
      { _id: oid, user_id: userId }, update, { returnDocument: 'after' }
    )
    if (!result) return null
    return { id: result._id.toString(), ...result, _id: undefined }
  },
  ...
  async deleteMonitor(monitorId, userId) {
    const db = await getDb()
    let oid
    try { oid = new ObjectId(monitorId) } catch { return false }
    const result = await db.collection('growth_monitors').deleteOne({ _id: oid, user_id: userId })
    return result.deletedCount > 0
  },

  async toggleMonitor(monitorId, userId, enabled) {
    const db = await getDb()
    let oid
    try { oid = new ObjectId(monitorId) } catch { return false }
    const result = await db.collection('growth_monitors').updateOne(
      { _id: oid, user_id: userId }, { $set: { enabled } }
    )
    return result.modifiedCount > 0
  },
  ...
  async getSchedule(userId) {
    const db = await getDb()
    const doc = await db.collection('monitor_schedules').findOne({ user_id: userId })
    if (!doc) return null
    return { enabled: doc.enabled, frequency: doc.frequency, last_run: doc.last_run, next_run: doc.next_run }
  },

  async setSchedule(userId, config) {
    const db = await getDb()
    const nextRun = config.enabled ? computeNextRun(config.frequency) : null
    const result = await db.collection('monitor_schedules').findOneAndUpdate(
      { user_id: userId },
      { $set: { enabled: config.enabled, frequency: config.frequency, next_run: nextRun, updated_at: new Date().toISOString() }, $setOnInsert: { user_id: userId, last_run: null, created_at: new Date().toISOString() } },
      { upsert: true, returnDocument: 'after' }
    )
    return { enabled: result.enabled, frequency: result.frequency, last_run: result.last_run, next_run: result.next_run }
  },

  async getDueSchedules() {
    const db = await getDb()
    const now = new Date().toISOString()
    const docs = await db.collection('monitor_schedules')
      .find({ enabled: true, next_run: { $lte: now } })
      .toArray()
    return docs.map(d => ({ user_id: d.user_id, frequency: d.frequency, _id: d._id }))
  },

  async markScheduleRun(userId, frequency) {
    const db = await getDb()
...
function computeNextRun(frequency) {
  const now = Date.now()
  const durations = { '6h': 21600000, '12h': 43200000, '24h': 86400000, '48h': 172800000, '7d': 604800000 }
  const ms = durations[frequency] || 86400000
