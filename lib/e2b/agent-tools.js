...
export async function handleSearchReplace(args, { projectId, isSelfEdit } = {}) {
  ...
  for (let i = 0; i < edits.length; i++) {
    ...
    if (idx === -1) {
      ...
      // Found with normalized whitespace — find the actual position
      // Map normalized position back to original content
      let origPos = 0
      let normPos = 0
      const origLines = content.split('\n')
      const normLines = normalizedContent.split('\n')
      // Since we only trimmed trailing spaces, line count is the same
      // Find which line the normalized match starts on
      let charCount = 0
      let startLine = -1
      for (let li = 0; li < normLines.length; li++) {
        ...
        charCount += normLines[li].length + 1 // +1 for \n
      }
      // Use the original lines to reconstruct the match region
