...
/**
 * Auto-recover a not-found path by searching for its basename in /app.
 * Returns { exactMatch?: string, candidates: string[] }.
 *
 * Deterministic safety net for when the AI guesses a wrong path. If the
 * basename uniquely identifies a real file, we load it transparently. If
 * there are 2-5 candidates, we surface them. Skipped for ambiguous
 * basenames (too short or generic like "index.js" / "page.js").
 */
async function autoRecoverPath(reqPath) {
  const path = await import('path')
  const { execSync } = await import('child_process')
  const basename = path.basename(reqPath || '')
  if (!basename || basename.length < 3) return { candidates: [] }
  // Too-generic basenames produce noise; require AI to use exec_command instead
  const GENERIC = new Set(['index.js', 'index.ts', 'index.jsx', 'index.tsx', 'page.js', 'page.tsx', 'page.jsx', 'route.js', 'layout.js', 'README.md'])
  if (GENERIC.has(basename)) return { candidates: [] }
  try {
    // Shell-escape basename — replace any non-safe chars
    const safeBasename = basename.replace(/[^a-zA-Z0-9._-]/g, '')
    if (!safeBasename) return { candidates: [] }
    const cmd = `find /app -type f -name '${safeBasename}' -not -path '*/node_modules/*' -not -path '*/.next/*' -not -path '*/.git/*' -not -path '*/dist/*' -not -path '*/build/*' 2>/dev/null | head -6`
    const out = execSync(cmd, { encoding: 'utf-8', timeout: 4000 }).trim()
    const candidates = out ? out.split('\n').filter(Boolean) : []
    if (candidates.length === 1) return { exactMatch: candidates[0], candidates }
    return { candidates }
  } catch {
    return { candidates: [] }
  }
  ...
  for (const reqPath of paths.slice(0, 5)) {
    try {
      ...
      if (fs.existsSync(fullPath) && fs.statSync(fullPath).isFile()) {
        ...
        // ── AUTO-RECOVERY ──
        // The AI guessed a path that doesn't exist. Instead of returning a
        // bare FILE NOT FOUND and hoping the AI retries, we deterministically
        // search for the basename and either load the file transparently
        // (single match) or surface candidates the AI must pick from.
        const recovery = await autoRecoverPath(reqPath)
        if (recovery.exactMatch) {
          try {
            const realFull = recovery.exactMatch
            const relPath = realFull.replace(/^\/app\//, '')
            let content = fs.readFileSync(realFull, 'utf-8')
            const lineCount = content.split('\n').length
            if (content.length > 30000) content = content.slice(0, 30000) + '\n// ... [truncated at 30K]'
            const numbered = addLineNumbers(content)
            const sizeWarning = lineCount > 500
              ? `\n\n**This file is ${lineCount} lines. Use \`search_replace\` with small targeted edits (copy exact text as old_str).**`
              : ''
            results.push(`## \`${relPath}\` (${lineCount} lines) — auto-recovered${sizeWarning}\n\n**NOTE:** \`${reqPath}\` did not exist, but \`${relPath}\` was the unique match for that filename. I loaded it for you. Use this real path (\`${relPath}\`) for any follow-up search_replace / edit_lines calls.\n\n\`\`\`\n${numbered}\n\`\`\``)
            console.log(`[ReadFiles-AutoRecover] ${reqPath} → ${relPath}`)
          } catch (recoverReadErr) {
            results.push(`## \`${reqPath}\` — FILE NOT FOUND (auto-recovery failed: ${recoverReadErr.message}). Use \`exec_command\` with \`find /app -type f -name "*<keyword>*"\` to locate the file.`)
          }
        } else if (recovery.candidates.length > 1) {
          const list = recovery.candidates
            .map(c => `- \`${c.replace(/^\/app\//, '')}\``)
            .join('\n')
          results.push(`## \`${reqPath}\` — FILE NOT FOUND. Candidates with same filename:\n${list}\n\nCall \`read_files\` again with the correct path from the list above. Do NOT ask the user — pick the most likely candidate based on context.`)
          console.log(`[ReadFiles-AutoRecover] ${reqPath} → ${recovery.candidates.length} candidates`)
        } else {
          results.push(`## \`${reqPath}\` — FILE NOT FOUND. Run \`exec_command\` with \`find /app -type f -name "*<keyword>*" 2>/dev/null | head -10\` to discover the real path, then call \`read_files\` again. Do NOT stop and ask the user.`)
        }
      }
    } catch (readErr) {
