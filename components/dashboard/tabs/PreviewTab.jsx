...
export function buildReactPreview({ cssFiles, jsFiles, jsxFiles, tsFiles, usesTailwind, imageAssets }) {
  ...
  const html = [
    ...
    '  /* Shared clean placeholder for ANY failure path — no scary red walls in',
    '     project thumbnails or the main preview pane. Real errors are still',
    '     console.warn\'d and postMessage\'d for debugging tools. */',
    '  function __renderCleanPlaceholder(reason) {',
    '    try { console.warn("[preview] render failed:", reason); } catch(e) {}',
    '    try { window.parent.postMessage({ type: "__PREVIEW_ERROR__", error: String(reason) }, "*"); } catch(e) {}',
    '    var root = document.getElementById("root");',
    '    if (!root) return;',
    '    root.innerHTML = \'\'',
    '    document.body.style.cssText = "margin:0;padding:0;min-height:100vh;background:radial-gradient(ellipse at top, #1e1b4b 0%, #0f172a 60%, #020617 100%);font-family:system-ui,-apple-system,sans-serif;display:flex;align-items:center;justify-content:center;";',
    '    var card = document.createElement("div");',
    '    card.style.cssText = "color:rgba(255,255,255,0.45);text-align:center;font-size:0.85rem;letter-spacing:0.02em;line-height:1.6;padding:1.5rem;";',
    '    card.innerHTML = \'<div style="font-size:1.05rem;color:rgba(255,255,255,0.7);margin-bottom:0.35rem;font-weight:500;">Preview rendering</div><div style="opacity:0.65;">Open the project to interact with it</div>\';',
    '    root.appendChild(card);',
    '  }',
    '  if (_Entry) {',
    '    /* ErrorBoundary catches runtime React errors (missing Providers, undefined',
    '       state, etc.) and routes to the clean placeholder instead of red text. */',
    ...
    '        EB.componentDidCatch = function(err) {',
    '          /* Schedule the clean placeholder swap-out on next tick so React',
    '             finishes its error commit phase before we wipe the root. */',
    '          setTimeout(function() { __renderCleanPlaceholder(err && err.message || err); }, 0);',
    '        };',
    '        EB.prototype.render = function() {',
    '          if (this.state.error) return null; /* placeholder will replace */',
    ...
    '    /* Compile errors: log to console for the project owner but show the',
    '       clean placeholder to viewers. The red error wall used to fill every',
    '       thumbnail on the dashboard. */',
    '    __renderCleanPlaceholder("Compile error: " + __errs.slice(0, 3).join(" | "));',
    '  }',
    ...
    '} catch (_e) {',
    '  /* Last-resort: anything thrown before the boundary mounts. */',
    '  try { __renderCleanPlaceholder(_e.message || _e); }',
    '  catch (e2) {',
    '    /* If even the placeholder fails, fall back to a minimal blank body — never the red wall. */',
    '    document.body.style.background = "#0f172a"; document.getElementById("root").innerHTML = "";',
    '  }',
    '}',
    '',
